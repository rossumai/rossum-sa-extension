...
const listing = (...names: any[]) => ({ result: names });
const err = (message: any, status = 400) => Object.assign(new Error(message), { status });

beforeEach(() => {
  vi.clearAllMocks();
  api.resetCollection();
  vi.mocked(mdh.createCollection).mockResolvedValue({});
  vi.mocked(mdh.renameCollection).mockResolvedValue({});
  vi.mocked(mdh.find).mockResolvedValue({ result: [] });
  vi.mocked(mdh.deleteMany).mockResolvedValue({});
});

describe('resolveCollection', () => {
  it('creates the new collection on a fresh org', async () => {
    vi.mocked(mdh.listCollections).mockResolvedValueOnce(listing('suppliers'));
  ...
  it('does nothing when the new collection already exists', async () => {
    vi.mocked(mdh.listCollections).mockResolvedValueOnce(listing(NEW, 'suppliers'));
    const r = await api.resolveCollection();
  ...
  it('migrates a legacy-only org by renaming', async () => {
    vi.mocked(mdh.listCollections).mockResolvedValueOnce(listing(OLD, 'suppliers'));
    const r = await api.resolveCollection();
  ...
  it('KEEPS WORKING on the legacy collection when the rename cannot happen', async () => {
    ...
    vi.mocked(mdh.listCollections).mockResolvedValueOnce(listing(OLD));
    vi.mocked(mdh.renameCollection).mockRejectedValueOnce(err('permission denied', 403));
    const r = await api.resolveCollection();
  ...
  it('treats a lost rename race as the merge state — both collections now exist', async () => {
    vi.mocked(mdh.listCollections).mockResolvedValueOnce(listing(OLD));
    vi.mocked(mdh.renameCollection).mockRejectedValueOnce(err('target namespace exists')); // LIVE-VERIFIED wording
    const r = await api.resolveCollection();
  ...
  it('rethrows a 401 rather than silently downgrading', async () => {
    vi.mocked(mdh.listCollections).mockResolvedValueOnce(listing(OLD));
    vi.mocked(mdh.renameCollection).mockRejectedValueOnce(err('Session expired.', 401));
  ...
  it('falls back to the legacy name when it cannot even list — never creates a second collection', async () => {
    vi.mocked(mdh.listCollections).mockRejectedValueOnce(err('boom', 500));
    const r = await api.resolveCollection();
    expect(r.use).toBe(OLD);
    expect(mdh.createCollection).not.toHaveBeenCalled();
    expect(mdh.renameCollection).not.toHaveBeenCalled();
...
describe('loadDeliverables in the merge state', () => {
  ...
  it('reads both collections, prefers the newest edit, and reports the legacy remainder', async () => {
    vi.mocked(mdh.find)
      ...
      .mockResolvedValueOnce({
        result: [doc('a', { text: 'old a', editedAt: 100 }), doc('b', { text: 'legacy only' })],
      });
  ...
  it('sends a NEW deliverable to the new collection even while legacy is being read', async () => {
    ...
    expect(mdh.insertOne).toHaveBeenCalledWith(
      NEW,
      expect.objectContaining({ _id: 'z', kind: 'requirement' }),
    );
  });
});
...
describe('revision documents', () => {
  ...
  it('prunes by id, and does not call out at all for an empty plan', async () => {
    ...
    expect(mdh.deleteMany).toHaveBeenCalledWith(NEW, {
      kind: 'revision',
      _id: { $in: ['r1', 'r2'] },
    });
    vi.mocked(mdh.deleteMany).mockClear();
