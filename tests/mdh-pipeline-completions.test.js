...
describe('classifyContext', () => {
  ...
  it('QUERY — operators inside $and/$or query clauses in $match (transparent)', () => {
    expect(ctx('[{ $match: { $and: [ { $e| } ] } }]')).toEqual({ position: 'key', keyCategory: 'QUERY' });
    expect(ctx('[{ $match: { $or: [ { amount: { $g| } } ] } }]')).toEqual({ position: 'key', keyCategory: 'QUERY' });
  });
  it('EXPRESSION — $and/$or inside $expr inherit expression context', () => {
    expect(ctx('[{ $match: { $expr: { $or: [ { $g| } ] } } }]')).toEqual({ position: 'key', keyCategory: 'EXPRESSION' });
  });
  ...
  it('EXPRESSION — deeply nested $group._id composite key stays an expression', () => {
    expect(ctx('[{ $group: { _id: { region: { $toU| } } } }]')).toEqual({ position: 'key', keyCategory: 'EXPRESSION' });
  });
