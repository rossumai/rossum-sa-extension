...
vi.mock('../src/fabry/architect/actions.js', () => ({ refineTurn: vi.fn(), answerRefine: vi.fn(), updateDeliverable: vi.fn() }));
...
let root;
function mount(props) { root = document.createElement('div'); document.body.appendChild(root); act(() => { render(h(RefineDock, props), root); }); return root; }
const flush = () => new Promise((r) => setTimeout(r, 0));
const accept = (r) => [...r.querySelectorAll('button')].find((b) => /accept/i.test(b.textContent));
const discard = (r) => [...r.querySelectorAll('button')].find((b) => /discard/i.test(b.textContent));
function submit(r, v) {
  const i = r.querySelector('input');
  act(() => { i.value = v; i.dispatchEvent(new Event('input', { bubbles: true })); });
  return act(async () => { i.dispatchEvent(new KeyboardEvent('keydown', { key: 'Enter', bubbles: true })); await flush(); });
}
...
describe('RefineDock (docked, inline, design-system AI input)', () => {
  ...
  it('an instruction → refineTurn (fresh chat) → shows the diff + enables Accept; Accept applies via updateDeliverable', async () => {
    actions.refineTurn.mockResolvedValue({ chatId: 'chat_1', proposal: '# refined requirement' });
    const r = mount({ deliverable: { id: 'a', text: '# base requirement' } });
    await submit(r, 'tighten it');
    expect(actions.refineTurn).toHaveBeenCalledWith(expect.objectContaining({ chatId: null, deliverableText: '# base requirement', instruction: 'tighten it' }));
  ...
  it('a follow-up instruction (before accept) reuses the chatId so Fabry builds on its last proposal', async () => {
    ...
    expect(actions.refineTurn).toHaveBeenNthCalledWith(2, expect.objectContaining({ chatId: 'chat_1', instruction: 'name the field' }));
  });
  it('renders the agent questions (interactive elements) inline when a turn asks — no diff, no Accept', async () => {
    actions.refineTurn.mockResolvedValue({ chatId: 'c', questions: [{ question: 'Which queue?' }] });
    const r = mount({ deliverable: { id: 'a', text: '# base' } });
    await submit(r, 'tighten');
  ...
  it('answering the questions calls answerRefine (same chat) and shows the resulting diff', async () => {
    actions.refineTurn.mockResolvedValue({ chatId: 'c', questions: [{ question: 'Which queue?' }] });
    actions.answerRefine.mockResolvedValue({ chatId: 'c', proposal: '# refined for the Invoices queue' });
    const r = mount({ deliverable: { id: 'a', text: '# base' } });
    await submit(r, 'tighten');
    const qin = r.querySelector('.fabry-q-input');
    act(() => { qin.value = 'Invoices'; qin.dispatchEvent(new Event('input', { bubbles: true })); });
    await act(async () => { r.querySelector('.fabry-q-submit').click(); await flush(); });
    expect(actions.answerRefine).toHaveBeenCalledWith(expect.objectContaining({ chatId: 'c', answers: [{ question: 'Which queue?', answer: 'Invoices' }] }));
    expect(r.querySelector('.' + diffStyles.diff)).toBeTruthy();
  });
  it('an empty proposal is never shown as a diff and cannot be accepted (guards a destructive Accept)', async () => {
    actions.refineTurn.mockResolvedValue({ chatId: 'c', proposal: '' });
    const r = mount({ deliverable: { id: 'a', text: '# base requirement' } });
  ...
  it('Discard clears the card and starts a fresh chat on the next instruction (no updateDeliverable)', async () => {
    actions.refineTurn.mockResolvedValue({ chatId: 'chat_1', proposal: '# refined' });
    const r = mount({ deliverable: { id: 'a', text: '# base' } });
    await submit(r, 'tighten');
    act(() => { discard(r).click(); });
    ...
    expect(actions.refineTurn).toHaveBeenLastCalledWith(expect.objectContaining({ chatId: null, instruction: 'again' }));
  });
});
