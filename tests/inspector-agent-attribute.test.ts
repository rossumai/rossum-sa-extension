...
import {
  ...
  buildAttributionPrompt as _bap,
  parseAttribution,
  runAttribution,
  parseFieldBatch,
  buildFieldBatchPrompt,
  runFieldBatchAttribution,
} from '../src/inspector/agentAttribute.js';
...
describe('buildAttributionPrompt', () => {
  ...
  it('reject prompt states the reject question + reason', () => {
    const p = buildAttributionPrompt({
      kind: 'reject',
      target: { rejectedAt: '2026-01-01', reason: 'bad total' },
      ...base,
    });
  ...
  it('includes the queue id so the agent can search that queue with its tools', () => {
    const p = buildAttributionPrompt({
      kind: 'reject',
      target: {},
      annotation: { id: 42, status: 'rejected', queueId: '3927215' },
      candidates: [],
    });
  ...
  it('caps the prompt under the agent 50k-char limit even with a huge candidate list, keeping the JSON instruction', () => {
    const many = Array.from({ length: 3000 }, (_, i) => ({
      id: i,
      name: `Extension number ${i} with a fairly long descriptive name`,
      type: 'function',
    ...
    const pf = buildFieldBatchPrompt(
      Array.from({ length: 6 }, (_, i) => ({ schemaId: `f${i}`, value: 'v' })),
      { annotation: { id: 1, status: 'x' }, candidates: many },
    );
...
describe('parseAttribution', () => {
  it('parses a valid verdict', () => {
    expect(
      parseAttribution(
        '{"culprit":{"kind":"hook","id":7,"name":"Rejector"},"confidence":"high","explanation":"it calls reject()"}',
      ),
    ).toEqual({
      culprit: { kind: 'hook', id: 7, name: 'Rejector' },
      confidence: 'high',
      explanation: 'it calls reject()',
    });
  });
  it('maps unknown culprit to null and clamps bad confidence to low', () => {
    expect(
      parseAttribution(
        'prose {"culprit":{"kind":"unknown"},"confidence":"???","explanation":"n/a"} tail',
      ),
  ...
  it('extracts the first complete object even when prose with a stray brace follows', () => {
    const reply =
      '{"culprit":{"kind":"hook","id":7,"name":"R"},"confidence":"high","explanation":"e"}\n\nNote: see {the docs}.';
    expect(parseAttribution(reply)).toEqual({
      culprit: { kind: 'hook', id: 7, name: 'R' },
      confidence: 'high',
      explanation: 'e',
    });
  });
...
describe('runAttribution', () => {
  const ctx = {
    annotation: { id: 1, status: 'rejected' },
    target: { rejectedAt: 't' },
    candidates: [],
  };
  it('primes the cautious persona then returns the parsed verdict', async () => {
    const agentApi = mockAgentApi(
      '{"culprit":{"kind":"hook","id":7,"name":"Rejector"},"confidence":"high","explanation":"x"}',
    );
...
describe('buildAttributionPrompt new kinds', () => {
  it('frames a message question read-only + JSON-only', () => {
    const p = _bap({
      ...
      target: { level: 'error', content: 'Total mismatch', schemaId: 'amount_due' },
      candidates: [],
    });
    expect(p).toMatch(/READ-ONLY/);
  ...
  it('frames a blocker explanation and an export question', () => {
    expect(
      _bap({ kind: 'blocker', target: { type: 'custom_x', schemaId: 'iban' }, candidates: [] }),
    ).toMatch(/custom_x/);
    expect(_bap({ kind: 'export', target: { error: 'HTTP 500' }, candidates: [] })).toMatch(
      /HTTP 500/,
    );
  });
});
...
describe('parseFieldBatch', () => {
  it('parses a fields array; unknown culprit → null; bad confidence → low', () => {
    const out = parseFieldBatch(
      '{"fields":[{"schema_id":"terms","culprit":{"kind":"rule","id":7,"name":"R"},"confidence":"high","explanation":"e"},{"schema_id":"iban","culprit":{"kind":"unknown"},"confidence":"bogus","explanation":"x"}]}',
    );
    expect(out.fields[0]).toEqual({
      schema_id: 'terms',
      culprit: { kind: 'rule', id: 7, name: 'R' },
      confidence: 'high',
      explanation: 'e',
    });
    ...
    const agentApi = {
      ...
      streamMessage: vi.fn(async (_id, content, { onEvent }) => {
        ...
        onEvent({
          type: 'text-delta',
          delta:
            '{"fields":[{"schema_id":"terms","culprit":{"kind":"rule","id":7,"name":"R"},"confidence":"medium","explanation":"e"}]}',
        });
        onEvent({ type: 'finish' });
      }),
    };
    const { verdicts } = await runFieldBatchAttribution({
      agentApi,
      items: [{ key: 'field:terms', schemaId: 'terms', value: '2/10' }],
      context: { annotation: {}, candidates: [], fields: {} },
    });
    expect(agentApi.streamMessage).toHaveBeenCalledWith(
      'c1',
      '/persona cautious',
