...
describe('previewMatch', () => {
  ...
  it('forwards an AbortSignal to both aggregate calls', async () => {
    ...
    expect(vi.mocked(api.aggregate).mock.calls[0][2]).toEqual({ signal: ac.signal });
    expect(vi.mocked(api.aggregate).mock.calls[1][2]).toEqual({ signal: ac.signal });
  });
});
...
describe('runBulkDelete', () => {
  ...
  it('skips the snapshot and undo toast when count exceeds UNDO_LIMIT', async () => {
    vi.mocked(api.deleteMany).mockResolvedValueOnce({ result: { deleted_count: 5000 } });
    const onSuccess = vi.fn();
  ...
  it('skips the undo toast when count is 0', async () => {
    vi.mocked(api.deleteMany).mockResolvedValueOnce({ result: { deleted_count: 0 } });
