...
describe('runChunkedInsert', () => {
  ...
  it('chunks at the shared dataset BATCH_SIZE (1000) and calls insertMany with ordered:false', async () => {
    ...
    expect(vi.mocked(api.insertMany).mock.calls[0][1]).toHaveLength(BATCH_SIZE);
    expect(vi.mocked(api.insertMany).mock.calls[0][2]).toBe(false); // ordered: false
  ...
  it('continues past a failed batch and records the failed range', async () => {
    ...
    vi.mocked(api.find).mockResolvedValueOnce({ result: [{ _id: 'id-2' }] }); // 1 of 2 landed
  ...
  it('stops between batches when the abort signal fires', async () => {
    ...
    let callCount = 0;
    vi.mocked(api.insertMany).mockImplementation(async () => {
      callCount++;
