...
describe('attestStep', () => {
  ...
  it('refuses a nonexistent mission id — nothing written', async () => {
    await store.startTrack();
    const before = state[PROGRESS_KEY]['https://x.rossum.app'];
    const setCalls = globalThis.chrome.storage.local.set.mock.calls.length;
    ...
    expect(globalThis.chrome.storage.local.set.mock.calls.length).toBe(setCalls);
  });
