...
describe('inspector api', () => {
  ...
  it('getAnnotation hits the annotation path', async () => {
    ...
    expect(a.id).toBe(5);
    expect(vi.mocked(globalThis.fetch).mock.calls[0][0]).toBe('https://api.example.rossum.ai/api/v1/annotations/5');
  });
});
...
describe('workflow + relation endpoints', () => {
  ...
  it('listWorkflowRuns hits /workflow_runs?', async () => {
    const calls: any = [];
    globalThis.fetch = (vi.fn(async (url) => { calls.push(String(url)); return { ok: true, status: 200, json: async () => ({ results: [], pagination: {} }) }; }) as any);
  ...
  it('listAnnotationsByIds sends a csv id filter with sideload', async () => {
    const calls: any = [];
    globalThis.fetch = (vi.fn(async (url) => { calls.push(String(url)); return { ok: true, status: 200, json: async () => ({ results: [], documents: [], queues: [] }) }; }) as any);
  ...
  it('getBlob sends the auth header and returns the blob', async () => {
    const blob = { size: 3 };
    let seenAuth = null;
    globalThis.fetch = (vi.fn(async (url, opts) => { seenAuth = opts.headers.Authorization; return { ok: true, status: 200, blob: async () => blob }; }) as any);
