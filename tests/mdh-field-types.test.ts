...
describe('foldBsonType', () => {
  it('folds numeric subtypes to number, others to category/other', () => {
    expect(['int', 'long', 'double', 'decimal'].map(foldBsonType)).toEqual([
      'number',
      'number',
      'number',
      'number',
    ...
    expect(['date', 'objectId', 'array', 'object'].map(foldBsonType)).toEqual([
      'other',
      'other',
      'other',
      'other',
...
describe('transformTypeBuckets', () => {
  ...
  it('mixed field picks the dominant category with a share', () => {
    const info = transformTypeBuckets([
      { _id: 'string', count: 8 },
      { _id: 'int', count: 2 },
    ])!;
    expect(info.dominant).toBe('string');
...
describe('deriveResolvedType', () => {
  ...
  const base = {
    override: undefined,
    fieldMap: { code: { field: 'code', op: '$eq' } },
    fieldTypeInfo: strInfo,
    parsedOk: true,
  };
  ...
  it('override beats dataset', () => {
    expect(deriveResolvedType('code', { ...base, override: 'number' })).toMatchObject({
      type: 'number',
      source: 'override',
    });
  });
  it('mixed field marks source mixed', () => {
    ...
    expect(deriveResolvedType('code', { ...base, fieldTypeInfo: mixed })).toMatchObject({
      type: 'string',
      source: 'mixed',
    });
  });
  it('invalid pipeline → invalid (value-based)', () => {
    expect(deriveResolvedType('code', { ...base, parsedOk: false })).toEqual({
      type: undefined,
      source: 'invalid',
    });
  });
  ...
  it('ambiguous → ambiguous', () => {
    expect(deriveResolvedType('x', { ...base, fieldMap: { x: { ambiguous: true } } })).toEqual({
      type: undefined,
      source: 'ambiguous',
    });
  });
  it('field type not yet resolved → detecting', () => {
    expect(deriveResolvedType('code', { ...base, fieldTypeInfo: undefined })).toMatchObject({
      type: undefined,
      source: 'detecting',
    });
  });
  it('null field info → no-data (value-based)', () => {
    expect(deriveResolvedType('code', { ...base, fieldTypeInfo: null })).toMatchObject({
      type: undefined,
      source: 'no-data',
    });
  });
  it('non-primitive dominant → other (value-based) with detected bson', () => {
    const oid = {
      dominant: 'other',
      dominantBson: 'objectId',
      share: 1,
      distribution: [],
      mixed: false,
    };
    expect(deriveResolvedType('code', { ...base, fieldTypeInfo: oid })).toMatchObject({
      type: undefined,
      source: 'other',
      detectedBson: 'objectId',
    });
  });
});
...
describe('deriveResolvedType — fieldTypeInfo arg', () => {
  const fieldMap = {
    cust: { field: 'customer_match', collection: '_PROD_material_match', op: '$eq' },
  };
  ...
  it('undefined info → detecting; null info → no-data; unmapped → no-field', () => {
    expect(
      deriveResolvedType('cust', { fieldMap, fieldTypeInfo: undefined, parsedOk: true }),
    ).toMatchObject({ type: undefined, source: 'detecting' });
    expect(
      deriveResolvedType('cust', { fieldMap, fieldTypeInfo: null, parsedOk: true }),
    ).toMatchObject({ type: undefined, source: 'no-data' });
    expect(
      deriveResolvedType('nope', { fieldMap, fieldTypeInfo: undefined, parsedOk: true }),
