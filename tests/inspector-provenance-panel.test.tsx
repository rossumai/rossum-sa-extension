...
describe('ProvenancePanel field attribution', () => {
        ...
        content: [
          {
            ...
            schema_id: 'terms',
            content: { value: '2/10' },
            validation_sources: ['rules'],
          },
        ],
      },
      resolved: { hooksById: {} },
    };
    store.setAttribution(fieldKey('terms'), {
      status: 'done',
      verdict: {
        culprit: { kind: 'rule', id: 7, name: 'Set terms' },
        confidence: 'medium',
        explanation: 'writes terms',
      },
      source: 'ai',
    });
  ...
  it('renders a confidence bar with threshold for engine fields', () => {
    store.data.value = {
        ...
        content: [
          {
            category: 'datapoint',
            schema_id: 'terms',
            content: { value: '2/10', rir_confidence: 0.31 },
      ...
      resolved: {
        hooksById: {},
        schema: { content: [{ category: 'datapoint', id: 'terms', score_threshold: 0.8 }] },
        queue: null,
      },
    };
    render(<ProvenancePanel />, root);
    ...
    expect(el.querySelector('.inspector-conf').getAttribute('title')).toMatch(
      /tick marks the automation threshold/i,
    );
  });
        ...
        content: [
          {
            category: 'datapoint',
            schema_id: 'terms',
            content: { value: 'x', rir_confidence: 1.4 },
            validation_sources: ['score'],
          },
        ],
      },
