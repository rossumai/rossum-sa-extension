...
describe('formatResource', () => {
  it("shows a function hook's implementation as code, not as escaped JSON", () => {
    const raw = JSON.stringify({
      id: 42,
      name: 'Matcher',
      config: { runtime: 'python3.12', code: HOOK_CODE },
    });
  ...
  it('shows JSON for any other resource — a schema, a queue', () => {
    const out = formatResource(
      JSON.stringify({ id: 3, name: 'Invoices', content: [{ category: 'section' }] }),
    );
    expect(out.language).toBe('json');
...
describe('resource views — a hook is two files behind one API resource', () => {
  ...
  describe('splitResourceView', () => {
    it('claims the marker and hands back the path that should actually be fetched', () => {
      expect(splitResourceView('/api/v1/hooks/42?view=json')).toEqual({
        path: '/api/v1/hooks/42',
        view: 'json',
      });
      expect(splitResourceView('/api/v1/hooks/42?view=code')).toEqual({
        path: '/api/v1/hooks/42',
        view: 'code',
      });
    });
    ...
    it('leaves a path without a marker completely alone', () => {
      expect(splitResourceView('/api/v1/hooks/42')).toEqual({
        path: '/api/v1/hooks/42',
        view: null,
      });
    ...
    it('keeps every other query parameter, in order', () => {
      expect(splitResourceView('/api/v1/annotations?id=1,2&view=json&sideload=documents')).toEqual({
        path: '/api/v1/annotations?id=1%2C2&sideload=documents',
        view: 'json',
      });
    });
    ...
    it('does NOT claim a `view` value it does not know — that one belongs to the server', () => {
      ...
      // silently eating it.
      expect(splitResourceView('/api/v1/x?view=compact')).toEqual({
        path: '/api/v1/x?view=compact',
        view: null,
      });
    });
  });
  ...
  describe('withResourceView', () => {
    it('builds the sibling key the switcher navigates to', () => {
      ...
      expect(withResourceView('/api/v1/hooks/42?view=json', 'code')).toBe(
        '/api/v1/hooks/42?view=code',
      );
      expect(withResourceView('/api/v1/annotations?id=1', 'code')).toBe(
        '/api/v1/annotations?id=1&view=code',
      );
    });
    ...
    it('clears the marker for null, giving the plain key an older build also understands', () => {
      ...
      // Deliberately not a declared view — the assertion is that it is ignored.
      expect(withResourceView('/api/v1/hooks/42?view=json', 'nonsense' as any)).toBe(
        '/api/v1/hooks/42',
      );
    });
    ...
    it('round-trips every declared view', () => {
      for (const v of RESOURCE_VIEWS) {
        expect(splitResourceView(withResourceView('/api/v1/hooks/42', v))).toEqual({
          path: '/api/v1/hooks/42',
          view: v,
        });
      }
  ...
  it('the fetcher never sends our marker to the API', async () => {
    const seen: any = [];
    const fetchImpl = (url: any) => {
      seen.push(url);
      return Promise.resolve({ ok: true, text: () => Promise.resolve(HOOK) });
    };
    const fetchResource = createResourceFetcher({
      domain: 'https://o.rossum.app',
      token: 't',
      fetchImpl,
    });
