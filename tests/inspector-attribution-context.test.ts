...
import {
  ...
  gatherExportContext,
  gatherFieldsContext,
} from '../src/inspector/attributionContext.js';
...
function fakeStore(overrides: any = {}) {
  const d = {
    ...
    resolved: {
      hooksById: {
        7: { id: 7, name: 'H', type: 'function', events: ['annotation_content.confirm'] },
      },
    },
    ...overrides.data,
  };
...
describe('gatherRejectContext', () => {
  ...
  it('falls back to api.listHooks (compact) when resolved.hooksById is empty', async () => {
    const a = {
      listHooks: vi.fn(async () => [{ id: 5, name: 'WH', type: 'webhook', events: [] }]),
    };
    const store = fakeStore({
      data: {
        resolved: { hooksById: {} },
        annotation: { id: 1, status: 'rejected', queue: 'https://x/api/v1/queues/9' },
      },
    });
...
describe('gatherLabelContext', () => {
  it('targets the label and includes compact candidates', async () => {
    const ctx = await gatherLabelContext({
      api: {},
      store: fakeStore(),
      labelId: '3',
      labelName: 'Urgent',
...
const store = {
  data: {
    value: {
      annotation: { id: 9, status: 'to_review', queue: 'https://h/api/v1/queues/3' },
      content: { content: [] },
      resolved: { hooksById: { 5: { id: 5, name: 'H', type: 'function', events: [] } } },
    },
  },
...
describe('new gatherers', () => {
  it('gatherMessageContext carries the message target + candidates + queue id', async () => {
    const ctx = await gatherMessageContext({
      api: apiSimple,
      store,
      message: { level: 'error', content: 'x', schemaId: 'iban' },
    });
  ...
  it('gatherBlockerContext + gatherExportContext carry their targets', async () => {
    expect(
      (await gatherBlockerContext({ api: apiSimple, store, blocker: { type: 't', schemaId: 'f' } }))
        .target,
    ).toEqual({ type: 't', schemaId: 'f' });
    expect((await gatherExportContext({ api: apiSimple, store, error: 'E' })).target).toEqual({
      error: 'E',
    });
  });
  it('gatherExportContext narrows candidates to export-event hooks', async () => {
    const storeExp = {
      data: {
        value: {
          annotation: { id: 9, status: 'failed_export', queue: 'https://h/api/v1/queues/3' },
          content: { content: [] },
          resolved: {
            hooksById: {
              5: {
