...
const BADGED = '<h2 id="3-architecture" tabindex="-1" class="has-state-label">3. Architecture'
...
describe('code copy', () => {
  it('adds one button per fenced block, scoped to the document', () => {
    ...
    expect(root.querySelector('.code-copy-btn').getAttribute('aria-label')).toBe('Copy code to clipboard');
  });
...
describe('section preview', () => {
  ...
  it('ignores links whose target is not a heading', () => {
    ...
    root.querySelector('a[href="#nothing"]').dispatchEvent(new window.MouseEvent('mouseover', { bubbles: true }));
...
describe('source viewer', () => {
  ...
  function withModal() {
    ...
    overlay.innerHTML = '<div id="srcTitle"></div><div id="srcPath"></div>'
...
describe('source viewer — an extension is two files (definition + implementation)', () => {
  ...
  const viewButtons = () => [...document.getElementById('srcViews').querySelectorAll('.source-view-btn')];
  const labels = () => viewButtons().map((b) => b.textContent);
  const pressed = () => viewButtons().filter((b) => b.getAttribute('aria-pressed') === 'true').map((b) => b.textContent);

  function mountHook(raw = HOOK) {
    const { root } = mountDoc(`Read [the hook](${ORIGIN}/api/v1/hooks/42) now.\n`);
    withModal();
    const resolve = vi.fn((key) => Promise.resolve(formatResource(raw, splitResourceView(key).view)));
    initSourceViewer(root, {
      isSourceLink: (href) => !!href && href.includes('/api/v1/'),
      keyFor: (href) => new URL(href).pathname,
      resolve,
  ...
  it('switching to Definition shows the WHOLE hook — the defect this fixes', async () => {
    ...
    await vi.waitFor(() => expect(viewButtons().length).toBe(2));
    viewButtons()[1].click();
    await vi.waitFor(() => expect(document.getElementById('srcCode').textContent).toMatch(/"name": "Index doctor"/));
...
describe('cross-deliverable preview', () => {
  ...
  const OTHER = () => {
    ...
    body.innerHTML = '<h1 id="architecture">Architecture</h1><p>Intro of the other doc.</p>'
      + '<h2 id="queues">Queues</h2><p>Queue detail.</p><p>More queue detail.</p>'
      + '<h2 id="hooks">Hooks</h2><p>Hook detail.</p>';
    return body;
  };
  const resolveExternal = (href) => {
    if (!href.startsWith('architecture')) return null;
    const frag = href.split('#')[1] || '';
    return { body: OTHER(), headingId: frag, title: 'Architecture' };
  };

  function hover(root, selector) {
    vi.advanceTimersByTime(1);
    root.querySelector(selector).dispatchEvent(new window.MouseEvent('mouseover', { bubbles: true }));
  ...
  it('previews the referenced section of another deliverable, and says which one', () => {
    ...
    const from = card.querySelector('.section-preview-from');
  ...
  it('the footer opens that deliverable instead of scrolling this one', () => {
    ...
    initSectionPreview(root, root, { resolveExternal, onOpenExternal: (href) => opened.push(href) });
    const card = hover(root, '.markdown-body a');
  ...
  it('ignores a reference the resolver cannot place, and external links entirely', () => {
    vi.useFakeTimers();
    const { root } = mountDoc('A [renamed doc](gone.md#x) and an [outside link](https://example.test/a).\n');
    initSectionPreview(root, root, { resolveExternal });
  ...
  it('still previews same-document sections when a resolver is present', () => {
    ...
    expect(card.textContent).toMatch(/Body of A/);
    expect(card.querySelector('.section-preview-from').hidden).toBe(true);
