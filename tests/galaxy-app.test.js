...
const sceneCalls = { created: 0, lastData: null, destroyed: 0, hoverCb: null, clickCb: null };
vi.mock('../src/galaxy/scene.js', () => ({
  createScene: () => {
    sceneCalls.created++;
    return {
      setData: (d) => { sceneCalls.lastData = d; },
      onHover: (cb) => { sceneCalls.hoverCb = cb; },
      onClick: (cb) => { sceneCalls.clickCb = cb; },
      focus: () => {},
      setIdleSpin: () => {},
      setVisibleTypes: () => {},
...
describe('Galaxy App', () => {
  ...
  it('pushes graph changes into the live scene after mount', async () => {
    mount(true);
    store.graph.value = { nodes: [{ id: 'queue:2', type: 'queue', rawId: '2', name: 'Q2', color: '#29d4c5', val: 5 }], links: [] };
    await vi.waitFor(() => {
      expect(sceneCalls.lastData.nodes.some((n) => n.id === 'queue:2')).toBe(true);
    });
  });
});
