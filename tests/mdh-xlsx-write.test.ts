...
async function buildWorkbook(docs: any, columns: any, opts = {}) {
  const ser = buildXlsxSerializer({ sheetName: 'Data', columns, ...opts });
  const parts: any = [];
  const writeBytes = async (chunk: any) => {
  ...
  // writeDocs/finish close over the sink handed to start() — passing it again is dead.
  await ser.writeDocs(docs);
  await ser.finish();
