...
describe('DocumentStrip', () => {
  it('says no document is open when there is no annotation', () => {
    render(<DocumentStrip ctx={CTX} annotationId={null} />, root);
  ...
  it('keeps the id when the name lookup fails', async () => {
    ...
    render(<DocumentStrip ctx={CTX} annotationId="99" />, root);
  ...
  it('does not fetch without a token', () => {
    ...
    vi.stubGlobal('fetch', fetchMock);
    render(<DocumentStrip ctx={{ domain: CTX.domain, token: null }} annotationId="5" />, root);
