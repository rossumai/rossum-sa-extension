...
describe('SpecView', () => {
  it('offers a two-way mode switch and no combined mode', () => {
    const labels = [...mount().querySelectorAll('.fabry-spec-modes button')].map(
      (b) => b.textContent,
    );
  ...
  it('switching mode changes the mode and keeps the chrome', () => {
    ...
    act(() => {
      (
        [...root.querySelectorAll('.fabry-spec-modes button')].find((b) =>
          /Edit/.test(b.textContent),
        ) as HTMLElement
      ).click();
    });
  ...
  it('clicking a section header targets that deliverable, and moves an existing pin', () => {
    const root = mount();
    act(() => {
      root.querySelectorAll<HTMLElement>('.fabry-spec-sec-hd')[1].click();
    });
    expect(store.spyTarget.value).toBe('d2');
    expect(store.pinnedTarget.value).toBe(null); // no pin was set, so none is created
    act(() => {
      store.setPinnedTarget('d2');
      root.querySelectorAll<HTMLElement>('.fabry-spec-sec-hd')[0].click();
    });
  ...
  it('collapses the inspector, and offers NO way to collapse the deliverable list', () => {
    ...
    act(() => {
      root.querySelector<HTMLElement>('[data-act="toggle-rail"]')!.click();
    });
  ...
  it('registers a navigator that jumps through the fast tween, not the browser slow one', async () => {
    ...
    act(() => {
      store.navigateOutline(null, 'd1');
    }); // a row click: the section itself
  ...
  it('renders every deliverable in edit mode as an editable field, with the same headers', () => {
    ...
    expect(
      [...root.querySelectorAll('.cm-mock')].map((t) => (t as HTMLInputElement).value).join(' '),
    ).toMatch(/alpha/);
    expect(
      [...root.querySelectorAll('.cm-mock')].map((t) => (t as HTMLInputElement).value).join(' '),
    ).toMatch(/beta/);
...
describe('review at document width', () => {
  ...
  it('renders the requested panel above its own section, and closes', () => {
    ...
    act(() => {
      store.setReviewTarget({ id: 'd2', kind: 'history' });
    });
    ...
    act(() => {
      host.querySelector<HTMLElement>('[data-act="review-close"]')!.click();
    });
    expect(root.querySelector('.fabry-spec-review')).toBe(null);
  });
  ...
  it('shows Refine for the refine kind', () => {
    ...
    const root = mount();
    act(() => {
      store.setReviewTarget({ id: 'd1', kind: 'refine' });
    });
  ...
  it('closes on Escape', () => {
    ...
    const root = mount();
    act(() => {
      store.setReviewTarget({ id: 'd1', kind: 'history' });
    });
