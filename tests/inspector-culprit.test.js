...
describe('labels', () => {
  it('extractLabelRules pulls label refs from any label-named payload key (enabled rules/actions only)', () => {
    const rules = [
      { id: 7, name: 'Tag high value', enabled: true, trigger_condition: 'field.amount_total > 10000', actions: [{ enabled: true, type: 'add_label', payload: { label: 'https://h/v1/labels/9898' } }] },
      { id: 8, name: 'disabled', enabled: false, actions: [{ enabled: true, payload: { labels: ['https://h/v1/labels/1'] } }] },
      { id: 9, name: 'no labels', enabled: true, actions: [{ enabled: true, type: 'show_message', payload: { content: 'x' } }] },
    ];
    const lr = extractLabelRules(rules);
    expect(lr.length).toBe(1);
    expect(lr[0]).toMatchObject({ ruleId: 7, ruleName: 'Tag high value', trigger: 'field.amount_total > 10000', labelIds: ['9898'] });
  });

  it('labelAttribution: applied-by-rule, applied-manually, governed-not-applied', () => {
    const labelsById = { 9898: { id: '9898', name: 'Priority: High', color: '#f00' }, 9905: { id: '9905', name: 'Duplicate suspected' }, 9901: { id: '9901', name: 'Needs review' } };
    const labelRules = [
      { ruleId: 7, ruleName: 'Tag high value', trigger: 'field.amount_total > 10000', labelIds: ['9898'] },
    ...
    expect(applied.find((l) => l.id === '9898')).toMatchObject({ name: 'Priority: High', rule: { name: 'Tag high value' }, reliability: REL.VERIFIED });
    const manual = applied.find((l) => l.id === '9901');
    expect(manual.rule).toBe(null);
    expect(manual.reliability).toBe(REL.UNAVAILABLE);
    expect(notApplied).toHaveLength(1);
    expect(notApplied[0]).toMatchObject({ id: '9905', name: 'Duplicate suspected', rule: { name: 'Flag dupes' } });
  });
});
...
describe('extension label attribution', () => {
  ...
  it('exportHookCandidates: lists export hooks + matches the failing one from logs', () => {
    const ehooks = [
      ...
      { id: 71, name: 'Export: Slack', type: 'webhook', active: true, events: ['annotation_content.export'] },
      { id: 72, name: 'Pre: validator', type: 'function', active: true, events: ['annotation_content.started'] },
    ];
    ...
    const withLog = exportHookCandidates(ehooks, [{ hook_id: 70, action: 'export', status: 'failed', message: '502 Bad Gateway' }]);
    expect(withLog.failing).toMatchObject({ hookId: 70, hookName: 'Export: ERP push', error: '502 Bad Gateway' });
  });

  it('buildPipeline orders by run_after, groups by phase, overlays logs, drops inactive', () => {
    const hooks = [
      { id: 1, name: 'A init', type: 'function', active: true, events: ['annotation_content.initialize'], run_after: [] },
      { id: 2, name: 'B init', type: 'function', active: true, events: ['annotation_content.initialize'], run_after: ['https://h/v1/hooks/1'] },
      { id: 3, name: 'Inactive', type: 'function', active: false, events: ['annotation_content.initialize'], run_after: [] },
      { id: 4, name: 'Export push', type: 'webhook', active: true, events: ['annotation_content.export'], run_after: [] },
    ];
    const logs = [{ hook_id: 4, action: 'export', status: 'failed', log_level: 'ERROR', message: '502', start: '2026-01-01T00:00:00.000Z', end: '2026-01-01T00:00:00.020Z', status_code: 0 }];
...
describe('provenance + detective', () => {
  it('buckets a field value to its primary validation source (verified values)', () => {
    ...
    const dm = fieldProvenance({ schema_id: 'y', validation_sources: ['data_matching', 'non_required'], content: { value: 'b' } });
    expect(dm.primary).toBe('data_matching');
    const none = fieldProvenance({ schema_id: 'z', validation_sources: [], content: { value: 'c' } });
    expect(none.primary).toBe('none');
    // full verified source set: human > data_matching > connector > rules > formula > score
    expect(fieldProvenance({ schema_id: 'a', validation_sources: ['score', 'formula'], content: {} }).primary).toBe('formula');
    expect(fieldProvenance({ schema_id: 'b', validation_sources: ['connector'], content: {} }).primary).toBe('connector');
    expect(fieldProvenance({ schema_id: 'c', validation_sources: ['rules', 'score'], content: {} }).primary).toBe('rules');
    expect(fieldProvenance({ schema_id: 'd', validation_sources: ['NA'], content: {} }).primary).toBe('none');
  });
  ...
  it('matchingExtensions finds MDH/matching hooks (settings.configurations | configs)', () => {
    ...
    expect(matchingExtensions(hooks).map((x) => x.hookId).sort()).toEqual([1, 2]);
  });
  ...
  it('matchConfigsForField pins a data_matching field to the specific MDH config (not every MDH hook)', () => {
    const hooks = [
      { id: 1, name: 'Master Data Hub', settings: { configurations: [
        ...
        { name: 'GL multi', mapping: { target_schema_id: 'item_gl' }, additional_mappings: [{ dataset_key: 'cc', target_schema_id: 'item_cost_center' }] },
      ] } },
      { id: 2, name: 'Other MDH', settings: { configs: [{ name: 'legacy', mapping: { target_schema_id: 'vat_code' } }] } },
      { id: 3, name: 'Not a matcher', settings: {} },
    ];
    expect(matchConfigsForField('sender_match', hooks)).toEqual([{ hookId: 1, hookName: 'Master Data Hub', configName: 'Supplier match' }]);
    // additional (multi-field) mapping target is found
    expect(matchConfigsForField('item_cost_center', hooks)).toEqual([{ hookId: 1, hookName: 'Master Data Hub', configName: 'GL multi' }]);
    // legacy settings.configs
    expect(matchConfigsForField('vat_code', hooks)).toEqual([{ hookId: 2, hookName: 'Other MDH', configName: 'legacy' }]);
