...
function mount() { const root = document.createElement('div'); render(h(Panel, null), root); return root; }
function rerender(root) { render(h(Panel, null), root); }
...
describe('DevTools Panel', () => {
  ...
  it('Save opens the diff overlay', async () => {
    ...
    store.patchTab(t.id, { original: { name: 'A' }, buffer: JSON.stringify({ name: 'B' }), dirty: true });
    const root = mount();
    root.querySelector('.rawjson-save').click();
  ...
  describe('tab bar', () => {
    ...
    it('closing a link tab (x) removes it and activates a neighbor; the page tab has no close button', () => {
      ...
      linkTabEl.querySelector('.rawjson-tab-close').click();
      rerender(root);
  ...
  it('capture-phase Cmd+F with real editor calls preventDefault and focus', async () => {
    ...
    render(h(JsonCodeEditor, { tabId: t.id }), edRoot);
  ...
  describe('context menu', () => {
    it('renders the context menu when linkMenu is set and clicking it opens a tab and clears the menu', async () => {
      ...
      const menu = root.querySelector('.rawjson-linkmenu');
  ...
  describe('tab context menu', () => {
    ...
    it('page tab has the rawjson-tab--page class and is not draggable', () => {
      ...
      expect(pageTab.draggable).toBe(false);
    });
    ...
    it('link tabs have the draggable attribute', () => {
      ...
      expect(linkTab.draggable).toBe(true);
    });
