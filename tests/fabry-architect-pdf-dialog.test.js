...
function openAndMount({ deliverableTitle = 'Scope', count = 3 } = {}) {
  const confirmed = [];
  openPdfDialog({ deliverableTitle, count }, (arg) => confirmed.push(arg));
  const root = document.createElement('div');
  document.body.appendChild(root);
  act(() => { render(modalContent.value.render(), root); });
  return { root, confirmed };
}
const rows = (root) => [...root.querySelectorAll('.fabry-arch-pdf-row')];
const input = (root, label) => rows(root).find((r) => r.textContent.includes(label)).querySelector('input');
...
describe('setPdfOptions', () => {
  it('coerces to booleans and drops unknown keys', () => {
    store.setPdfOptions({ contents: 1, states: '', verdicts: true, bogus: 'x' });
    expect(store.pdfOptions.value).toEqual({ contents: true, verdicts: true });
