...
const names = (root: any) => [...root.querySelectorAll('.collection-item-name')].map((e) => e.textContent);
...
describe('Sidebar — filter the collection list by name', () => {
  ...
  it('narrows the extension-collections group too, and opens it so a match is visible', async () => {
    ...
    expect(group.querySelector('.collection-hidden-toggle')!.getAttribute('aria-expanded')).toBe('true');
    expect([...group.querySelectorAll('.collection-item-name')].map((e: any) => e.textContent))
      .toEqual([COLLECTION]);
    // the customer's own collection does not match, so the main list is empty
    expect(root.querySelectorAll('.collection-list')[0].querySelectorAll('.collection-item')).toHaveLength(0);
  });
  ...
  it('emphasises matches inside the extension-collections group as well', async () => {
    ...
    await type(root, GROUP_QUERY);
    const group = root.querySelector('.collection-hidden-group')!;
    expect([...group.querySelectorAll('.collection-item-hit')].map((e: any) => e.textContent))
      .toEqual([GROUP_QUERY]);
  });
  ...
  it('reports the group count as matched-of-total while filtering', async () => {
    ...
    const root = await mount();
    await waitFor(() => root.querySelector('.collection-hidden-group'));
    const groupCount = () => root.querySelector('.collection-hidden-group .sidebar-count')!.textContent;
  ...
  it('does not let the caret write a preference it cannot honour while filtering', async () => {
    ...
    try {
      ...
      expect(setSpy).not.toHaveBeenCalled();
      expect(store.showHiddenCollections.value).toBe(false);
      expect(root.querySelector('.collection-hidden-toggle')!.getAttribute('aria-expanded')).toBe('true');
    } finally {
      (chrome.storage.local as any).set = realSet;
    }
  });
  ...
  it('keeps the caret working once the filter is cleared', async () => {
    ...
    await type(root, GROUP_QUERY);
    await type(root, '');
    root.querySelector('.collection-hidden-toggle')!.dispatchEvent(new MouseEvent('click', { bubbles: true }));
    await tick();
  ...
  it('clears the filter when a collection is created, so the new selection is on screen', async () => {
    ...
    const plus = [...root.querySelectorAll('.icon-btn')]
      .find((b: any) => b.getAttribute('title') === 'New collection')!;
  ...
  it('clears the filter when a collection is renamed, for the same reason', async () => {
    ...
    root.querySelector('.collection-action-menu-btn')!.dispatchEvent(new MouseEvent('click', { bubbles: true }));
    await tick();
