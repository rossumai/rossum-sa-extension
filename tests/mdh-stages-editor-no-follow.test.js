...
const mock = vi.hoisted(() => ({ text: '', onCursorStage: null }));
...
vi.mock('../src/mdh/components/PipelineEditor.jsx', () => ({
  default: ({ editorRef, onCursorStage }) => {
...
describe('hovering a stage in the editor', () => {
  ...
  it('does not scroll the pane even with Auto-scroll on and the section far down', async () => {
    ...
    pane.getBoundingClientRect = () => ({ top: 0, bottom: 300, left: 0, right: 400, width: 400, height: 300 });
    panel.querySelector('[data-entry="5"]').getBoundingClientRect = () => ({ top: 900, bottom: 1100, left: 0, right: 400, width: 400, height: 200 });
    render(h(StageLinkOverlay, { editorRef: { current: api2 }, panelRef: { current: panel } }), mount());
