...
const read = (f: any) => fs.readFileSync(path.join(ROOT, f), 'utf8');
...
function importGraph(files: any) {
  const uses = new Map();
  const add = (target: any, name: any) => {
...
function exportedNames(text: any) {
  const names = new Set();
  let m: any;
...
describe('every export has a consumer', () => {
  it('has no export that nothing imports', () => {
    ...
    for (const f of CODE) {
      ...
      const allowed = (UNCONSUMED_EXPORTS_ALLOWLIST as any)[f] || [];
