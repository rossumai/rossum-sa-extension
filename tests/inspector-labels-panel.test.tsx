...
beforeEach(() => {
  ...
  store.data.value = {
    ...
    resolved: { labelsById: { 3: { id: '3', name: 'Urgent', color: '#f00' } }, labelRules: [] },
  };
  vi.clearAllMocks();
...
describe('LabelsPanel AI attribution', () => {
  it('renders an applied non-rule label attribution seeded by the orchestrator', async () => {
    store.setAttribution('label:3', {
      status: 'done',
      verdict: {
        culprit: { kind: 'hook', id: 8, name: 'Labeler' },
        confidence: 'medium',
        explanation: 'sets Urgent when total>1000',
      },
    });
  ...
  it('does not show AI attribution for a rule-applied label', async () => {
    store.data.value = {
      ...
      resolved: {
        ...
        labelRules: [
          { ruleId: 1, ruleName: 'Flag urgent', trigger: 'total > 1000', labelIds: ['3'] },
        ],
      },
    };
    render(<LabelsPanel />, root);
