...
async function leafPaths() {
  const aggregate = async (_c: any, pipeline: any) => {
    const facet = pipeline[pipeline.length - 1].$facet;
    const out = {};
    for (const [fk, stages] of Object.entries(facet as Record<string, any>)) {
...
const importTail = (docs: any) => dedupeById(restoreDocs(docs, SHAPE).docs).kept;

function expectRoundTrip(restored: any) {
