...
function walk(dir: any, filterExt: any): any {
  const out = [];
...
// Every BARE class="…" literal (single class, no whitespace) in src/**/*.tsx,
...
function topLevelSelectorClasses(css: any) {
  const names = new Set();
...
describe('CSS class collision boundary', () => {
  it('no class used bare in src/**/*.tsx also appears as a top-level selector in dist/console/console.css', () => {
    ...
    const offenders = bareShortClasses()
      .filter((c: any) => topLevel.has(c.name))
      .map((c: any) => `${c.name} @ ${c.file}:${c.line}`);
