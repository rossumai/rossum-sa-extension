...
beforeEach(() => {
  ...
  globalThis.chrome = { runtime: { sendMessage: vi.fn() } } as any;
});
...
describe('dataset-mgmt-suggest banner', () => {
  ...
  it('does nothing on non-MDH pages', () => {
    init({ pathname: '/document/123' } as any);
  ...
  it('the Open button sends the token + origin to the background worker', () => {
    ...
    document.querySelector<HTMLElement>('.rossum-sa-extension-dm-open')!.click();
  ...
  it('has a close button that removes the banner', () => {
    init({ pathname: MDH_PATH } as any);
    const close = document.querySelector<HTMLElement>('.rossum-sa-extension-dm-close');
  ...
  it('stays dismissed for the session — does not re-inject after closing', () => {
    init({ pathname: MDH_PATH } as any);
    document.querySelector<HTMLElement>('.rossum-sa-extension-dm-close')!.click();
