...
import { runAgentQuery, continueAgentQuery, buildGenPrompt, buildFixPrompt, capRows, MAX_CORRECTIONS, MAX_ROWS } from '../src/mdh/agent/agentQuery.js';
...
function makeAgentApi(replies: any) {
  ...
  return {
    ...
    streamMessage: vi.fn(async (_id, content, { onEvent }) => {
      ...
      if (/answersRequest/.test(content)) { // semantic-verify turn → auto-pass
...
function makeApi(results: any) {
  const q = [...results];
  return { aggregate: vi.fn(async () => { const r = q.shift(); if (r instanceof Error) throw r; return r; }) };
}
...
describe('runAgentQuery', () => {
  it('verifies a good pipeline (rows > 0), row-caps it, and primes the cautious persona', async () => {
    ...
    const { pipelineText, note, transcript } = await runAgentQuery({ api, agentApi, request: 'x', collection: 'c', fields: ['a'] });
    expect(JSON.parse(pipelineText!)).toEqual([{ $match: { a: 1 } }, { $limit: MAX_ROWS }]); // capped
    expect(note).toEqual({ kind: 'verified', rowCount: 2 });
    expect(agentApi.streamMessage).toHaveBeenCalledWith('chat_x', '/persona cautious', expect.anything());
    // the executed (verified) pipeline is the capped one
    expect(api.aggregate).toHaveBeenCalledWith('c', [{ $match: { a: 1 } }, { $limit: MAX_ROWS }], expect.anything());
    ...
    expect(transcript.some((t) => t.role === 'system' && /answers the request/.test(t.text))).toBe(true);
  });
    ...
    const agentApi = {
      ...
      streamMessage: vi.fn(async (_id, content, { onEvent }) => {
        ...
        if (/answersRequest/.test(content)) {
          const verdict = verifyCall++ === 0 ? '{"answersRequest":false,"score":10,"issue":"wrong field"}' : '{"answersRequest":true,"score":90}';
          onEvent({ type: 'text-delta', delta: verdict }); onEvent({ type: 'finish' }); return;
        }
        onEvent({ type: 'text-delta', delta: gens.shift() ?? '' }); onEvent({ type: 'finish' });
      }),
    };
    const api = makeApi([{ result: [{ a: 1 }] }, { result: [{ a: 2 }] }]);
    const { pipelineText, note, transcript } = await runAgentQuery({ api, agentApi, request: 'x', collection: 'c' });
    expect(note.kind).toBe('refined');
    expect(JSON.parse(pipelineText!)).toEqual([{ $match: { a: 2 } }, { $limit: MAX_ROWS }]);
    expect(transcript.some((t) => t.role === 'system' && /needs refinement.*wrong field/.test(t.text))).toBe(true);
  });
  ...
  it('refines on an empty result, then verifies', async () => {
    ...
    const { pipelineText, note } = await runAgentQuery({ api, agentApi, request: 'x', collection: 'c' });
  ...
  it('refines on an execution error, then verifies', async () => {
    ...
    expect((await runAgentQuery({ api, agentApi, request: 'x', collection: 'c' })).note.kind).toBe('refined');
  });
  ...
  it('returns declined (never executes) when the agent outputs an empty array', async () => {
    ...
    const api = makeApi([]);
    const { pipelineText, note } = await runAgentQuery({ api, agentApi, request: 'delete everything', collection: 'c' });
    expect(pipelineText).toBeNull();
  ...
  it('BLOCKS a write pipeline ($out/$merge): never executed, never applied', async () => {
    ...
    const api = makeApi([]);
    const { pipelineText, note } = await runAgentQuery({ api, agentApi, request: 'copy the collection', collection: 'c' });
    expect(pipelineText).toBeNull();
  ...
  it('captures the agent reasoning AND full reply in the transcript', async () => {
    ...
    const { transcript } = await runAgentQuery({ api, agentApi, request: 'open invoices', collection: 'c' });
  ...
  it('continueAgentQuery reuses the existing chat (no createChat / no persona prime) and appends the transcript', async () => {
    ...
    const prior = [{ role: 'user', text: 'first request' }, { role: 'assistant', text: '[]' }];
    const { pipelineText, note, transcript, chatId } = await continueAgentQuery({
      api, agentApi, chatId: 'existing_chat', request: 'now only over 10',
      collection: 'c', currentPipeline: '[{"$match":{}}]', // Only role/text are read back in this assertion; the rest of a Turn is irrelevant here.
      transcript: prior as any,
    });
    expect(agentApi.createChat).not.toHaveBeenCalled();
    expect(agentApi.streamMessage).not.toHaveBeenCalledWith('existing_chat', '/persona cautious', expect.anything());
    expect(chatId).toBe('existing_chat');
    expect(note.kind).toBe('verified');
    expect(JSON.parse(pipelineText!)).toEqual([{ $match: { amount: { $gt: 10 } } }, { $limit: MAX_ROWS }]);
  ...
  it('gives up after MAX_CORRECTIONS on a persistently empty result', async () => {
    const agentApi = makeAgentApi(['[{"$match":{"a":1}}]', '[{"$match":{"a":2}}]', '[{"$match":{"a":3}}]']);
...
describe('buildGenPrompt / buildFixPrompt', () => {
  it('gen prompt carries the design rules: JSON-only, read-only, row cap, no-tools, samples, current pipeline', () => {
    const p = buildGenPrompt({ request: 'top vendors', collection: 'v', fields: ['name'], samples: [{ name: 'x' }], currentPipeline: '[{"$match":{"a":1}}]' });
  ...
  it('fix prompt reflects the verdict and re-states the read-only / JSON-only rules', () => {
    expect(buildFixPrompt({ verdict: { kind: 'error', error: 'boom' } })).toMatch(/failed with error: boom/);
    expect(buildFixPrompt({ verdict: { kind: 'empty' } })).toMatch(/0 matching documents/);
    expect(buildFixPrompt({ verdict: { kind: 'mismatch', issue: 'wrong sort' } })).toMatch(/do not correctly answer.*wrong sort/);
    expect(buildFixPrompt({ verdict: { kind: 'empty' } })).toMatch(/READ-ONLY.*ONLY the JSON array/s);
  });
});
