...
describe('full data exploration flow', () => {
  ...
  it('query → sort → filter → paginate → reset', async () => {
    ...
    vi.mocked(api.aggregate).mockResolvedValueOnce({ result: page1 });
    ...
    vi.mocked(api.aggregate).mockResolvedValueOnce({ result: page1Sorted });
    ...
    vi.mocked(api.aggregate).mockResolvedValueOnce({ result: page1Filtered });
    ...
    vi.mocked(api.aggregate).mockResolvedValueOnce({ result: page2 });
  ...
  it('query with placeholders → substitute → execute', async () => {
    ...
    // Now the query runs
    vi.mocked(api.aggregate).mockResolvedValueOnce({ result: [{ name: 'result' }] });
  ...
  it('pagination uses cached total count on second fetch', async () => {
    ...
    vi.mocked(api.aggregate).mockResolvedValueOnce({ result: [{ count: 250 }] });
    ...
    vi.mocked(api.aggregate).mockResolvedValueOnce({ result: [{ count: 251 }] });
    await hooks.pagination.fetchTotalCount('products');
