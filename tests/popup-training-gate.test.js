...
async function mountApp(tab, seed) {
  ...
  render(h(App, { tab }), document.body);
  await waitFor(() => document.body.querySelector('.brand-name'));
}

function clickVersion(times) {
  const el = document.body.querySelector('.version');
  for (let i = 0; i < times; i++) el.click();
}

function clickBrandName(times) {
  const el = document.body.querySelector('.brand-name');
  for (let i = 0; i < times; i++) el.click();
}
