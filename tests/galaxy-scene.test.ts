...
const { captured, Vector2, Vector3, Color, hits } = vi.hoisted(() => {
  class Vector2 {
    ...
    set(x: any, y: any) { this.x = x; this.y = y; return this; }
  }
  class Vector3 {
    declare x: any;
    declare y: any;
  ...
  class Color {
    ...
    lerp(c: any, t: any) { this.r = this.r + (c.r - this.r) * t; this.g = this.g + (c.g - this.g) * t; this.b = this.b + (c.b - this.b) * t; return this; }
  }
  return {
    captured: {
...
vi.mock('three', () => {
  ...
  class Sprite {
    ...
    declare position: any;
    declare scale: any;
    constructor(material: any) { this.material = material; this.scale = new Vector3(); this.position = new Vector3(); }
  }
  class SpriteMaterial {
    declare opacity: any; constructor() { this.opacity = 1; } }
  class CanvasTexture {}
  class Mesh {
    ...
    remove(o: any) { const i = this.children.indexOf(o); if (i >= 0) this.children.splice(i, 1); }
  }
  class SphereGeometry { dispose() {} }
  class MeshBasicMaterial {
    declare color: any;
    constructor(opts: any = {}) { this.color = opts.color || new Color(); }
    dispose() {}
  }
  class MeshStandardMaterial {
    ...
    constructor(opts: any = {}) { this.color = opts.color || new Color(); this.roughness = opts.roughness; this.metalness = opts.metalness; }
    dispose() {}
  }
  class HemisphereLight { constructor() { captured.lightInstances.push(this); } }
  class DirectionalLight {
    declare position: any; constructor() { this.position = new Vector3(); captured.lightInstances.push(this); } }
  class Fog {
    declare color: any;
    declare far: any;
    declare near: any; constructor(color: any, near: any, far: any) { this.color = color; this.near = near; this.far = far; } }
  class BufferAttribute {
    declare array: any;
    declare itemSize: any;
    declare needsUpdate: any;
    constructor(array: any, itemSize: any) { this.array = array; this.itemSize = itemSize; this.needsUpdate = false; }
  }
  class BufferGeometry {
    declare attributes: any;
    constructor() { this.attributes = {}; }
    setAttribute(name: any, attr: any) { this.attributes[name] = attr; }
    dispose() {}
  }
  class LineSegments {
    declare geometry: any;
...
vi.mock('three/addons/controls/OrbitControls.js', () => ({
  OrbitControls: class {
    ...
    update() {}
    dispose() {}
    addEventListener(type: any, fn: any) { this._listeners[type] = fn; }
  },
}));
// --- d3-force-3d mock -----------------------------------------------------
vi.mock('d3-force-3d', () => {
  function makeSim() {
    const sim = {
      force() { return sim; },
      alpha(v: any) { return v === undefined ? 0 : sim; }, // getter (no-arg) returns a settled alpha
...
describe('createScene (three.js + d3-force-3d)', () => {
  ...
  beforeEach(() => {
    ...
    window.matchMedia = ((q) => ({
      ...
      removeEventListener: (_t: any, fn: any) => { const i: any = mqListeners.indexOf(fn); if (i >= 0) mqListeners.splice(i, 1); },
    }) as any);
  ...
  it('setData creates a mesh per node and starts a d3 simulation', () => {
    ...
    expect(meshLikeWithUserData.map((m: any) => m.userData.id).sort()).toEqual(['org:1', 'queue:1']);
  ...
  it('re-tones fog and rebuilds labels when the OS color scheme flips', () => {
    ...
    const linkSeg = group.added.find((o: any) => o && o.geometry && o.geometry.attributes && o.geometry.attributes.color);
  ...
  it('attaches a label to engine nodes (every node type is labeled)', () => {
    ...
    const group = captured.groupInstances[0];
    const engineMesh = group.added.find((o: any) => o && o.userData && o.userData.id === 'engine:7');
  ...
  it('dims non-selected nodes (sphere + label) when a node is selected', () => {
    ...
    const group = captured.groupInstances[0];
    const meshOf = (id: any) => group.added.find((o: any) => o && o.userData && o.userData.id === id);
  ...
  const meshById = (id: any) => captured.groupInstances[0].added.find((o: any) => o && o.userData && o.userData.id === id);
  ...
  it('draws run_after edges with a source->target gradient (reference edges stay solid)', () => {
    ...
    const linkSeg = captured.groupInstances[0].added.find((o: any) => o && o.geometry && o.geometry.attributes && o.geometry.attributes.color);
  ...
  it('drives fog near/far from the live camera distance every frame (stays visible when zoomed out)', () => {
    ...
    let frameCb = null;
    vi.mocked(window.requestAnimationFrame).mockImplementation((cb) => { frameCb = cb; return 1; });
