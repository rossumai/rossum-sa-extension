...
describe('mergeDeliverables', () => {
  const A = deliverable({ id: 'a', order: 1, text: 'new A', editedAt: 200 });
  const Aold = deliverable({ id: 'a', order: 1, text: 'old A', editedAt: 100 });
  const B = deliverable({ id: 'b', order: 2, text: 'legacy only', editedAt: 50 });
  ...
  it('lets a NEWER legacy edit win — an older build wrote after we migrated', () => {
    const newer = deliverable({ id: 'a', order: 1, text: 'legacy is newer', editedAt: 300 });
  ...
  it('falls back to createdAt, and never drops a doc that has neither stamp', () => {
    const noStamp = deliverable({ id: 'c', order: 3 });
    const created = deliverable({ id: 'a', order: 1, text: 'created only', createdAt: 500 });
  ...
  it('tolerates absent lists and unidentified docs', () => {
    ...
    // Deliberately malformed: a doc with no id at all, and one whose id is null. Both are
    // what mergeDeliverables must drop, so the casts are the point of the assertion.
    expect(mergeDeliverables([{ order: 1 } as any], [{ id: null } as any])).toEqual([]);
  });
});
