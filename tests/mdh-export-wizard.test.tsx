...
describe('ExportWizard', () => {
  it('preselects All records when no filter is active, and disables Current filter with the reason', async () => {
    ...
    const all = await waitFor(() =>
      [...root.querySelectorAll('[data-testid="export-scope"] button')].find((b) =>
        b.textContent.trim().startsWith('All records'),
      ),
    );
    expect(all.getAttribute('aria-pressed')).toBe('true');
    const filtered = [...root.querySelectorAll('[data-testid="export-scope"] button')].find((b) =>
      b.textContent.trim().startsWith('Current filter'),
    );
    expect((filtered as HTMLButtonElement).disabled).toBe(true);
  ...
  it('defaults to All records even when a filter is active (Current filter stays enabled, unselected)', async () => {
    ...
    expect((filtered as HTMLButtonElement).disabled).toBe(false);
  });
  ...
  it('shows both scope counts inside the segmented buttons', async () => {
    ...
    const all = await waitFor(() => {
      const b = [...root.querySelectorAll('[data-testid="export-scope"] button')].find((x) =>
        x.textContent.trim().startsWith('All records'),
      );
    ...
    await waitFor(() => {
      const b = [...root.querySelectorAll('[data-testid="export-scope"] button')].find((x) =>
        x.textContent.trim().startsWith('Current filter'),
      );
    ...
    const nf = await waitFor(() =>
      [...noFilter.querySelectorAll('[data-testid="export-scope"] button')].find((x) =>
        x.textContent.trim().startsWith('Current filter'),
      ),
    );
  ...
  it('the Download button restates count and format, per scope', async () => {
    const root = mount({ ...base, filterState: FILTER });
    await waitFor(() =>
      /Download 3 records · JSON/.test(
        root.querySelector('[data-testid="export-download"]')!.textContent,
      ),
    );
    (
      [...root.querySelectorAll('[data-testid="export-scope"] button')].find((x) =>
        x.textContent.trim().startsWith('Current filter'),
      ) as HTMLElement
    ).click();
    await waitFor(() =>
      /Download 7 records · JSON/.test(
        root.querySelector('[data-testid="export-download"]')!.textContent,
      ),
    );
  });
  ...
  it('the what-will-happen list is scope-aware', async () => {
    ...
    await waitFor(() =>
      /Only records matching the current pipeline/.test(
        root.querySelector('[data-testid="export-plan"]')!.textContent,
      ),
    );
  ...
  it('shows the exact count and filename in the count line (all-records uses totalCount)', async () => {
    const root = mount(base);
    await waitFor(() =>
      /Exports 3 records to vendors\.json/.test(
        root.querySelector('[data-testid="export-count"]')!.textContent,
      ),
    );
  });
  ...
  it('warns inline above 10,000 documents — no popup', async () => {
    ...
    expect(root.querySelector<HTMLButtonElement>('[data-testid="export-download"]')!.disabled).toBe(
      false,
    );
  });
  ...
  it('switching format swaps the options strip and preview kind, and re-labels Download', async () => {
    ...
    const csvBtn = [...root.querySelectorAll('[data-testid="export-format"] button')].find(
      (b) => b.textContent.trim() === 'CSV',
    );
    (csvBtn as HTMLElement).click();
    await waitFor(() => root.querySelector('[data-testid="export-csv-bom"]'));
    await waitFor(() =>
      /sku,price/.test(root.querySelector('[data-testid="export-preview"]')!.textContent),
    );
    expect(root.querySelector('[data-testid="export-download"]')!.textContent).toMatch(
      /Download 3 records · CSV/,
    );
    const xlsxBtn = [...root.querySelectorAll('[data-testid="export-format"] button')].find(
      (b) => b.textContent.trim() === 'Excel',
    );
    (xlsxBtn as HTMLElement).click();
  ...
  it('what-will-happen shows the columns line only for column formats', async () => {
    ...
    (
      [...root.querySelectorAll('[data-testid="export-format"] button')].find(
        (b) => b.textContent.trim() === 'CSV',
      ) as HTMLElement
    ).click();
    await waitFor(() =>
      /union of fields/.test(root.querySelector('[data-testid="export-plan"]')!.textContent),
    );
  ...
  it('Download hands the full config to onExport and closes the modal', async () => {
    ...
    await waitFor(() =>
      /Exports 7 records/.test(root.querySelector('[data-testid="export-count"]')!.textContent),
    );
    root.querySelector<HTMLElement>('[data-testid="export-download"]')!.click();
  ...
  it('a failed count never blocks Download (config carries count: null)', async () => {
    ...
    const btn = await waitFor(() => {
      const b = root.querySelector<HTMLButtonElement>('[data-testid="export-download"]');
  ...
  it('column discovery only runs for column formats, lazily, and is cached across a csv<->xlsx switch', async () => {
    ...
    const groupCalls = () =>
      vi
        ...
        .mock.calls.filter(([, pipeline]) => JSON.stringify(pipeline).includes('$group')).length;
  ...
  it('shows "Building preview…" (not "Preview unavailable") while CSV columns are still loading', async () => {
    ...
    await waitFor(() =>
      /"sku"/.test(root.querySelector('[data-testid="export-preview"]')!.textContent),
    );
    (
      [...root.querySelectorAll('[data-testid="export-format"] button')].find(
        (b) => b.textContent.trim() === 'CSV',
    ...
    await waitFor(() =>
      /Building preview/.test(root.querySelector('[data-testid="export-preview"]')!.textContent),
    );
    expect(root.querySelector('[data-testid="export-preview"]')!.textContent).not.toMatch(
      /Preview unavailable/,
    );
    resolveDiscovery({
      result: [
  ...
  it('aborts in-flight count/sample/discovery aggregations when the wizard unmounts (e.g. modal closed)', async () => {
    const signals: any = [];
    vi.mocked(api.aggregate).mockImplementation((_c, _p, opts) => {
