...
describe('DocView with many sections', () => {
  it('renders one section per deliverable, tagged with id and slug', async () => {
    ...
    expect([...root.querySelectorAll('section[data-deliverable]')].map((s) => s.dataset.slug))
  ...
  it('renders a host-supplied header inside each section', async () => {
    const root = mount({ headerFor: (s) => h('div', { class: 'mk-hd' }, s.slug) });
  ...
  it('exposes page-wide geometry for the scroll spy', async () => {
    ...
    expect(docRef.current.sectionTops().map((t) => t.id)).toEqual(['d1', 'd2']);
    expect(docRef.current.headingTops().map((h2) => h2.docId)).toContain('d2');
  });
