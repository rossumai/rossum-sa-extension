...
function mountDoc(doc, nameFor, ensure = () => {}) {
  return new EditorView({
    state: EditorState.create({ doc, extensions: [rossumNames(nameFor, ensure)] }),
    parent: document.body,
  });
}
async function waitFor(fn, tries = 200) { for (let i = 0; i < tries; i++) { if (fn()) return; await new Promise((r) => setTimeout(r, 0)); } throw new Error('waitFor timed out'); }

const line = (url) => `  "queue": "${url}",\n`;
...
describe('cmNames', () => {
  ...
  it('shows the name after async resolution (ensure → refresh effect → rebuild)', async () => {
    ...
    const ensure = (u, cb) => { resolved = true; setTimeout(cb, 0); }; // resolve, then notify
  ...
  it('joins multiple names on one line with a middot', () => {
    ...
    const doc = `  "hooks": ["${a}", "${b}"],\n`;
    const view = mountDoc(doc, (u) => ({ status: 'done', name: u.endsWith('5') ? 'Alpha' : 'Beta' }));
