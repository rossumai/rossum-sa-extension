...
import {
  schemaIdForDatapoint,
  fieldThresholds,
  computeVerdict,
  buildEvidence,
} from '../src/inspector/evidence.js';
    ...
    children: [
      {
        ...
        id: 101,
        schema_id: 'po_number',
        content: { value: '', rir_confidence: 0.31 },
      },
      {
        category: 'multivalue',
        children: [
          { category: 'datapoint', id: 102, schema_id: 'item_total', content: { value: '5' } },
        ],
      },
    ],
  },
...
describe('computeVerdict', () => {
  ...
  it('automated annotation → success', () => {
    const v = computeVerdict({
      annotation: { status: 'exported', automated: true },
      blocker: null,
      content: null,
      queue,
      schema: null,
  ...
  it('automation off → automation-off with queue fact', () => {
    const v = computeVerdict({
      annotation: { status: 'to_review', automated: false },
      blocker: null,
      content: null,
    ...
    const blocker = {
      content: [
        { type: 'error_message', level: 'annotation' },
        {
          type: 'low_score',
          schema_id: 'po_number',
          samples: [{ datapoint_id: 101, details: { score: 0.31, threshold: 0.8 } }],
        },
      ],
    };
    const v = computeVerdict({
  ...
  it('rejected / failed_export outrank in-review', () => {
    ...
    expect(
      computeVerdict({
        annotation: { status: 'failed_export' },
        blocker: null,
        content: null,
        queue,
        schema: null,
  ...
  it('rejected status takes precedence over blockers', () => {
    ...
    const v = computeVerdict({
      annotation: { status: 'rejected', automated: true },
      blocker,
      content: null,
      queue,
      schema: null,
  ...
  it('failed_export status takes precedence over blockers', () => {
    const blocker = { content: [{ type: 'error_message', level: 'annotation' }] };
    const v = computeVerdict({
      annotation: { status: 'failed_export', automated: true },
      blocker,
      content: null,
      queue,
      schema: null,
  ...
  it('missing queue/schema robustness with low_score blocker', () => {
    const blocker = {
      content: [{ type: 'low_score', schema_id: 'total', samples: [{ details: { score: 0.5 } }] }],
    };
    const v = computeVerdict({
      annotation: { status: 'to_review', automated: false },
      blocker,
...
describe('buildEvidence — core items', () => {
  function baseInput(over = {}) {
    return {
      ...
      blocker: { content: [] },
      content: { content: [] },
      queue: { default_score_threshold: 0.8 },
      schema: null,
      document: null,
      parentDocument: null,
      relations: [],
        ...
        messages: [
          {
            ...
            content: 'Bad value',
            id: 101,
            detail: { rule_id: 7, rule_name: 'PO required' },
          },
        ],
        labels: [],
      },
        ...
        content: [
          {
            category: 'section',
            children: [
              { category: 'datapoint', id: 101, schema_id: 'po_number', content: { value: '' } },
            ],
          },
        ],
      },
  ...
  it('merges a residual AI attribution into the matching item', () => {
    const input = baseInput({
      annotation: {
        id: 1,
        status: 'to_review',
        messages: [{ type: 'error', content: 'X', detail: {} }],
        labels: [],
      },
      attributions: {
        'message:0': {
          ...
          source: 'ai',
          verdict: {
            culprit: { kind: 'hook', id: 9, name: 'Exporter' },
            confidence: 'medium',
            explanation: 'e',
          },
        },
            ...
            children: [
              {
                ...
                id: 1,
                schema_id: 'total',
                content: { value: '10', rir_confidence: 0.97 },
            ...
            children: [
              {
                ...
                id: 1,
                schema_id: 'total',
                content: { value: '99', rir_confidence: 0.97 },
                validation_sources: ['score'],
              },
              {
                category: 'multivalue',
                    ...
                    children: [
                      {
                        ...
                        content: { value: '5', rir_confidence: 0.6 },
                        validation_sources: ['score'],
                      },
                    ],
                  },
                    ...
                    children: [
                      {
                        ...
                        id: 11,
                        schema_id: 'item_amount',
                        content: { value: '7', rir_confidence: 0.95 },
                        validation_sources: ['score'],
                      },
                    ],
                  },
                    ...
                    children: [
                      {
                        ...
                        id: 12,
                        schema_id: 'item_amount',
                        content: { value: '9', rir_confidence: 0.5 },
                        validation_sources: ['score'],
                      },
                    ],
                  },
        ...
        content: [
          {
            ...
            children: [
              { category: 'datapoint', id: 'total' },
              {
                ...
                children: {
                  category: 'tuple',
                  children: [{ category: 'datapoint', id: 'item_amount', score_threshold: 0.9 }],
                },
              },
            ],
          },
        ...
        content: [
          {
            type: 'low_score',
            schema_id: 'recipient',
            samples: [{ datapoint_id: 5, details: { score: 0.5 } }],
          },
        ],
      },
      schema: {
        content: [
          {
            category: 'section',
            children: [{ category: 'datapoint', id: 'recipient', score_threshold: 0.9 }],
          },
        ],
      },
    });
  ...
  it('unavailable enrichment produces an explicit gap item', () => {
    const input = baseInput({
      enrichment: { workflow: [], notes: [], hookLogs: 'unavailable', ruleLogs: [] },
    });
  ...
  it('live present → per-row drift items plus a summary item, all verified', () => {
    const input = baseInput({
      annotation: {
        id: 1,
        status: 'to_review',
        messages: [{ type: 'error', content: 'Removed one' }],
        labels: [],
      },
      live: {
        messages: [{ type: 'warning', content: 'Added one' }],
        matchedTriggerRules: [{ id: 5, name: 'R' }],
      },
    });
    const { items } = buildEvidence(input);
