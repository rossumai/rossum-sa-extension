...
describe('BlockedPanel message attribution', () => {
  it('renders a self-attributed error message culprit (verified, no agent)', () => {
    store.data.value = {
      annotation: {
        id: 1,
        messages: [{ type: 'error', content: 'Bad', detail: { hook_id: 5, hook_name: 'H' } }],
      },
      blocker: { content: [] },
      resolved: { queue: null },
    };
  ...
  it('renders an orchestrator-fed AI culprit + explanation for an unattributed message', () => {
    store.data.value = {
      annotation: {
        id: 1,
        messages: [{ type: 'error', content: 'Mystery', detail: { request_id: 'r1' } }],
      },
      blocker: { content: [] },
      resolved: { queue: null },
    };
    store.setAttribution(messageKey(0), {
      status: 'done',
      verdict: {
        culprit: { kind: 'hook', id: 9, name: 'Guesser' },
        confidence: 'medium',
        explanation: 'emits this on total mismatch',
      },
      source: 'ai',
    });
    render(<BlockedPanel />, root);
  ...
  it('shows the live phase while an unattributed message is being reasoned', () => {
    store.data.value = {
      annotation: { id: 1, messages: [{ type: 'error', content: 'M', detail: {} }] },
      blocker: { content: [] },
      resolved: { queue: null },
    };
    store.setAttribution(messageKey(0), {
      status: 'loading',
      phase: 'reading extension logs',
      source: 'ai',
    });
    render(<BlockedPanel />, root);
  ...
  it('renders a programmatic (verified) message culprit from its request_id correlation', () => {
    store.data.value = {
      annotation: {
        id: 1,
        messages: [{ type: 'error', content: 'M', detail: { request_id: 'r1' } }],
      },
      blocker: { content: [] },
      resolved: { queue: null },
    };
    store.setAttribution(messageKey(0), {
      status: 'done',
      verdict: {
        culprit: { kind: 'hook', id: 5, name: 'Correlated' },
        confidence: null,
        explanation: '',
      },
  ...
  it('renders the AI explanation for a non-standard blocker', () => {
    store.data.value = {
      ...
      blocker: { content: [{ type: 'custom_weird_blocker' }] },
      resolved: { queue: null },
    };
    store.setAttribution(blockerKey(0), {
      status: 'done',
      verdict: {
        culprit: null,
        confidence: 'low',
        explanation: 'means the ERP link is misconfigured',
      },
      source: 'ai',
    });
    render(<BlockedPanel />, root);
