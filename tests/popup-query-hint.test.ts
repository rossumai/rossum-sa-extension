...
describe('placeHint', () => {
  ...
  it('pulls a wide tip left so it never runs off the right edge', () => {
    const p = placeHint({ left: 300, right: 312, top: 100, bottom: 112 } as any, { width: 300, height: 40 }, VIEW);
  ...
  it('never returns a negative coordinate', () => {
    const p = placeHint({ left: -50, right: -38, top: -20, bottom: -8 } as any, { width: 200, height: 40 }, VIEW);
...
const opened = (r: any) => seen(() => expect(r.querySelector('.mdh-q-hint')?.style.visibility).toBe('visible'));
