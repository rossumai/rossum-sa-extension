...
const D = { id: 'd1', text: '# One', order: 1, title: 'One', titleSource: 'manual' };
function mount() {
...
const implTab = (root) => [...root.querySelectorAll('.fabry-rail-tab')].find((t) => /Implement/.test(t.textContent));
function openImplement(root) { act(() => { implTab(root).click(); }); return root; }
...
describe('Implement panel — now in the inspector rail', () => {
  ...
  it('the Implement button opens the Arm dialog (count=1), and its onConfirm runs the loop', () => {
    ...
    expect(openArmDialog.mock.calls[0][0]).toBe(1);
    act(() => { openArmDialog.mock.calls[0][1](); });
