// The mechanical 1:1 proof for the localpages port (spec 2026-08-17, §8).
//
// tests/fixtures/localpages/ holds upstream's own examples/basic/*.md AND the HTML
// that upstream localpages@4d43f26 produces for them (expected/*.{live,export}.html,
// generated by running its src/render.mjs directly). This test requires the ported
// renderer to reproduce that byte-for-byte.
//
// It is what makes "ported verbatim" a checkable claim rather than a promise: any
// drift in markdown-it, the plugins, the slugify, the alert markup, the figure
// wrapping or the .md→.html rewrite fails here.
// Regenerating the fixtures against a newer upstream is how a future localpages
...
const expected = (name, mode) => fs.readFileSync(path.join(FIX, 'expected', `${name}.${mode}.html`), 'utf8');
...
describe('the one deliberate divergence: <state-label> is no longer document markup', () => {
  ...
  test('upstream\'s states.md now differs from upstream only in that respect', () => {
    ...
    const strip = (h) => h.replace(/<span class="state-label[\s\S]*?<\/span><\/span>/g, '')
