...
function scriptReplies(map: any) {
  ...
  vi.mocked(agentApi.streamMessage).mockImplementation(
    async (chatId, content, { onEvent }: any) => {
...
describe('loadArchitect', () => {
  it('loads deliverables + persisted (stale) results', async () => {
    vi.mocked(api.loadDeliverables).mockResolvedValueOnce({
      deliverables: [deliverable({ id: 'a', text: '# A', order: 1 })],
      results: { a: { verdict: 'pass', evidence: 'ok', chatId: 'c', ranAt: 5, stale: true } },
      implement: {},
      legacyCount: 0,
    });
    await loadArchitect();
    ...
    vi.mocked(api.loadDeliverables).mockResolvedValueOnce({
      ...
      implement: {
        a: {
          ...
          ranAt: null,
          status: 'passing',
          tasks: [{ id: 'k1', text: 't1', status: 'done' }],
          writes: [{ tool: 'create_rule', ok: true }],
          stale: true,
        },
      },
    });
  ...
  it('guards against a concurrent double load', async () => {
    vi.mocked(api.loadDeliverables).mockResolvedValue({
      deliverables: [],
      results: {},
      implement: {},
      legacyCount: 0,
...
describe('add/open/update/delete', () => {
  it('addDeliverable seeds the FIRST deliverable with the demo example and opens it', async () => {
    ...
    expect(api.addDeliverable).toHaveBeenCalledWith(
      expect.objectContaining({ id: d.id, text: d.text, order: 1 }),
    );
  });
  ...
  it('refineTurn: first turn opens a cautious chat and applies the first instruction', async () => {
    scriptReplies({
      INSTRUCTION:
        '# Invoices queue is automated\n\nThe Invoices queue processes documents automatically.',
    });
    const res = (await refineTurn({
      chatId: null,
      deliverableText: 'the invoices Q should be automatic',
  ...
  it('refineTurn: a follow-up reuses the chat and sends just the instruction (no re-setup)', async () => {
    ...
    const res = await refineTurn({
      ...
      deliverableText: 'base',
      instruction: 'also name the field',
    });
    expect(res!.chatId).toBe('chat_existing');
    expect(agentApi.createChat).not.toHaveBeenCalled();
    const contents = vi.mocked(agentApi.streamMessage).mock.calls.map((c) => c[1]);
    expect(contents.some((c) => /also name the field/.test(c) && !/REQUIREMENT:/.test(c))).toBe(
      true,
    );
  });
  ...
  it('openDeliverable sets activeId', () => {
    openDeliverable('z');
    expect(store.activeId.value).toBe('z');
  });
...
describe('runAll', () => {
  it('checks each deliverable in its own chat, records verdicts, and persists results (stale cleared)', async () => {
    store.deliverables.value = [
      deliverable({ id: 'a', text: 'ALPHA', order: 1 }),
      deliverable({ id: 'b', text: 'BETA', order: 2 }),
    ];
    ...
    expect(api.saveResult).toHaveBeenCalledWith(
      'a',
      expect.objectContaining({ verdict: 'pass', chatId: 'chat_0' }),
    );
...
describe('renameDeliverable / generateTitle / backfillTitles', () => {
  it('renameDeliverable sets the store title (trimmed) and persists it as a MANUAL rename', async () => {
    ...
    expect(store.deliverables.value[0]).toMatchObject({
      title: 'Add VAT Rule',
      titleSource: 'manual',
    });
  ...
  it('generateTitle is a no-op when the deliverable already has a title', async () => {
    store.deliverables.value = [
      deliverable({
        id: 'a',
        text: 'Add a VAT rule to the Invoices queue',
        order: 1,
        title: 'Existing',
      }),
    ];
  ...
  it('generateTitle is a no-op when the text declares its own heading', async () => {
    ...
    store.deliverables.value = [
      deliverable({ id: 'a', text: '# Invoices Queue Automation\nmust be touchless', order: 1 }),
    ];
    await generateTitle('a');
    expect(agentApi.createChat).not.toHaveBeenCalled();
  ...
  it('generateTitle still runs when the heading is not on the first non-empty line', async () => {
    store.deliverables.value = [
      deliverable({ id: 'a', text: '> banner line\n\n# Not the first line', order: 1 }),
    ];
  ...
  it('backfillTitles generates titles only for untitled, headingless deliverables with enough text', async () => {
    store.deliverables.value = [
      ...
      deliverable({
        id: 'b',
        text: 'Add a currency rule to the Invoices queue',
        order: 2,
      ...
      deliverable({
        id: 'd',
        text: '# Vendor Matching\nmatch every vendor against the master data',
        order: 4,
      }),
    ];
    vi.mocked(agentApi.createChat).mockResolvedValue('chat_x');
