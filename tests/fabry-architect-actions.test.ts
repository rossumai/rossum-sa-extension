...
function scriptReplies(map: any) {
  let n = 0;
  vi.mocked(agentApi.createChat).mockImplementation(async () => 'chat_' + (n++));
  vi.mocked(agentApi.streamMessage).mockImplementation(async (chatId, content, { onEvent }: any) => {
...
describe('loadArchitect', () => {
  it('loads deliverables + persisted (stale) results', async () => {
    vi.mocked(api.loadDeliverables).mockResolvedValueOnce({ deliverables: [deliverable({ id: 'a', text: '# A', order: 1 })], results: { a: { verdict: 'pass', evidence: 'ok', chatId: 'c', ranAt: 5, stale: true } }, implement: {}, legacyCount: 0 });
    await loadArchitect();
  ...
  it('rehydrates persisted implement-loop state (status / tasks / write audit)', async () => {
    vi.mocked(api.loadDeliverables).mockResolvedValueOnce({
      deliverables: [deliverable({ id: 'a', text: '# A', order: 1 })], legacyCount: 0,
      results: {},
      implement: { a: { attempt: 0, summary: '', chatId: null, journal: [], ranAt: null, status: 'passing', tasks: [{ id: 'k1', text: 't1', status: 'done' }], writes: [{ tool: 'create_rule', ok: true }], stale: true } },
    });
    await loadArchitect();
  ...
  it('tolerates a loadDeliverables result with no implement map (older shape)', async () => {
    vi.mocked(api.loadDeliverables).mockResolvedValueOnce({ deliverables: [deliverable({ id: 'a', text: 'A', order: 1 })], results: {}, implement: {}, legacyCount: 0 });
    await loadArchitect();
    expect(store.implement.value).toEqual({});
  });
  it('records loadError without throwing', async () => {
    vi.mocked(api.resolveCollection).mockRejectedValueOnce(new Error('nope'));
    await loadArchitect();
  ...
  it('selects the first deliverable on first open (no active selection)', async () => {
    vi.mocked(api.loadDeliverables).mockResolvedValueOnce({ deliverables: [deliverable({ id: 'a', text: 'A', order: 1 }), deliverable({ id: 'b', text: 'B', order: 2 })], results: {}, implement: {}, legacyCount: 0 });
  ...
  it('guards against a concurrent double load', async () => {
    vi.mocked(api.loadDeliverables).mockResolvedValue({ deliverables: [], results: {}, implement: {}, legacyCount: 0 });
...
describe('add/open/update/delete', () => {
  ...
  it('addDeliverable creates a BLANK deliverable once one already exists', async () => {
    store.deliverables.value = [deliverable({ id: 'x', text: '# existing', order: 1 })];
  ...
  it('refineTurn: first turn opens a cautious chat and applies the first instruction', async () => {
    ...
    const contents = vi.mocked(agentApi.streamMessage).mock.calls.map((c) => c[1]);
  ...
  it('answerRefine: sends the formatted answers to the SAME chat and returns the revised proposal', async () => {
    ...
    expect(res.chatId).toBe('chat_existing');
    expect(res.proposal).toMatch(/Invoices queue/);
    expect(vi.mocked(agentApi.streamMessage).mock.calls[0][1]).toBe('Invoices'); // single answer → bare answer (formatAnswers)
  });
  ...
  it('updateDeliverable updates store text live, marks its result stale, and persists editedAt', async () => {
    store.deliverables.value = [deliverable({ id: 'a', text: '# old', order: 1 })];
  ...
  it('updateDeliverable no-ops when unchanged', async () => {
    store.deliverables.value = [deliverable({ id: 'a', text: 'same', order: 1 })];
  ...
  it('deleteDeliverable removes it, its result, and clears activeId if open', async () => {
    store.deliverables.value = [deliverable({ id: 'a', text: 'x', order: 1 })];
...
describe('reorder / moveDeliverable', () => {
  ...
  it('moveDeliverable no-ops for an unknown id or the same position', async () => {
    store.deliverables.value = [deliverable({ id: 'a', text: 'A', order: 0 }), deliverable({ id: 'b', text: 'B', order: 1 })];
...
describe('runAll', () => {
  it('checks each deliverable in its own chat, records verdicts, and persists results (stale cleared)', async () => {
    store.deliverables.value = [deliverable({ id: 'a', text: 'ALPHA', order: 1 }), deliverable({ id: 'b', text: 'BETA', order: 2 })];
  ...
  it('never sends a write-enabling flag (read-only contract)', async () => {
    store.deliverables.value = [deliverable({ id: 'a', text: 'ALPHA', order: 1 })];
    scriptReplies({ ALPHA: 'VERDICT: PASS' });
    await runAll();
    for (const call of vi.mocked(agentApi.createChat).mock.calls) expect(call.length).toBe(0);
    for (const call of vi.mocked(agentApi.streamMessage).mock.calls) {
      const opts = call[2] || {};
...
describe('renameDeliverable / generateTitle / backfillTitles', () => {
  it('renameDeliverable sets the store title (trimmed) and persists it as a MANUAL rename', async () => {
    store.deliverables.value = [deliverable({ id: 'a', text: 'Add a VAT rule', order: 1 })];
  ...
  it('generateTitle opens a READ-ONLY chat and sets the parsed title on an untitled deliverable', async () => {
    store.deliverables.value = [deliverable({ id: 'a', text: 'Add a VAT rule to the Invoices queue', order: 1 })];
    let n = 0;
    vi.mocked(agentApi.createChat).mockImplementation(async () => 'chat_' + (n++));
    vi.mocked(agentApi.streamMessage).mockImplementation(async (chatId, content, { onEvent }: any) => {
    ...
    const opts = vi.mocked(agentApi.streamMessage).mock.calls[0][2]!;
    expect(opts.mcpMode).toBeUndefined();
    expect('mcp_mode' in opts).toBe(false);
  });

  it('generateTitle is a no-op when the deliverable already has a title', async () => {
    store.deliverables.value = [deliverable({ id: 'a', text: 'Add a VAT rule to the Invoices queue', order: 1, title: 'Existing' })];
    await generateTitle('a');
    expect(agentApi.createChat).not.toHaveBeenCalled();
  });

  it('generateTitle is a no-op when the deliverable text is too short', async () => {
    store.deliverables.value = [deliverable({ id: 'a', text: 'short', order: 1 })];
    await generateTitle('a');
    expect(agentApi.createChat).not.toHaveBeenCalled();
  });
  ...
  it('generateTitle is a no-op when the text declares its own heading', async () => {
    ...
    store.deliverables.value = [deliverable({ id: 'a', text: '# Invoices Queue Automation\nmust be touchless', order: 1 })];
    await generateTitle('a');
    expect(agentApi.createChat).not.toHaveBeenCalled();
  ...
  it('generateTitle still runs when the heading is not on the first non-empty line', async () => {
    store.deliverables.value = [deliverable({ id: 'a', text: '> banner line\n\n# Not the first line', order: 1 })];
    vi.mocked(agentApi.createChat).mockResolvedValue('chat_x');
    vi.mocked(agentApi.streamMessage).mockImplementation(async (chatId, content, { onEvent }: any) => {
  ...
  it('backfillTitles generates titles only for untitled, headingless deliverables with enough text', async () => {
    store.deliverables.value = [
      deliverable({ id: 'a', text: 'Add a VAT rule to the Invoices queue', order: 1 }),
      deliverable({ id: 'b', text: 'Add a currency rule to the Invoices queue', order: 2, title: 'Already Titled' }),
      deliverable({ id: 'c', text: 'short', order: 3 }),
      deliverable({ id: 'd', text: '# Vendor Matching\nmatch every vendor against the master data', order: 4 }),
    ];
    vi.mocked(agentApi.createChat).mockResolvedValue('chat_x');
    vi.mocked(agentApi.streamMessage).mockImplementation(async (chatId, content, { onEvent }: any) => {
      onEvent({ type: 'text-delta', delta: 'Generated Title' });
    ...
    expect(store.deliverables.value.find((d) => d.id === 'c')!.title).toBe(''); // too short, untouched (still the loader's empty default)
    expect(store.deliverables.value.find((d) => d.id === 'd')!.title).toBe(''); // declares its own heading, untouched (still the loader's empty default)
