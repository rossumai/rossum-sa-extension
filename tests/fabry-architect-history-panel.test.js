...
const D = { id: 'd1', text: 'current text here', editedAt: Date.now() - 60_000 };
...
const rows = (root) => [...root.querySelectorAll('.fabry-arch-hist-btn')];
...
describe('HistoryPanel', () => {
  ...
  it('restores the selected version, and cannot restore before its text is loaded', () => {
    ...
    const root = mount();
    act(() => { store.selectedRevision.value = 'r1'; });
    expect(root.querySelector('.fabry-arch-hist-restore').disabled).toBe(true);

    act(() => { store.revisionTexts.value = { r1: 'old' }; });
    const btn = root.querySelector('.fabry-arch-hist-restore');
