...
describe('ImportConfirm', () => {
  it('insert step list explains verified insert behavior and enables Go', async () => {
    ...
    expect(root.querySelector<HTMLButtonElement>('[data-testid="import-go"]')!.disabled).toBe(false);
    await openPlan(root);
  ...
  it('update requires match keys (Go disabled until a key is chosen)', async () => {
    ...
    expect(noKeys.querySelector<HTMLButtonElement>('[data-testid="import-go"]')!.disabled).toBe(true);
    const withKeys = mount(<ImportConfirm {...base} mode="update" keys={['sku']} />);
    expect(withKeys.querySelector<HTMLButtonElement>('[data-testid="import-go"]')!.disabled).toBe(false);
  ...
  it('multi-key update copy states AND semantics; single-key copy does not', async () => {
    const multi = mount(<ImportConfirm {...base} docs={[{ sku: 'A', region: 'EU' }]} mode="update" keys={['sku', 'region']} />);
    ...
    const single = mount(<ImportConfirm {...base} docs={[{ sku: 'A' }]} mode="update" keys={['sku']} />);
  ...
  it('summary sentence: multi-key update appends (all must match)', () => {
    const root = mount(<ImportConfirm {...base} docs={[{ sku: 'A', region: 'EU' }]} mode="update" keys={['sku', 'region']} />);
  ...
  it('shows a muted one-line pass state with the sample size', () => {
    ...
    const root = mount(<ImportConfirm {...base} shape={shape} shapeCount={137} docs={[{ sku: 'B2', price: 20 }]} />);
  ...
  it('says "all N existing records" when the sample covered the whole collection', () => {
    const shape = deriveShape([{ sku: 'A1', price: 10 }]);
    const root = mount(<ImportConfirm {...base} shape={shape} shapeCount={150} shapeCoversAll docs={[{ sku: 'B2', price: 20 }]} />);
  ...
  it('disables Go while the shape sample is still loading, for every mode', () => {
    ...
    expect(insert.querySelector<HTMLButtonElement>('[data-testid="import-go"]')!.disabled).toBe(true);

    const update = mount(<ImportConfirm {...base} mode="update" keys={['sku']} shapeLoading />);
    expect(update.querySelector<HTMLButtonElement>('[data-testid="import-go"]')!.disabled).toBe(true);

    const replace = mount(<ImportConfirm {...base} mode="replace" shapeLoading />);
    expect(replace.querySelector<HTMLButtonElement>('[data-testid="import-go"]')!.disabled).toBe(true);
  });

  it('shows the sample note inside the red panel on mismatch', () => {
    const shape = deriveShape([{ sku: 'A1', price: 10, region: 'EU' }]);
    const bad = mount(<ImportConfirm {...base} shape={shape} shapeCount={42} docs={[{ sku: 'B2', price: 20 }]} />);
  ...
  it('reports a whitespace-only column difference explicitly and visibly', () => {
    const shape = deriveShape([{ sku: 'A1', price: 10 }]);
    const root = mount(<ImportConfirm {...base} shape={shape} shapeCount={1} docs={[{ 'sku ': 'B2', price: 20 }]} />);
    const err = root.querySelector('[data-testid="import-shape-error"]')!;
  ...
  it('renders Missing/Unexpected names through the whitespace-revealing renderer', () => {
    ...
    const root = mount(<ImportConfirm {...base} shape={shape} shapeCount={1} docs={[{ zone: 'EU' }]} />);
    const err = root.querySelector('[data-testid="import-shape-error"]');
  ...
  it('the acknowledgement checkbox overrides the mismatch and keeps the error visible', () => {
    const shape = deriveShape([{ sku: 'A1', price: 10, region: 'EU' }]);
    let override = false;
    const setShapeOverride = (v: any) => { override = v; };
    const root = mount(<ImportConfirm {...base} mode="insert" shape={shape} setShapeOverride={setShapeOverride} />);
    ...
    expect(err).toBeTruthy();
    expect(err!.textContent).toMatch(/blocked/i);
    expect(over.querySelector<HTMLInputElement>('[data-testid="shape-override"]')!.checked).toBe(true);
    expect(over.querySelector<HTMLButtonElement>('[data-testid="import-go"]')!.disabled).toBe(false);
  ...
  it('states which side each error came from — positionally, via the ledger columns', () => {
    ...
    const root = mount(<ImportConfirm {...base} shape={shape} shapeCount={1} docs={[{ sku: 'B2', at: 'text', extra: 1 }]} />);
    const err = root.querySelector('[data-testid="import-shape-error"]')!;
  ...
  it('names the missing side explicitly too — the collection column carries its type, the file column reads "(absent)"', () => {
    ...
    const root = mount(<ImportConfirm {...base} shape={shape} shapeCount={1} docs={[{ sku: 'B2' }]} />);
    const err = root.querySelector('[data-testid="import-shape-error"]')!;
  ...
  it('renders the ledger table: headers, root grouping (only when >1 row shares a root), row order, and the flat-cause summary', () => {
    ...
    const root = mount(<ImportConfirm
      {...base}
      shape={shape}
      shapeCount={500}
  ...
  it('says "1 field arrived flat" (singular) for exactly one flattened cause', () => {
    ...
    const root = mount(<ImportConfirm {...base} shape={shape} shapeCount={1} docs={[{ key: 'flat', sku: 'X' }]} />);
  ...
  it('tags a whitespace row as "spelling" so it is not misread as a type, and renders an absent side as "(absent)", muted and italic', () => {
    ...
    const root2 = mount(<ImportConfirm {...base} shape={shape2} shapeCount={1} docs={[{ sku: 'B2' }]} />);
  ...
  it('renders the restore summary when one is given, and nothing when it is null', () => {
    const withIt = mount(<ImportConfirm {...base} restoreSummary="Restored 9 nested columns to match the collection." />);
