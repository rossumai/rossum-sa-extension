...
describe('FlashOnChange', () => {
  it('renders the value without a flash class on first mount', async () => {
    ...
    render(<FlashOnChange value="42" />, root);
    await flush();
  ...
  it('applies the flash-value class when the value changes', async () => {
    ...
    await flush();
    await flush();
    render(<FlashOnChange value="99" />, root);
    await flush();
    await flush();
  ...
  it('keeps flashing on every subsequent change', async () => {
    const root = document.createElement('div');
    document.body.appendChild(root);
    render(<FlashOnChange value="a" />, root);
    await flush();
    await flush();
    render(<FlashOnChange value="b" />, root);
    await flush();
    await flush();
    render(<FlashOnChange value="c" />, root);
    await flush();
    await flush();
    expect(root.querySelector('.flash-value')).not.toBeNull();
