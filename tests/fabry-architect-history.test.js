...
beforeEach(() => {
  ...
  resetSession();
  api.listRevisions.mockResolvedValue({ result: [] });
  api.getRevision.mockResolvedValue({ result: [] });
...
describe('version capture on save', () => {
  it('stores the PRE-EDIT text once per editing session, not once per autosave', async () => {
    ...
    const doc = api.addRevision.mock.calls[0][0];
  ...
  it('opens a new version when the source changes — an accepted Refine is its own act', async () => {
    ...
    expect(api.addRevision.mock.calls[1][0]).toMatchObject({ text: 'typed', source: 'refine' });
  });
  ...
  it('saves the text even when the history write fails', async () => {
    seed('before');
    api.addRevision.mockRejectedValueOnce(new Error('history down'));
  ...
  it('prunes past the cap, keeping the earliest', async () => {
    ...
    api.listRevisions.mockResolvedValue({ result: many });
    await updateDeliverable('d1', 'after');
    await flush();
    const [, dropped] = api.deleteRevisions.mock.calls[0];
...
describe('restore', () => {
  it('snapshots the current text first, then writes the old text back', async () => {
    seed('current text');
    api.getRevision.mockResolvedValueOnce({ result: [{ _id: 'r1', text: 'old text' }] });
  ...
  it('reports a version that is no longer stored instead of writing an empty document', async () => {
    seed('current text');
    api.getRevision.mockResolvedValueOnce({ result: [] });
...
describe('loadRevisions', () => {
  it('maps and sorts newest-first, and does not refetch what it already has', async () => {
    ...
    expect(store.revisions.value.d1.items.map((r) => r.id)).toEqual(['b', 'a']);
    await loadRevisions('d1');
  ...
  it('records an error without throwing', async () => {
    api.listRevisions.mockRejectedValueOnce(new Error('nope'));
    await loadRevisions('d1');
