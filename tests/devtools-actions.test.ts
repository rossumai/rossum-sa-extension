...
const asJson = (data: any) => Promise.resolve({ kind: 'json', data });
...
describe('loadResource', () => {
  ...
  it('resolves a schema-via-queue descriptor by fetching the queue then its schema', async () => {
    ...
    const getJson = vi
      .fn()
      .mockResolvedValueOnce({ schema: 'https://acme.rossum.app/api/v1/schemas/9' }); // the queue (via)
    ...
    expect(store.activeTab()!.resource).toEqual({
      type: 'schema',
      id: '9',
      apiPath: '/api/v1/schemas/9',
      label: 'Schema',
    });
  ...
  it('resolves an inbox-via-queue descriptor by fetching the queue then its inbox', async () => {
    ...
    const getJson = vi
      .fn()
      .mockResolvedValueOnce({ inbox: 'https://acme.rossum.app/api/v1/inboxes/9' }); // the queue (via)
  ...
  it('loadResource resolves a via:org descriptor to the organization', async () => {
    ...
    const getJson = vi.fn().mockResolvedValueOnce({
      results: [{ url: 'https://acme.rossum.app/api/v1/organizations/1' }],
    });
    ...
    expect(store.activeTab()!.resource).toEqual({
      type: 'organization',
      id: '1',
      apiPath: '/api/v1/organizations/1',
      label: 'Organization',
    });
  });
  ...
  it('loadResource shows a list read-only', async () => {
    ...
    await loadResource(t.id, {
      getResource: () => asJson({ results: [], pagination: {} }),
      patch: vi.fn(),
    });
    expect(store.activeTab()!.readOnly).toBe(true);
  });
  ...
  it('loadResource shows a clear 404 message (e.g. out-of-org / support-access user)', async () => {
    const t = store.openTab({
      type: 'user',
      id: '359651',
      apiPath: '/api/v1/users/359651',
      label: 'User',
    });
    await loadResource(t.id, {
      getResource: () => Promise.reject(Object.assign(new Error('x'), { status: 404 })),
      patch: vi.fn(),
    });
...
describe('saveResource', () => {
  it('PATCHes only changed keys, re-fetches canonical, then reloads the inspected page', async () => {
    ...
    store.patchTab(t.id, {
      ...
      buffer: JSON.stringify({ id: 1, name: 'B' }),
    });
    requestDiff(t.id);
  ...
  it('keeps the buffer and surfaces the server message on 400, and does not reload', async () => {
    ...
    requestDiff(t.id);
    const patch = vi.fn(() =>
      Promise.reject(Object.assign(new Error('x'), { status: 400, body: 'bad' })),
    );
    const getJson = vi.fn();
    const reload = vi.fn();
    await saveResource(t.id, { getJson, patch, reload });
  ...
  it('saveResource reloads the inspected page only for the page tab', async () => {
    // page tab => reload
    const p = store.openTab(
      { type: 'queue', id: '1', apiPath: '/api/v1/queues/1', label: 'Queue' },
      'page',
    );
    store.patchTab(p.id, { original: { name: 'A' }, buffer: JSON.stringify({ name: 'B' }) });
    requestDiff(p.id);
    const reloadP = vi.fn();
    await saveResource(p.id, {
      getJson: vi.fn(() => Promise.resolve({ name: 'B' })),
    ...
    // link tab => NO reload
    const l = store.openTab(
      { type: 'hook', id: '2', apiPath: '/api/v1/hooks/2', label: 'Hook' },
      'link',
    );
    store.patchTab(l.id, { original: { name: 'A' }, buffer: JSON.stringify({ name: 'B' }) });
    requestDiff(l.id);
    const reloadL = vi.fn();
    await saveResource(l.id, {
      getJson: vi.fn(() => Promise.resolve({ name: 'B' })),
      patch: vi.fn(() => Promise.resolve({})),
      reload: reloadL,
    });
...
describe('resource-change guard', () => {
  it('does not write stale state to a tab that navigated to a different resource mid-save', async () => {
    ...
    store.patchTab(t.id, {
      resource: { type: 'queue', id: '2', apiPath: '/api/v1/queues/2', label: 'Queue' },
      original: null,
      buffer: '',
      diffPreview: null,
    });
...
describe('openRequestPath', () => {
  ...
  it('opens a single resource as an editable tab (no query, has id)', () => {
    ...
    expect(r.tab!.resource).toEqual({
      type: 'queue',
      id: '9',
      apiPath: '/api/v1/queues/9',
      label: 'Queue',
    });
  ...
  it('returns an error (no tab) for a cross-host URL', () => {
    const r = openRequestPath(
      'https://other.rossum.app/api/v1/queues/1',
      'https://elis.rossum.app',
      deps,
    )!;
