...
function mount(props: any) { const el = document.createElement('div'); document.body.appendChild(el); render(h(ExportWizard, props), el); return el; }
...
describe('ExportWizard', () => {
  ...
  it('the Download button restates count and format, per scope', async () => {
    ...
    ([...root.querySelectorAll('[data-testid="export-scope"] button')].find((x) => x.textContent.trim().startsWith('Current filter')) as HTMLElement).click();
  ...
  it('the what-will-happen list is scope-aware', async () => {
    ...
    ([...root.querySelectorAll('[data-testid="export-scope"] button')].find((b) => b.textContent.trim().startsWith('Current filter')) as HTMLElement).click();
  ...
  it('warns inline above 10,000 documents — no popup', async () => {
    ...
    expect(root.querySelector<HTMLButtonElement>('[data-testid="export-download"]')!.disabled).toBe(false);
  });
  ...
  it('what-will-happen shows the columns line only for column formats', async () => {
    ...
    ([...root.querySelectorAll('[data-testid="export-format"] button')].find((b) => b.textContent.trim() === 'CSV') as HTMLElement).click();
  ...
  it('a failed count never blocks Download (config carries count: null)', async () => {
    ...
    const btn = await waitFor(() => { const b = root.querySelector<HTMLButtonElement>('[data-testid="export-download"]'); return b && !b.disabled ? b : null; });
  ...
  it('column discovery only runs for column formats, lazily, and is cached across a csv<->xlsx switch', async () => {
    ...
    const groupCalls = () => vi.mocked(api.aggregate).mock.calls.filter(([, pipeline]) => JSON.stringify(pipeline).includes('$group')).length;
    ...
    ([...root.querySelectorAll('[data-testid="export-format"] button')].find((b) => b.textContent.trim() === 'Excel') as HTMLElement).click();
