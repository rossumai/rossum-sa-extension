...
function mount() { const el = document.createElement('div'); render(h(PageRail, null), el); return el; }
...
describe('PageRail', () => {
  ...
  it('renders thumbnails with page numbers and a Rossum deep link', () => {
    store.pagePreviews.value = { status: 'done', total: 2, pages: [
    ...
    expect(el.querySelector('.inspector-page').getAttribute('href')).toBe('https://org.example/document/42');
    expect(el.textContent).toContain('2 pages');
  });

  it('shows a load-more button with the remaining count', () => {
    store.pagePreviews.value = { status: 'done', total: 6, pages: [
      { number: 1, width: 800, height: 1100, objectUrl: 'blob:one' },
...
describe('store.clearPagePreviews', () => {
  it('revokes object URLs and nulls the signal', () => {
    ...
    store.pagePreviews.value = { status: 'done', total: 1, pages: [{ number: 1, objectUrl: 'blob:x' }], rest: [] };
