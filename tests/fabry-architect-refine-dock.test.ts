...
let root: any;
function mount(props: any) { root = document.createElement('div'); document.body.appendChild(root); act(() => { render(h(RefineDock, props), root); }); return root; }
const flush = () => new Promise((r) => setTimeout(r, 0));
const accept = (r: any) => [...r.querySelectorAll('button')].find((b) => /accept/i.test(b.textContent));
const discard = (r: any) => [...r.querySelectorAll('button')].find((b) => /discard/i.test(b.textContent));
...
describe('RefineDock (docked, inline, design-system AI input)', () => {
  ...
  it('an instruction → refineTurn (fresh chat) → shows the diff + enables Accept; Accept applies via updateDeliverable', async () => {
    vi.mocked(actions.refineTurn).mockResolvedValue({ chatId: 'chat_1', proposal: '# refined requirement' });
    const r = mount({ deliverable: { id: 'a', text: '# base requirement' } });
  ...
  it('renders the agent questions (interactive elements) inline when a turn asks — no diff, no Accept', async () => {
    vi.mocked(actions.refineTurn).mockResolvedValue({ chatId: 'c', questions: [{ question: 'Which queue?' }] });
    const r = mount({ deliverable: { id: 'a', text: '# base' } });
    await submit(r, 'tighten');
  ...
  it('answering the questions calls answerRefine (same chat) and shows the resulting diff', async () => {
    vi.mocked(actions.refineTurn).mockResolvedValue({ chatId: 'c', questions: [{ question: 'Which queue?' }] });
    vi.mocked(actions.answerRefine).mockResolvedValue({ chatId: 'c', proposal: '# refined for the Invoices queue' });
    const r = mount({ deliverable: { id: 'a', text: '# base' } });
    await submit(r, 'tighten');
