...
vi.mock('../src/fabry/architect/actions.js', () => ({
  ...
  reImplement: vi.fn(), renameDeliverable: vi.fn(),
}));
...
function mount() { const root = document.createElement('div'); document.body.appendChild(root); act(() => render(<ArchitectSidebar />, root)); return root; }
...
describe('Sidebar implement controls (kill-switch on)', () => {
  ...
  it('kebab menu shows an Implement item that opens the arm dialog and chains to reImplement', async () => {
    ...
    const implBtn = [...menu!.querySelectorAll('.fabry-arch-menu-item')].find((b) => /implement/i.test(b.textContent));
    expect(implBtn).toBeTruthy();
    act(() => { (implBtn as HTMLElement).click(); });
  ...
  it('hides the kebab Implement item when the kill-switch is off', () => {
    ...
    act(() => { root.querySelector<HTMLElement>('.fabry-arch-kebab')!.click(); });
    const menu = root.querySelector('.fabry-arch-menu');
    expect([...menu!.querySelectorAll('.fabry-arch-menu-item')].find((b) => /implement/i.test(b.textContent))).toBeFalsy();
  });
  it('kebab menu Rename opens a prompt that renames the deliverable', () => {
    ...
    act(() => { root.querySelector<HTMLElement>('.fabry-arch-kebab')!.click(); });
    const menu = root.querySelector('.fabry-arch-menu');
    const renameBtn = [...menu!.querySelectorAll('.fabry-arch-menu-item')].find((b) => /rename/i.test(b.textContent));
    expect(renameBtn).toBeTruthy();
    act(() => { (renameBtn as HTMLElement).click(); });
