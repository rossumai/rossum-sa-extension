...
function mount(props: any) {
  ...
  render(<PlaceholderInputs {...props} />, root);
  return root;
}
...
describe('PlaceholderInputs UI', () => {
  ...
  it('the Auto option shows the true auto type even when overridden', () => {
    ...
    expect(root.querySelector<HTMLSelectElement>('select.placeholder-type-select')!.value).toBe(
      'string',
    );
  });
  it('offers Boolean only for true/false and Null only for empty/null', () => {
    const valueBased = () => ({ type: undefined, autoType: undefined });
    const optsFor = (value: any) =>
      [
        ...mount({
          ...common,
          values: { code: value },
          resolvedTypeFor: valueBased,
