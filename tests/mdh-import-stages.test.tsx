...
import {
  ...
  formatBytes,
  formatDuration,
} from '../src/mdh/components/ImportStages.jsx';
    ...
    const root = mount(
      <ImportSummary
        result={{
          ...
          deleted: 0,
          skipped: 0,
          failedBatches: [{ startIdx: 0, endIdx: 999, count: 1000, message: 'batch op errors' }],
          cancelled: false,
        }}
        fileMeta={{ name: 'x.json' }}
        onClose={() => {}}
...
describe('ImportProgress indeterminate', () => {
  ...
  it('shows a live heartbeat (status, check count, elapsed, file) when the server poll info is present', () => {
    const root = mount(
      <ImportProgress
        progress={{
          ...
          checks: 12,
          elapsedMs: 42_000,
          file: { filename: 'data.json', size: 30715 },
        }}
        onCancel={() => {}}
      />,
    );
    ...
    expect(
      [...root.querySelectorAll('.' + mstyles.actions + ' button')].some((b) =>
        /stop watching/i.test(b.textContent),
      ),
    ).toBe(true);
  });
});
