...
describe('preloadDeliverables', () => {
  ...
  const nowDeps = () => ({ requestIdleCallback: (fn) => { fn(); return 1; }, cancelIdleCallback: () => {} });
  ...
  it('cancelling stops the sweep mid-way', () => {
    ...
    const pending = [];
    const deps = { requestIdleCallback: (fn) => { pending.push(fn); return pending.length; }, cancelIdleCallback: () => {} };
