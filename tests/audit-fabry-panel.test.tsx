...
function mount() { render(<FabryPanel />, root); return root; }
function expandToggle(el: any) { act(() => { el.querySelector('.audit-fabry-toggle').click(); }); }
...
describe('FabryPanel', () => {
  ...
  it('collapsed bar shows the Fabry-generated takeaway line as the preview, dropping the rest', () => {
    store.fabry.value = { status: 'done', chatId: 'c1', error: null, forView: null, turns: [
  ...
  it('collapsed bar shows a streaming placeholder while the summary is still generating', () => {
    store.fabry.value = { status: 'running', chatId: null, error: null, forView: null, turns: [
  ...
  it('an error turn shows an honest note when expanded', () => {
    store.fabry.value = { status: 'error', chatId: null, error: 'x', forView: null, turns: [{ id: 1, question: null, text: '', reasoning: '', tools: [], state: 'error' }] };
    const el = mount();
    expandToggle(el);
  ...
  it('Enter in the ask input calls askAuditFabry with the value', () => {
    store.fabry.value = { status: 'done', chatId: 'c1', error: null, forView: null, turns: [{ id: 1, question: null, text: 'x', reasoning: '', tools: [], state: 'done' }] };
    const el = mount();
    expandToggle(el);
  ...
  it('View investigation lives in the bar (sibling of the toggle, never nested inside it) and only shows once expanded with a done turn', () => {
    store.fabry.value = { status: 'done', chatId: 'c1', error: null, forView: null, turns: [{ id: 1, question: null, text: 'x', reasoning: 'because search', tools: ['search'], state: 'done' }] };
    const el = mount();
...
describe('FabryPanel — stale summary (view changed since it was computed)', () => {
  it('collapsed + stale (forView mismatches the live view signature): shows the "view changed" marker next to the preview', () => {
    sigValue = 'sig-A';
    store.fabry.value = { status: 'done', chatId: 'c1', error: null, forView: 'sig-OLD', turns: [
      { id: 1, question: null, text: 'Quiet page.', reasoning: '', tools: [], state: 'done' },
    ] };
    const el = mount();
  ...
  it('collapsed + not stale (forView matches the live view signature): no marker', () => {
    sigValue = 'sig-A';
    store.fabry.value = { status: 'done', chatId: 'c1', error: null, forView: 'sig-A', turns: [
      { id: 1, question: null, text: 'Quiet page.', reasoning: '', tools: [], state: 'done' },
    ] };
    const el = mount();
    expect(el.querySelector('.audit-fabry-preview')).toBeTruthy();
  ...
  it('already open: becomes stale once rows land (availability -> available) and auto-triggers refreshSummary via the effect', async () => {
    ...
    store.fabry.value = { status: 'done', chatId: 'c1', error: null, forView: 'sig-1', turns: [
      { id: 1, question: null, text: 'Quiet page.', reasoning: '', tools: [], state: 'done' },
    ] };
    const el = mount();
    ...
    await act(async () => { store.availability.value = 'available'; }); // new rows have landed
  ...
  it('does not auto-refresh while a turn is already streaming, even once stale and available', async () => {
    ...
    await act(async () => {
      store.fabry.value = { ...store.fabry.value, status: 'running',
        turns: [...store.fabry.value.turns, { id: 2, question: null, text: '', reasoning: '', tools: [], state: 'streaming' }] };
    });
  ...
  it('preview shows the LAST summary turn once a refresh has appended a second one', () => {
    sigValue = 'sig-1';
    store.fabry.value = { status: 'done', chatId: 'c1', error: null, forView: 'sig-1', turns: [
      { id: 1, question: null, text: 'Old takeaway.\n- a\nNext step: x.', reasoning: '', tools: [], state: 'done' },
      { id: 2, question: null, text: 'New takeaway.\n- b\nNext step: y.', reasoning: '', tools: [], state: 'done' },
    ] };
    const el = mount();
    const preview = el.querySelector('.audit-fabry-preview');
  ...
  it('auto-refresh effect skips a view whose refreshFailedFor already matches the live signature; re-arms once filters change again', async () => {
    sigValue = 'sig-1';
    store.availability.value = 'available';
    store.fabry.value = { status: 'done', chatId: 'c1', error: null, forView: 'sig-1', refreshFailedFor: null, turns: [
      { id: 1, question: null, text: 'Quiet page.', reasoning: '', tools: [], state: 'done' },
    ] };
    const el = mount();
    ...
    await act(async () => {
      store.fabry.value = { ...store.fabry.value, status: 'error', error: 'boom', forView: 'sig-OLD-before-fail', refreshFailedFor: 'sig-1' };
    });
    expect(refreshSummary).not.toHaveBeenCalled();
    ...
    sigValue = 'sig-2';
    await act(async () => { store.fabry.value = { ...store.fabry.value }; }); // force a render pass
    expect(refreshSummary).toHaveBeenCalledTimes(1);
  });
  ...
  it('expand-click still retries manually even when refreshFailedFor already matches the live signature', () => {
    sigValue = 'sig-1';
    store.availability.value = 'available';
    store.fabry.value = { status: 'error', chatId: 'c1', error: 'boom', forView: 'sig-OLD', refreshFailedFor: 'sig-1', turns: [
