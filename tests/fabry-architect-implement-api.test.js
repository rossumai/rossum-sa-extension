...
describe('loadDeliverables (implement fields)', () => {
  it('returns persisted (stale) implement state', async () => {
    mdh.find.mockResolvedValue({ result: [
      { _id: 'a', kind: 'requirement', text: 'A', order: 1, implementStatus: 'passing', attempts: 2, implementRanAt: 5, lastImplementSummary: 'made rule', lastImplementWrites: [{ tool: 'create_rule', ok: true }], implementTasks: [{ id: 'k1', text: 't1', status: 'done' }] },
...
describe('saveImplementResult', () => {
  it('persists status + tasks + caps the journal to the last 10', async () => {
    ...
    const tasks = [{ id: 'k1', text: 't1', status: 'done' }, { id: 'k2', text: 't2', status: 'failed' }];
    await api.saveImplementResult('a', { status: 'failed', attempts: 3, writes: [], summary: 's', chatId: 'c', ranAt: 9, journal, tasks });
  ...
  it('defaults implementTasks to [] when none passed', async () => {
    await api.saveImplementResult('a', { status: 'passing', attempts: 1, writes: [], summary: '', chatId: null, ranAt: 1, journal: [] });
    expect(mdh.updateOne.mock.calls[0][2].$set.implementTasks).toEqual([]);
  });
});
