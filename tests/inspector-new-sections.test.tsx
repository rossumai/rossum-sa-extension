...
describe('IntakeSection', () => {
  it('renders intake items with data-evidence-id anchors', () => {
    store.data.value = { annotation: { id: 1 }, blocker: null, content: null, resolved: { _intakeLoaded: true } };
...
describe('WorkflowSection', () => {
  it('renders run + steps, current step highlighted', () => {
    store.data.value = { annotation: { id: 1 }, blocker: null, content: null, resolved: { _workflowLoaded: true } };
    store.evidence.value = { verdict: {}, items: [
...
describe('DriftSection', () => {
  ...
  it('two added messages get distinct, unique data-evidence-id anchors', () => {
    store.data.value = { annotation: { id: 1, messages: [] }, blocker: null, content: null, resolved: {} };
    store.live.value = { messages: [{ type: 'error', content: 'B1' }, { type: 'error', content: 'B2' }], matchedTriggerRules: [] };
    const el = mount(<DriftSection />);
    const anchors = [...el.querySelectorAll('[data-evidence-id^="drift:added:"]')].map((n) => n.getAttribute('data-evidence-id'));
