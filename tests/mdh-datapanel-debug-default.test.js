...
vi.mock('../src/mdh/components/PipelineEditor.jsx', () => ({
  default: ({ editorRef, initialValue, onChange, onValidChange }) => {
    ...
    if (editorRef) {
      ...
      editorRef.current = {
        ...
        setValue: (v) => { if (v === mock.text) return; mock.text = v; if (onChange) onChange(); },
...
describe('DataPanel — Aggregate Pipeline Debug visibility', () => {
  ...
  it('the editor was seeded with the default pipeline (sanity: mock reflects a real no-op setValue)', async () => {
    ...
    expect(parsed.some((s) => s && s.$match), 'default pipeline has a $match stage').toBe(true);
  });
});
