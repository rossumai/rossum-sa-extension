...
let tabsCreateMock: any;
let tabsQueryMock: any;
let tabsUpdateMock: any;
let windowsUpdateMock: any;
let windowCloseSpy: any;
...
describe('activateTab', () => {
  it('activates the tab, focuses its window, and closes the popup', async () => {
    await activateTab({ id: 7, windowId: 3 } as any);
  ...
  it('still closes the popup if tabs.update throws', async () => {
    ...
    await activateTab({ id: 1, windowId: 1 } as any);
    expect(windowCloseSpy).toHaveBeenCalled();
  });
});
