...
vi.mock('../src/fabry/architect/components/SourceEditor.jsx', () => ({
  default: ({ text, onChange }) => h('textarea', {
    class: 'cm-mock', value: text, onInput: (e) => onChange && onChange(e.currentTarget.value),
  }),
}));
...
const D = [
  { id: 'd1', text: '# One\n\nalpha\n', order: 1, title: '', titleSource: '', state: 'verified', stateDate: '2026-08-12' },
...
beforeEach(() => {
  ...
  store.results.value = { d1: { verdict: 'pass', evidence: 'ok', stale: false } };
...
describe('SpecView', () => {
  ...
  it('clicking a section header targets that deliverable, and moves an existing pin', () => {
    const root = mount();
    act(() => { root.querySelectorAll('.fabry-spec-sec-hd')[1].click(); });
    expect(store.spyTarget.value).toBe('d2');
    expect(store.pinnedTarget.value).toBe(null);          // no pin was set, so none is created
    act(() => { store.setPinnedTarget('d2'); root.querySelectorAll('.fabry-spec-sec-hd')[0].click(); });
  ...
  it('registers a navigator that jumps through the fast tween, not the browser slow one', async () => {
    ...
    expect(animateScrollTop.mock.calls[0][0]).toBe(root.querySelector('.docs-root'));
  ...
  it('renders every deliverable in edit mode as an editable field, with the same headers', () => {
    ...
    expect([...root.querySelectorAll('.cm-mock')].map((t) => t.value).join(' ')).toMatch(/alpha/);
    expect([...root.querySelectorAll('.cm-mock')].map((t) => t.value).join(' ')).toMatch(/beta/);
  });
});
...
describe('review at document width', () => {
  ...
  it('renders the requested panel above its own section, and closes', () => {
    ...
    expect(host.closest('[data-deliverable]').dataset.deliverable).toBe('d2');

    act(() => { host.querySelector('[data-act="review-close"]').click(); });
    expect(root.querySelector('.fabry-spec-review')).toBe(null);
  });
