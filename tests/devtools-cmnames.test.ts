...
async function waitFor(fn: any, tries = 200) {
...
describe('cmNames', () => {
  ...
  it('skips non-nameable URLs (nameFor null) without a widget or ensure', () => {
    const ensure = vi.fn();
    const view = mountDoc(
      line('https://acme.rossum.app/api/v1/annotations/1/content'),
      () => null,
      ensure,
    );
    expect(view.dom.querySelector('.rawjson-name')).toBeNull();
  ...
  it('skips a resolved link that has no display name', () => {
    const view = mountDoc(line('https://acme.rossum.app/api/v1/queues/3'), () => ({
      status: 'done',
      name: null,
    }));
    expect(view.dom.querySelector('.rawjson-name')).toBeNull();
  ...
  it('shows the name after async resolution (ensure → refresh effect → rebuild)', async () => {
    ...
    let resolved = false;
    const nameFor = () =>
      resolved ? { status: 'done', name: 'Later' } : { status: 'none', name: null };
    const ensure = (u: any, cb: any) => {
      resolved = true;
      setTimeout(cb, 0);
    }; // resolve, then notify
  ...
  it('joins multiple names on one line with a middot', () => {
    ...
    const doc = `  "hooks": ["${a}", "${b}"],\n`;
    const view = mountDoc(doc, (u: any) => ({
      status: 'done',
      name: u.endsWith('5') ? 'Alpha' : 'Beta',
    }));
