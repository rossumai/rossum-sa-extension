...
describe('architect api v2', () => {
  ...
  it('uses the _SA_EXTENSION__fabry_architect collection', () => {
    expect(api.COLLECTION).toBe('_SA_EXTENSION__fabry_architect');
  });

  it('ensureCollection tolerates already-exists but rethrows 401', async () => {
    vi.mocked(mdh.createCollection).mockRejectedValueOnce(
      Object.assign(new Error('exists'), { status: 400 }),
    );
  ...
  it('loadDeliverables maps docs and derives stale persisted results', async () => {
    ...
    expect(mdh.find).toHaveBeenCalledWith('_SA_EXTENSION__fabry_architect', {
  ...
  it('loadDeliverables maps a persisted title verbatim', async () => {
    vi.mocked(mdh.find).mockResolvedValueOnce({
      result: [{ _id: 'a', kind: 'requirement', text: '# A', order: 1, title: 'A Nice Title' }],
    });
    const { deliverables } = await api.loadDeliverables();
    expect(deliverables).toEqual([
      {
  ...
  it('loadDeliverables tolerates a missing result envelope', async () => {
    vi.mocked(mdh.find).mockResolvedValueOnce({});
    expect(await api.loadDeliverables()).toEqual({
      deliverables: [],
      results: {},
      implement: {},
      legacyCount: 0,
  ...
  it('addDeliverable inserts the documented shape', async () => {
    ...
    expect(mdh.insertOne).toHaveBeenCalledWith('_SA_EXTENSION__fabry_architect', {
      _id: 'x',
      kind: 'requirement',
      text: 'body',
      order: 3,
  ...
  it('updateDeliverable $sets text + editedAt', async () => {
    ...
    expect(mdh.updateOne).toHaveBeenCalledWith(
      '_SA_EXTENSION__fabry_architect',
      { _id: 'x' },
      { $set: { text: 'new', editedAt: 222 } },
    );
  });
  ...
  it('saveResult $sets the last-run fields', async () => {
    ...
    expect(mdh.updateOne).toHaveBeenCalledWith(
      '_SA_EXTENSION__fabry_architect',
      { _id: 'x' },
      { $set: { lastVerdict: 'fail', lastEvidence: 'bad', lastChatId: 'c9', ranAt: 333 } },
    );
  });
  ...
  it('saveTitle $sets title + its source by _id', async () => {
    ...
    expect(mdh.updateOne).toHaveBeenCalledWith(
      '_SA_EXTENSION__fabry_architect',
      { _id: 'x' },
      { $set: { title: 'A Nice Title', titleSource: 'manual' } },
    );
  });
