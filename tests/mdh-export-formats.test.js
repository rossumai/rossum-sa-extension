...
describe('export format registry', () => {
  ...
  it('fetchCount uses the finite config.count with NO api call (fast path)', async () => {
    const stages = [{ $match: { region: 'EU' } }];
    const job = buildExportJob({ scope: 'filtered', formatId: 'json', opts: {}, columns: null, count: 42 }, 'vendors', stages);
    expect(await job.fetchCount()).toBe(42);
  ...
  it('fetchCount runs a real $count over the job pipelineStages when config.count is null (CRITICAL fix)', async () => {
    api.aggregate.mockResolvedValue({ result: [{ total: 99 }] });
    const stages = [{ $match: { region: 'EU' } }];
    const job = buildExportJob({ scope: 'filtered', formatId: 'json', opts: {}, columns: null, count: null }, 'vendors', stages);
  ...
  it('fetchCount runs the real count for the "all" scope too, over [{$match:{}}]', async () => {
    api.aggregate.mockResolvedValue({ result: [{ total: 5 }] });
    const allJob = buildExportJob({ scope: 'all', formatId: 'json', opts: {}, columns: null, count: undefined }, 'vendors', null);
  ...
  it('fetchCount resolves 0 when the real count aggregation returns no rows', async () => {
    ...
    const job = buildExportJob({ scope: 'all', formatId: 'json', opts: {}, columns: null, count: NaN }, 'vendors', null);
  ...
  it('fetchCount propagates an aggregate rejection instead of silently degrading to 0', async () => {
    api.aggregate.mockRejectedValue(new Error('aggregate failed'));
    const job = buildExportJob({ scope: 'all', formatId: 'json', opts: {}, columns: null, count: null }, 'vendors', null);
