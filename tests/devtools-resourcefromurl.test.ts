...
describe('resourceFromApiUrl', () => {
  it('maps known collections to type/label with an id and apiPath', () => {
    expect(resourceFromApiUrl('https://acme.rossum.app/api/v1/schemas/42')).toEqual({
      type: 'schema',
      id: '42',
      apiPath: '/api/v1/schemas/42',
      label: 'Schema',
    });
    expect(resourceFromApiUrl('https://acme.rossum.app/api/v1/workspaces/7')).toEqual({
      type: 'workspace',
      id: '7',
      apiPath: '/api/v1/workspaces/7',
      label: 'Workspace',
    });
    expect(resourceFromApiUrl('https://acme.rossum.app/api/v1/organizations/1')).toEqual({
      type: 'organization',
      id: '1',
  ...
  it('maps inboxes to type inbox with a mapped label', () => {
    expect(resourceFromApiUrl('https://acme.rossum.app/api/v1/inboxes/5')).toEqual({
      type: 'inbox',
      id: '5',
      apiPath: '/api/v1/inboxes/5',
      label: 'Inbox',
    });
  });
  it('falls back generically for an unknown collection', () => {
    expect(resourceFromApiUrl('https://acme.rossum.app/api/v1/unknowns/5')).toEqual({
      type: 'unknowns',
      id: '5',
      apiPath: '/api/v1/unknowns/5',
      label: 'Unknown',
    });
  });
  ...
  it('singularizes generic fallback labels correctly', () => {
    ...
    expect(resourceFromApiUrl('https://acme.rossum.app/api/v1/duplicates/4')!.label).toBe(
      'Duplicate',
    );
  ...
  it('captures a sub-resource path ending in a numeric id (e.g. a datapoint content)', () => {
    expect(
      resourceFromApiUrl(
        'https://acme.rossum.app/api/v1/annotations/138328520/content/19453284337',
      ),
    ).toEqual({
      type: 'annotations',
      id: '138328520',
  ...
  it('tolerates a trailing slash / query on a numeric sub-resource path', () => {
    expect(
      resourceFromApiUrl('https://acme.rossum.app/api/v1/annotations/1/content/2/')!.apiPath,
    ).toBe('/api/v1/annotations/1/content/2');
    expect(
      resourceFromApiUrl('https://acme.rossum.app/api/v1/annotations/1/content/2?x=1')!.label,
    ).toBe('Content 2');
  });
  ...
  it('still tolerates a trailing slash on a no-sub URL', () => {
    expect(resourceFromApiUrl('https://acme.rossum.app/api/v1/hooks/3/')!.id).toBe('3');
    expect(resourceFromApiUrl('https://acme.rossum.app/api/v1/hooks/3/')!.apiPath).toBe(
      '/api/v1/hooks/3',
    );
  });
  it('marks organization_groups read-only (viewable but not PATCHable)', () => {
    expect(resourceFromApiUrl('https://acme.rossum.app/api/v1/organization_groups/49101')).toEqual({
      type: 'organization_group',
      id: '49101',
      apiPath: '/api/v1/organization_groups/49101',
...
describe('genericResourceFromPath', () => {
  it('builds a read-only descriptor for a bare collection', () => {
    expect(genericResourceFromPath('/api/v1/queues')).toEqual({
      type: 'queues',
      apiPath: '/api/v1/queues',
      label: 'queues',
      readOnly: true,
    });
  });
