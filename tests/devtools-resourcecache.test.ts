...
describe('pickName', () => {
  it('shows a user as "username (First Last)", degrading gracefully', () => {
    expect(cache.pickName('user', { username: 'jdoe', first_name: 'Jane', last_name: 'Doe' })).toBe(
      'jdoe (Jane Doe)',
    );
  ...
  it('shows a document by original_file_name', () => {
    expect(
      cache.pickName('documents', { original_file_name: 'invoice.pdf', name: 'ignored' }),
    ).toBe('invoice.pdf');
...
describe('cache', () => {
  ...
  it('put derives a user name from an apiPath', () => {
    expect(cache.put('/api/v1/users/7', { username: 'ab', first_name: 'A', last_name: 'B' })).toBe(
      'ab (A B)',
    );
  });
