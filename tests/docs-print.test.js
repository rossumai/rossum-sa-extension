...
describe('buildPrintDocument', () => {
  ...
  it('no longer prints a manual state badge — status is the check verdict alone', () => {
    ...
    const { html } = buildPrintDocument({
      deliverables: [{ id: 'a', title: 'A', text: 'body', state: 'verified', stateDate: '2026-08-12' }],
...
describe('print staging', () => {
  ...
  it('does nothing without a document', async () => {
    const deps = { sessionSet: vi.fn(), tabsCreate: vi.fn(), uuid: () => 'x', now: () => 0, getURL: (p) => p, getCurrentTab: vi.fn() };
