...
import {
  getSchemaHints,
  detectNumericStringFields,
  leafFieldTypes,
  arrayLeafPaths,
  summarizeSearchIndexes,
  extendedJsonType,
...
describe('schema-hint detectors (pure, from in-memory records)', () => {
  ...
  it('maps leaf field types and treats extended-JSON as its semantic type', () => {
    const t = leafFieldTypes([
      { n: 1, s: 'a', ok: true, d: { $date: '2020-01-01' }, id: { $oid: 'abc' } },
    ]);
    expect(t).toEqual({ n: 'number', s: 'string', ok: 'boolean', d: 'date', id: 'objectId' });
  });

  it('finds array leaf paths for $unwind', () => {
    expect(arrayLeafPaths([{ items: [{ sku: 'A', qty: 2 }] }])).toEqual([
      'items[].qty',
      'items[].sku',
    ]);
