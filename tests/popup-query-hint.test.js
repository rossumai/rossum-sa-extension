...
const dot = (top) => ({ left: 10, right: 22, top, bottom: top + 12 });
...
describe('placeHint', () => {
  ...
  it('pulls a wide tip left so it never runs off the right edge', () => {
    const p = placeHint({ left: 300, right: 312, top: 100, bottom: 112 }, { width: 300, height: 40 }, VIEW);
  ...
  it('never returns a negative coordinate', () => {
    const p = placeHint({ left: -50, right: -38, top: -20, bottom: -8 }, { width: 200, height: 40 }, VIEW);
...
const seen = (fn) => vi.waitFor(fn, { timeout: 5000, interval: 10 });
...
const opened = (r) => seen(() => expect(r.querySelector('.mdh-q-hint')?.style.visibility).toBe('visible'));

let root = null;
afterEach(() => { if (root) render(null, root); root = null; document.body.innerHTML = ''; });
function mount(props) {
  root = document.createElement('div');
  document.body.appendChild(root);
  render(h(QueryItem, { index: 0, label: 'vendor lookup', onCopy: () => {}, onOpen: () => {}, ...props }), root);
  return root;
}

describe('QueryItem — the hint never occupies layout', () => {
  const ERR = { status: 'error', hint: '400: "compound.filter[0].queryString.query" cannot be empty' };
  ...
  it('renders no inline detail for skipped or gated either', () => {
    expect(mount({ status: { status: 'skipped', hint: 'cascade short-circuited' } }).querySelector('.mdh-q-detail')).toBeNull();
    render(null, root);
    expect(mount({ status: { status: 'gated', hint: 'action_condition gates this' } }).querySelector('.mdh-q-detail')).toBeNull();
  });
  ...
  it('carries inline coordinates, which is what the fixed rule positions', async () => {
    const r = mount({ status: ERR });
    r.querySelector('.mdh-q-status').dispatchEvent(new MouseEvent('mouseenter', { bubbles: false }));
