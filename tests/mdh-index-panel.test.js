...
beforeEach(() => {
  ...
  api.collectionStats.mockResolvedValue({ result: [] });
});
...
describe('IndexPanel — copy is create-ready', () => {
  it('Copy emits a clean { indexName, keys, options } definition', async () => {
    ...
    root.querySelector('.action-copy').click();
  ...
  it('copied JSON drops the internal v and is not the raw listed object', async () => {
    api.listIndexes.mockResolvedValue({ result: [{ v: 2, key: { a: 1 }, name: 'a_1' }] });
    const root = mount();
    await vi.waitFor(() => expect(root.querySelector('.action-copy')).not.toBeNull());
    root.querySelector('.action-copy').click();
...
describe('IndexPanel — size from $collStats', () => {
  it('shows per-index size and collection totals', async () => {
    api.listIndexes.mockResolvedValue({ result: [{ v: 2, key: { ALT1: 1 }, name: 'products_alt1_idx' }] });
    api.collectionStats.mockResolvedValue({ result: [{
  ...
  it('degrades silently when $collStats fails', async () => {
    api.listIndexes.mockResolvedValue({ result: [{ v: 2, key: { a: 1 }, name: 'a_1' }] });
    api.collectionStats.mockRejectedValue(new Error('not authorized'));
    const root = mount();
...
describe('IndexPanel — async create surfaces operation outcome', () => {
  ...
  async function openAndSubmitCreate(root) {
  ...
  it('sets the red error banner (error.value) when the operation fails', async () => {
    api.listIndexes.mockResolvedValue({ result: [] });
    api.createIndex.mockResolvedValue(OP_ACCEPT);
    api.waitForOperation.mockRejectedValue(new Error('E11000 duplicate key on a:1'));
  ...
  it('shows an info opNotice while running, then clears it and re-lists on finish', async () => {
    ...
    api.createIndex.mockResolvedValue(OP_ACCEPT);
    let resolveOp;
    api.waitForOperation.mockReturnValue(new Promise((r) => { resolveOp = r; }));
    const root = mountWithModal();

    await vi.waitFor(() => expect(buttonByText(root, '+ Create')).toBeTruthy());
    const listCallsBefore = api.listIndexes.mock.calls.length;
    await openAndSubmitCreate(root);
    ...
    expect(api.listIndexes.mock.calls.length).toBeGreaterThan(listCallsBefore); // re-list on finish
  });

  it('clears the in-progress opNotice when the collection changes mid-operation', async () => {
    api.listIndexes.mockResolvedValue({ result: [] });
    api.createIndex.mockResolvedValue(OP_ACCEPT);
    api.waitForOperation.mockReturnValue(new Promise(() => {})); // never resolves → stays RUNNING
    const root = mountWithModal();
