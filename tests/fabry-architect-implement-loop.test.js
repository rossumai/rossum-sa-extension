...
const ds = (n) => Array.from({ length: n }, (_, i) => ({ id: 'd' + i, text: 'Add VAT validation ' + i }));
const oneD = () => [{ id: 'd0', text: 'Add VAT validation' }];
const collect = () => { const ev = {}; return { onEvent: (id, p) => { ev[id] = { ...(ev[id] || {}), ...p }; }, ev }; };
const taskByText = (tasks, text) => (tasks || []).find((t) => t.text === text);
