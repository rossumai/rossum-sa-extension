...
describe('usePipeline type-aware functions', () => {
  ...
  it('referencedFields lists resolved (collection, field) pairs, skipping ambiguous', () => {
    const p = getPipeline();
    expect(p.referencedFields(M)).toEqual([{ collection: 'col', field: 'code' }]);
    expect(p.referencedFields('[{"$match":{"$or":[{"a":"{x}"},{"b":"{x}"}]}}]')).toEqual([]);
  });
  it('ensureFieldTypes merges resolver output per collection and reports change', async () => {
    const p = getPipeline();
    const fake = async () => ({ code: strInfo });
    expect(await p.ensureFieldTypes([{ collection: 'col', field: 'code' }], fake)).toBe(true);
    expect(p.fieldTypes.value.col.code.dominant).toBe('string');
    expect(await p.ensureFieldTypes([{ collection: 'col', field: 'code' }], fake)).toBe(false); // already known
  });
