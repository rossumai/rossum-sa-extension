...
function mockAllSources() {
  api.getAnnotation.mockResolvedValue({ id: 1, status: 'to_review', messages: [], queue: 'https://h/api/v1/queues/3' });
  api.getContent.mockResolvedValue({ content: [] });
  api.getQueue.mockResolvedValue({ id: 3 });
  ...
  api.listWorkflowSteps.mockResolvedValue([]);
  api.listPages.mockResolvedValue([]);
  api.getBlob.mockResolvedValue({ size: 1 });
}
...
describe('page previews', () => {
  it('loads page list + first-batch blobs into pagePreviews with object URLs', async () => {
    ...
    try {
      ...
      expect(pv.pages.map((p) => p.number)).toEqual([1, 2]); // sorted by number
...
describe('closeAnnotation (back to landing)', () => {
  it('clears the annotation and prevents in-flight loaders from writing', async () => {
    ...
    mockAllSources();
    let resolveRules;
    api.listRules.mockReturnValue(new Promise((r) => { resolveRules = r; })); // keep gather in flight
    store.setAnnotationId('1');
