...
const D = [
  { id: 'd1', text: '# One', order: 1, title: 'One', titleSource: 'manual' },
  { id: 'd2', text: '# Two', order: 2, title: 'Two', titleSource: 'manual' },
];
function mount() {
  const root = document.createElement('div');
  document.body.appendChild(root);
  act(() => { render(h(InspectorRail, null), root); });
  return root;
}
beforeEach(() => {
  vi.clearAllMocks();
  ...
  store.results.value = {
    d1: { verdict: 'pass', evidence: 'ok', stale: false },
    d2: { verdict: 'fail', evidence: 'no rule', stale: false },
  };
...
describe('InspectorRail', () => {
  ...
  it('HOLDS the target while a check runs for the shown deliverable', () => {
    const root = mount();
    act(() => { store.setResult('d1', { running: true }); });
    act(() => { store.setSettledTarget('d2', { immediate: true }); });
    expect(root.querySelector('.fabry-rail-name').textContent).toMatch(/One/);
  ...
  it('offers to open a diff at document width, for the panels that need the room', () => {
    ...
    act(() => { [...root.querySelectorAll('.fabry-rail-tab')].find((t) => /History/.test(t.textContent)).click(); });
    act(() => { root.querySelector('.fabry-rail-wide').click(); });
