...
describe('normalizeRequestInput', () => {
  ...
  it('rejects a full URL when the current domain cannot be resolved (empty / schemeless)', () => {
    expect(normalizeRequestInput('https://elis.rossum.app/api/v1/queues/9', '')!.error).toBeTruthy();
    expect(normalizeRequestInput('https://elis.rossum.app/api/v1/queues/9', 'elis.rossum.app')!.error).toBeTruthy();
  });
});
