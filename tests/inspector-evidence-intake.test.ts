...
describe('intakeEvidence', () => {
  it('classifies arrival by attachment_status (verified vocabulary)', () => {
    ...
    for (const [status, re] of cases) {
      const items = intakeEvidence({
        ...base,
        document: { attachment_status: status, arrived_at: '2026-07-01T09:14:00Z' },
      });
  ...
  it('duplicate relation lists sibling annotation ids, edit relations ignored', () => {
    const items = intakeEvidence({
      ...
      relations: [
        ...
        {
          type: 'duplicate',
          annotations: ['https://x/api/v1/annotations/1', 'https://x/api/v1/annotations/2'],
        },
      ],
    });
  ...
  it('email with unknown shape degrades honestly', () => {
    const items = intakeEvidence({
      ...base,
      document: { attachment_status: 'processed', email: 'https://x/api/v1/emails/9' },
      email: {},
    });
...
describe('workflowEvidence', () => {
  ...
  it('run + steps + assignee activity', () => {
    const items = workflowEvidence({
      ...base,
      workflowRuns: [
        {
          id: 10,
          workflow_status: 'in_review',
          current_step: 'https://x/api/v1/workflow_steps/3',
      ...
      workflowSteps: [
        {
          id: 2,
          url: 'https://x/api/v1/workflow_steps/2',
          name: 'Team lead',
          ordering: 1,
          mode: 'any',
        },
        {
          id: 3,
          url: 'https://x/api/v1/workflow_steps/3',
          name: 'Finance',
          ordering: 2,
          mode: 'all',
        },
      ...
      enrichment: {
        workflow: [
          {
            action: 'step_started',
            workflow_step: 'https://x/api/v1/workflow_steps/3',
            assignees: ['https://x/api/v1/users/77'],
          },
        ],
      },
    });
      ...
      enrichment: {
        workflow: [
          ...
          },
          {
            action: 'step_started',
            workflow_step: 'https://x/api/v1/workflow_steps/3',
            assignees: ['https://x/api/v1/users/1'],
