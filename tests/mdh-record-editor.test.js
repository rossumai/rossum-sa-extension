...
async function flushUntilEditorReady(root) {
  for (let i = 0; i < 10; i++) {
...
describe('openRecordEditor — edit mode', () => {
  ...
  it('submits unmodified default → updateOne sees $set with full record (empty $unset stripped)', async () => {
    ...
    api.updateOne.mockResolvedValueOnce({ result: { matched_count: 1, modified_count: 0 } });
  ...
  it('submits with a filled $unset → updateOne sees both $set and $unset', async () => {
    ...
    api.updateOne.mockResolvedValueOnce({ result: { matched_count: 1, modified_count: 1 } });
