...
- MDH state: `mdhPipelineWidth`, `mdhUploadsColumnWidths`, `mdhActiveView`, `mdhSelectedCollection`, `mdhActivePanel`, `mdhOpsSearch`, `mdhOverviewChartsScale` (Overview Charts scale, shared by both charts: `linear`|`sqrt`|`log`), `mdhResultsView` (results view: `list`|`table`|`stages`; legacy `json` reads as `list`), `mdhStagesAutoscroll` (Stages-view auto-scroll toggle, default on), `mdhStagesSampleSize` (Stages-view records per stage: `10`|`25`|`50`, default 10)
