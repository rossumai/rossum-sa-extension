...
- **`llmPipeline.js` + `aiPipelineLoop.js` + `aiContext.js`** — the AI pipeline input (NL → aggregation). `llmPipeline.js` is pure (prompt building + schema-hint blocks, reply/fence parsing, `ensureRowLimit` 50-row cap, the `// 🤖 AI request:` editor comment, `detectNumericStringFields`/`leafStringFields`/`summarizeSearchIndexes`). `aiPipelineLoop.js`'s `runAiPipeline` is an agentic self-correcting loop: generate → execute against the collection → on a backend error or 0 rows, feed the outcome back to `llmchat` as a fresh one-shot user message and retry (max 3, stop on no-progress; tracks RAW model output separately from the capped applied text). `aiContext.js`'s `getSchemaHints` (cached per collection) enriches the prompt with sample docs + distinct low-card values (one `$facet`) + numeric-string fields + Atlas Search indexes — so the model uses exact stored values, `$toInt` for string-of-digits compares, and `$search` for free-text. Engine = Rossum internal `POST /api/v1/internal/llmchat` (`api.llmChat`/`api.probeLlmChat`, on `baseDomain`) — per-org feature-flagged; a cheap empty-body probe at MDH init sets `store.aiAvailable` so the colourful input (restored from `d99e48c`, `console.css .nl-search-*`) renders only where it works. No status notices (the applied query is the feedback); loader shows rotating MongoDB/Rossum gerunds. Live-verified on a customer dev org.
...
- MDH state: `mdhPipelineWidth`, `mdhUploadsColumnWidths`, `mdhActiveView`, `mdhSelectedCollection`, `mdhActivePanel`, `mdhOpsSearch`, `mdhOverviewChartsScale` (Overview Charts scale, shared by both charts: `linear`|`sqrt`|`log`), `mdhResultsView` (results view: `list`|`table`|`stages`; legacy `json` reads as `list`)
