...
/** One lexeme. `value` is the decoded literal for STRING/NUMBER, the text for IDENT/OP. */
export type Token = { type: string; value?: string | number };

/** The expression AST. A discriminated union, so evalNode narrows on `type`. */
export type Node =
...
class Parser {
  ...
  parseComparison(): Node {
    let left: Node = this.parsePrimary();
    while (true) {
      const t = this.peek();
      if (!t) break;
  ...
  parsePrimary(): Node {
    ...
    if (t.type === 'STRING') return { type: 'literal', value: t.value as string };
    if (t.type === 'NUMBER') return { type: 'literal', value: t.value as number };
    if (t.type === 'OP' && t.value === '-') {
      const child: Node = this.parsePrimary();
...
function pyLt(a: any, b: any): boolean {
...
export function evalCondition(expr: unknown): { result: boolean | null; error: string | null } {
  ...
  try {
    ...
    return { result: null, error: (e as Error)?.message || String(e) };
  }
}
