...
export const defaultDeps = {
  getLocal: (keys: string[]) => chrome.storage.local.get(keys),
  setLocal: (obj: Record<string, unknown>) => chrome.storage.local.set(obj),
  getSession: (keys: string[]) => chrome.storage.session.get(keys),
  setSession: (obj: Record<string, unknown>) => chrome.storage.session.set(obj),
...
async function sessionId(d: any): Promise<string> {
...
async function collectOne(msg: any, deps: Partial<typeof defaultDeps> = {}) {
  const d = { ...defaultDeps, ...deps };
  try {
    const name = msg && msg.name;
    if (typeof name !== 'string' || !(EVENT_NAMES as readonly string[]).includes(name)) return 0;
    ...
    let clientId: string | undefined = stored.usageClientId as string | undefined;
    if (!clientId) {
      clientId = d.uuid();
