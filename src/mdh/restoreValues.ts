...
const childPath = (prefix: string, key: string) =>
  prefix ? `${prefix}.${encodeSegment(key)}` : encodeSegment(key);
...
function restoreLeaf(value: any, path: string, ctx: Ctx): any {
  ...
  if (value === '' || value === null) {
    if (types && ctx.optional.has(path)) {
      ctx.hit.dropped.add(path);
      return DROP;
    }
    if (types && value === '' && want && want !== 'string') {
      ctx.hit.nulled.add(path);
      return null;
    }
    return value;
  }
  ...
  if (want && typeof value === 'string') {
    ...
    if (want === 'array') {
      ...
      ctx.hit.arrays.add(path);
      return [value]; // XML's 1-element collapse
    }
    if (want === 'date' && ISO_RE.test(value)) {
      ctx.hit.dates.add(path);
      return { $date: value };
    }
    if (want === 'objectId' && OID_RE.test(value)) {
      ctx.hit.oids.add(path);
      return { $oid: value };
    }
    if (want === 'number') {
      const n = inferValue(value);
      if (typeof n === 'number') {
        ctx.hit.numbers.add(path);
        return n;
      }
    }
    if (want === 'bool' && (value === 'true' || value === 'false')) {
      ctx.hit.bools.add(path);
      return value === 'true';
    }
  ...
  if (typeof value === 'string') {
    ...
    if (ctx.inferTypes) {
      ...
      if (v !== value) {
        if (typeof v === 'number') ctx.hit.numbers.add(path);
        else ctx.hit.bools.add(path);
        return v;
      }
    }
  }
...
export function restoreDocs(
  docs: any[],
  shape: any | null,
  { inferTypes = false }: { inferTypes?: boolean } = {},
  ...
  if (conflicts.size > 0) {
    warnings.push(
      `${conflicts.size} column(s) could not be nested because a conflicting field exists: ${[...conflicts].slice(0, 5).join(', ')}.`,
    );
  }
  ...
  return {
    docs: out,
    summary: {
      nestedColumns: ctx.hit.nested.size,
      json: ctx.hit.json.size,
      dates: ctx.hit.dates.size,
      oids: ctx.hit.oids.size,
      numbers: ctx.hit.numbers.size,
      bools: ctx.hit.bools.size,
      arrays: ctx.hit.arrays.size,
      dropped: ctx.hit.dropped.size,
      nulled: ctx.hit.nulled.size,
      warnings,
    },
  };
}
...
export function formatRestoreSummary(
  ...
  { hasShape, shapeError = false }: { hasShape: boolean; shapeError?: boolean },
): string | null {
  const parts: string[] = [];
  ...
  if (shapeError)
    return `Restored ${list} · couldn’t read the collection’s types, so values were left as text.`;
