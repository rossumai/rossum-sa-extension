...
function hasStructuralChange(prev: any, next: any) {
...
async function pollOperations() {
  try {
    ...
    const newById = new Map(newOps.map((o: any) => [o._id, o]));
    ...
    store.operations.value = store.operations.value.map((prevOp) => {
      const nextOp: any = newById.get(prevOp._id);
...
export async function initMdh({
  ...
  if (tab.mdhSelectedCollection) {
    store.selectedCollection.value = tab.mdhSelectedCollection as string;
  }
  if (tab.mdhActivePanel) {
    store.activePanel.value = tab.mdhActivePanel as string;
  }
