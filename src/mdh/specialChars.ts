...
export function classifySpecial(cp: number): { category: string; name: string; abbr: string } | null {
  if (cp === 0x20) return null; // ordinary space is never special
  const abbr = ABBR[cp as keyof typeof ABBR] || cpLabel(cp);
  if (SPACE_CPS.has(cp)) return { category: 'space', name: NAMES[cp as keyof typeof NAMES] || 'SPACE', abbr };
  if (ZERO_WIDTH_CPS.has(cp)) return { category: 'zero-width', name: NAMES[cp as keyof typeof NAMES] || 'ZERO WIDTH', abbr };
  if (BIDI_CPS.has(cp)) return { category: 'bidi', name: NAMES[cp as keyof typeof NAMES] || 'BIDI CONTROL', abbr };
  if (isControl(cp)) return { category: 'control', name: NAMES[cp as keyof typeof NAMES] || ('CONTROL ' + cpLabel(cp)), abbr };
  return null;
}

export function hasSpecial(str: unknown): boolean {
  if (typeof str !== 'string') return false;
  for (const ch of str) {
    if (classifySpecial(ch.codePointAt(0) as number)) return true;
  }
  return false;
}

export function tokenizeSpecial(str: string, { limit }: { limit?: number } = {}) {
  const tokens: any[] = [];
  let buf = '';
  let count = 0;
  let truncated = false;
