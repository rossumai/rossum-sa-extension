...
const ZERO_WIDTH_CPS = new Set([0x200b, 0x200c, 0x200d, 0x2060, 0xfeff, 0x00ad, 0x180e]);
...
function isControl(cp: number): boolean {
  return cp <= 0x1f || cp === 0x7f || (cp >= 0x80 && cp <= 0x9f) || cp === 0x2028 || cp === 0x2029;
}

export function classifySpecial(
  cp: number,
): { category: string; name: string; abbr: string } | null {
  if (cp === 0x20) return null; // ordinary space is never special
  const abbr = ABBR[cp as keyof typeof ABBR] || cpLabel(cp);
  if (SPACE_CPS.has(cp))
    return { category: 'space', name: NAMES[cp as keyof typeof NAMES] || 'SPACE', abbr };
  if (ZERO_WIDTH_CPS.has(cp))
    return { category: 'zero-width', name: NAMES[cp as keyof typeof NAMES] || 'ZERO WIDTH', abbr };
  if (BIDI_CPS.has(cp))
    return { category: 'bidi', name: NAMES[cp as keyof typeof NAMES] || 'BIDI CONTROL', abbr };
  if (isControl(cp))
    return {
      category: 'control',
      name: NAMES[cp as keyof typeof NAMES] || 'CONTROL ' + cpLabel(cp),
      abbr,
    };
  return null;
}
