...
function hasWriteStage(arr) {
  return Array.isArray(arr) && stripWriteStages(arr).length !== arr.length;
}

export function capRows(arr) {
  if (!Array.isArray(arr)) return arr;
  const has = (k) => arr.some((s) => s && typeof s === 'object' && k in s);
...
export function schemaHintParts({ knownValues = null, topValues = null, ranges = null,
  numericStringFields = null, searchIndexes = null, fieldTypes = null, arrayPaths = null } = {}) {
  const parts = [];
...
export function buildFixPrompt({ verdict, samples = null }) {
  const parts = [];
...
export function buildVerifyPrompt({ request, pipelineText, rows }) {
  return [
...
async function generate(agentApi, chatId, content, signal) {
  const acc = newAcc();
  await agentApi.streamMessage(chatId, content, { signal, onEvent: (ev) => foldEvents(acc, [ev]) });
...
async function semanticVerify(agentApi, chatId, ctx, signal) {
  const acc = newAcc();
  await agentApi.streamMessage(chatId, buildVerifyPrompt(ctx), { signal, onEvent: (ev) => foldEvents(acc, [ev]) });
  const raw = replyText(acc);
...
async function verify(api, collection, arr, signal) {
...
async function runLoop({ api, agentApi, chatId, request, collection, fields, samples, currentPipeline, hints, transcript, onPhase, signal }) {
  ...
  let refined = false;
  const done = (rowCount) => ({ pipelineText, note: { kind: refined ? 'refined' : 'verified', rowCount } });
  const giveUp = (v) => (v.kind === 'mismatch'
    ? done(v.rowCount)
...
export async function runAgentQuery({ api, agentApi, request, collection, fields = [], samples = null,
  currentPipeline = '', hints = {}, onPhase = () => {}, signal }) {
  const transcript = [];
...
export async function continueAgentQuery({ api, agentApi, chatId, request, collection, fields = [], samples = null,
  currentPipeline = '', hints = {}, transcript = [], onPhase = () => {}, signal }) {
  const t = [...transcript];
