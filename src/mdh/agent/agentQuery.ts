...
export function schemaHintParts({ knownValues = null, topValues = null, ranges = null,
  numericStringFields = null, searchIndexes = null, fieldTypes = null, arrayPaths = null }: SchemaHints = {}) {
  const parts: string[] = [];
...
export function buildGenPrompt({ request, collection, fields = [], samples = null, currentPipeline = '',
  knownValues = null, topValues = null, ranges = null, numericStringFields = null, searchIndexes = null, fieldTypes = null, arrayPaths = null }: {
  ...
  if (Array.isArray(samples) && samples.length) {
    parts.push(`Sample documents (how values are actually stored):\n${JSON.stringify(samples.slice(0, SAMPLE_DOCS))}`);
  }
  parts.push(...schemaHintParts({ knownValues, topValues, ranges, numericStringFields, searchIndexes, fieldTypes, arrayPaths }));
  parts.push(`Current pipeline:\n${currentPipeline && currentPipeline.trim() ? currentPipeline.trim() : '[]'}`);
  parts.push('Respond with ONLY the pipeline: a single valid JSON array of stages. No prose, no explanation, no markdown, no code fences, no text before or after.');
...
export function buildFixPrompt(
  { verdict, samples = null }:
  { verdict: { kind: string; error?: string; issue?: string }; samples?: any[] | null },
) {
  const parts: string[] = [];
  if (verdict.kind === 'error') parts.push(`That pipeline failed with error: ${verdict.error || 'unknown error'}.`);
  else if (verdict.kind === 'mismatch') parts.push(`That pipeline ran but its results do not correctly answer the request${verdict.issue ? `: ${verdict.issue}` : ''}.`);
  else parts.push('That pipeline returned 0 matching documents — likely a value-format or field mismatch.');
  if (Array.isArray(samples) && samples.length) {
    parts.push(`Sample documents (how values are actually stored):\n${JSON.stringify(samples.slice(0, SAMPLE_DOCS))}`);
  }
  parts.push('Fix it so it correctly answers the original request. Stay READ-ONLY (no $out/$merge/writes), do not call tools, and respond with ONLY the JSON array — no prose or markdown.');
  return parts.join('\n\n');
}

// Semantic verifier prompt: judge whether the ACTUAL result rows answer the request.
export function buildVerifyPrompt(
  { request, pipelineText, rows }: { request: string; pipelineText: string; rows?: any[] | null },
) {
  return [
...
async function generate(agentApi: any, chatId: string, content: string, signal?: AbortSignal) {
  const acc = newAcc();
  await agentApi.streamMessage(chatId, content, { signal, onEvent: (ev: unknown) => foldEvents(acc, [ev]) });
...
async function semanticVerify(agentApi: any, chatId: string, ctx: any, signal?: AbortSignal) {
  const acc = newAcc();
  await agentApi.streamMessage(chatId, buildVerifyPrompt(ctx), { signal, onEvent: (ev: unknown) => foldEvents(acc, [ev]) });
  const raw = replyText(acc);
...
async function verify(api: any, collection: string | null | undefined, arr: any[], signal?: AbortSignal): Promise<Verdict> {
  if (hasWriteStage(arr)) return { kind: 'error', error: 'write stages ($out/$merge) are not permitted; produce a read-only pipeline' };
  if (!collection) return { kind: 'unrun' };
  try {
    const res = await api.aggregate(collection, arr, { signal });
    const rows = res?.result || [];
    return rows.length ? { kind: 'ok', rowCount: rows.length, rows: rows.slice(0, SAMPLE_DOCS) } : { kind: 'empty' };
  } catch (e) {
...
function verdictLabel(v: Verdict) {
  switch (v.kind) {
    case 'ok': return `Ran the query — ${v.rowCount} row${v.rowCount === 1 ? '' : 's'}`;
    case 'empty': return 'Ran the query — 0 matching rows';
    case 'error': return `Execution error: ${v.error || 'unknown'}`;
    case 'unrun': return 'Applied without running (no collection selected)';
    default: return v.kind;
  }
}
function blockLabel(note: { kind: string }) {
  switch (note.kind) {
    case 'no-pipeline': return 'The agent did not return a pipeline.';
    case 'declined': return 'The agent declined — no read-only query is possible for that request.';
    case 'blocked': return 'Blocked: that pipeline would modify data (write stage).';
    default: return note.kind;
  }
}
function refineReason(v: Verdict) {
  if (v.kind === 'error') return 'Refine: fix the execution error.';
  if (v.kind === 'mismatch') return `Refine: the result doesn't fully answer the request${v.issue ? ` — ${v.issue}` : ''}.`;
...
async function runLoop({ api, agentApi, chatId, request, collection, fields, samples, currentPipeline, hints, transcript, onPhase, signal }: LoopArgs) {
  onPhase('generate');
  ...
  if (first.block) { transcript.push({ role: 'system', text: blockLabel(first.block.note) }); return first.block; }
  let { pipelineText, arr } = first;

  let refined = false;
  const done = (rowCount?: number) => ({ pipelineText, note: { kind: refined ? 'refined' : 'verified', rowCount } });
  const giveUp = (v: Verdict) => (v.kind === 'mismatch'
    ? done(v.rowCount)
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    if (mech.kind === 'ok') {
      ...
      transcript.push({ role: 'system', text: sem.ok ? 'Reviewed — the result answers the request ✓' : `Reviewed — needs refinement: ${sem.issue || 'does not fully answer the request'}` });
...
export async function runAgentQuery({ api, agentApi, request, collection, fields = [], samples = null,
  currentPipeline = '', hints = {}, onPhase = () => {}, signal }: Omit<LoopArgs, 'chatId' | 'transcript' | 'onPhase'> & { onPhase?: (phase: string) => void }) {
  ...
  const res = await runLoop({ api, agentApi, chatId, request, collection, fields, samples, currentPipeline, hints, transcript, onPhase, signal });
...
export async function continueAgentQuery({ api, agentApi, chatId, request, collection, fields = [], samples = null,
  currentPipeline = '', hints = {}, transcript = [], onPhase = () => {}, signal }: Omit<LoopArgs, 'transcript' | 'onPhase'> & { transcript?: Turn[]; onPhase?: (phase: string) => void }) {
  const t = [...transcript];
  const res = await runLoop({ api, agentApi, chatId, request, collection, fields, samples, currentPipeline, hints, transcript: t, onPhase, signal });
