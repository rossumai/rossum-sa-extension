...
export function outcomeFor(request: any, note: any) {
  const rows = (n: any) => `${n} row${n === 1 ? '' : 's'}`;
  switch (note?.kind) {
    case 'verified':
    case 'refined':
...
function PhaseTracker({ phase, hadRefine }: { phase?: string | null; hadRefine?: boolean }) {
...
const roleLabel = (r: any) => (r === 'user' ? 'You' : r === 'assistant' ? 'Mr. Fabry' : 'Run');

// One transcript turn — assistant shows reasoning (if any) + full reply in a <pre>.
function Turn({ t }: { t: any }) {
  return (
...
function showTranscript(session: any, editorRef: any, onUpdate: any) {
...
export default function AgentBox({ editorRef }: { editorRef?: { current: JsonEditorHandle | null } }) {
  ...
  // Finished-run result line (see outcomeFor).
  const [outcome, setOutcome] = useState<any>(null);
  // { chatId, transcript, ctx }
  ...
  async function submit(value: any) {
    ...
    try {
      ...
      const hints = await getSchemaHints(api, col as string, records.value).catch(() => ({}));
