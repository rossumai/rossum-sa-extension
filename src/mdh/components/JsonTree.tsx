...
export function countFields(val: any) {
...
function writeClipboard(text: any) {
  try {
...
export function CopyButton({ getText, kind = 'value' }: { getText: () => string; kind?: string }) {
...
// The sort/filter plumbing is threaded down every row, so it is named once.
...
function JsonTreeRow({ fieldKey, value, fullPath, depth, collapseDepth, sortState, filterState, onSort, onFilter, readOnly }: {
  ...
  const [flash, setFlash] = useState<'key' | 'value' | null>(null);

  function triggerFlash(target: any) {
    setFlash(target);
