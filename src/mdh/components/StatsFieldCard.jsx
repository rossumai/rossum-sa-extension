...
function filterToRecords(field, value, isPlaceholder) {
...
function TypeChip({ profile }) {
  if (profile.isMixed) {
    const label = profile.types.slice(0, 2).map((t) => friendlyType(t.type)).join(' + ')
...
function TopValues({ profile, limit }) {
  const isDate = !!profile.date;
  const base = (profile.topValues || []).slice(0, limit);
  const tokens = (profile.sentinel && profile.sentinel.values) || [];
  const tokenSet = new Set(tokens.map((s) => s.value));
  const norm = (v) => (typeof v === 'string' ? v.trim().toLowerCase() : null);
  const shownTokens = new Set(base.map((v) => norm(v.value)).filter((t) => t && tokenSet.has(t)));
  const pinned = tokens.filter((s) => !shownTokens.has(s.value)).map((s) => ({ value: s.value, count: s.count, placeholder: true }));
  const rows = [...base.map((v) => ({ ...v, placeholder: tokenSet.has(norm(v.value)) })), ...pinned];
...
export default function StatsFieldCard({ profile, indexedBy, dimmed }) {
        ...
        <div class="stats-msgs">
          {hasSentinel && (
            <div class="stats-msg is-error">
              <b>Placeholder</b> {profile.sentinel.values.slice(0, 2).map((s) => `"${s.value}" ×${s.count.toLocaleString()}`).join(' · ')}
