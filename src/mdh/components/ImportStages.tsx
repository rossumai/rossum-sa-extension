...
export function ImportProgress({ progress, onCancel }: { progress: any; onCancel?: () => void }) {
  ...
  const LABELS: Record<string, string> = { analyze: 'Analyzing', insert: 'Inserting', update: 'Updating', replace: 'Replacing', delete: 'Deleting', uploading: 'Uploading', processing: 'Processing on the server' };
...
export function ImportSummary(
  { result, fileMeta, onClose }: { result: any; fileMeta?: any; onClose: () => void },
) {
      ...
      return (
        <Fragment>
          <div class="import-result-header partial">
            ...
            <span>Cancelled{fileMeta?.name && <span class="import-result-filename"> {'·'} {fileMeta.name}</span>}</span>
          </div>
  ...
  return (
    <Fragment>
      <div class={`import-result-header ${ok ? 'success' : 'partial'}`}>
        ...
        <span>{cancelled ? 'Cancelled' : ok ? 'Import complete' : 'Import partially complete'}
      ...
      <ul class="import-result-list">
        ...
        {appliedVerb && applied > 0 && <li>{appliedVerb} <strong>{applied.toLocaleString()}</strong> record{applied === 1 ? '' : 's'}</li>}
        {inserted > 0 && <li>Inserted <strong>{inserted.toLocaleString()}</strong> document{inserted === 1 ? '' : 's'}</li>}
        {skipped > 0 && <li>Skipped <strong>{skipped.toLocaleString()}</strong> non-matching row{skipped === 1 ? '' : 's'}</li>}
        {failedBatches.length > 0 && (
          <li style="color:var(--danger)"><strong>{failedBatches.length}</strong> operation{failedBatches.length === 1 ? '' : 's'} failed
            <ul class="import-failure-list">
              {failedBatches.slice(0, 5).map((b: any, i: any) => (
                <li key={i}>
                  {b.count > 0 && typeof b.startIdx === 'number'
                    ? <Fragment>Rows {b.startIdx.toLocaleString()}{b.endIdx !== b.startIdx ? `${'–'}${b.endIdx.toLocaleString()}` : ''} ({b.count.toLocaleString()}): </Fragment>
                    : null}
