...
function sortProfiles(profiles, sort) {
  ...
  else arr.sort((a, b) => (isFlagged(b) - isFlagged(a)) || (b.diversityPct - a.diversityPct)); // issues first
  return arr;
}

export default function StatsFieldGrid({ profiles, indexMap }) {
  const [sort, setSort] = useState('issues');
  const [filter, setFilter] = useState('');
  const f = filter.trim().toLowerCase();
  const sorted = sortProfiles(profiles, sort);
  const matches = (p) => p.field.toLowerCase().includes(f);
  ...
  return (
    <div>
      <div class="stats-fgrid-head">
        ...
        <input class="stats-fgrid-filter" placeholder={'filter fields…'} value={filter} onInput={(e) => setFilter(e.target.value)} />
        <span class="view-seg">
          {SORTS.map(([k, label]) => (
