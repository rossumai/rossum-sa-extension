...
const timeCls = (ms: any) =>
  'pipeline-inspect-time' + (ms > SLOW_QUERY_MS ? ' pipeline-inspect-time-slow' : '');
...
function StageToggle({
  ...
  return (
    <input
      ...
      onClick={(e) => {
        e.stopPropagation();
        if (onToggle) onToggle(entryIndex);
      }}
    />
  );
}
...
function StageHeader({
  ...
  if (typeof count === 'number') {
    countText =
      typeof prevCount === 'number' && prevCount !== count
  ...
  if (info.error) {
    return (
      <div class="pipeline-inspect-error">
        {info.error.status ? `HTTP ${info.error.status}: ` : ''}
        {info.error.message}
      </div>
    );
  }
  if (!info.docs || info.docs.length === 0) {
    return (
      <div class="pipeline-inspect-empty">
        <span class="pipeline-inspect-empty-icon" aria-hidden="true">
          {'\u26A0'}
        </span>
...
export default function StagesView({
  ...
  const { counts, inputInfo } = useStageCounts(collection, activeStages) as {
  ...
  useEffect(() => {
    ...
    activeStages.forEach((_, i) => {
      ...
      api
        .aggregate(collection, [...stripWriteStages(prefix), { $limit: sampleSize }], {
          signal: controller.signal,
        })
        .then((res) => {
          if (!controller.signal.aborted)
            setPreviews((p) => ({ ...p, [i]: { docs: res.result || [] } }));
        })
        .catch((err) => {
          if (err?.name === 'AbortError' || controller.signal.aborted) return;
          setPreviews((p) => ({
            ...p,
            [i]: { error: { message: err?.message || String(err), status: err?.status } },
          }));
        });
    });
  ...
  useEffect(() => {
    ...
    const controller = new AbortController();
    api
      .aggregate(collection, [{ $limit: sampleSize }], { signal: controller.signal })
      .then((res) => {
        if (!controller.signal.aborted)
          setPreviews((p) => ({ ...p, input: { docs: res.result || [] } }));
      })
      .catch((err) => {
        if (err?.name === 'AbortError' || controller.signal.aborted) return;
        setPreviews((p) => ({
          ...p,
          input: { error: { message: err?.message || String(err), status: err?.status } },
        }));
      });
    return () => controller.abort();
  ...
  const sectionCls = (idx: any) =>
    'pipeline-inspect-section' + (highlightIdx === idx ? ' pipeline-inspect-highlight' : '');
  ...
  const stageCountLabel =
    list.length === activeStages.length
    ...
    <div class="pipeline-inspect" ref={rootRef}>
      <div class="pipeline-inspect-opts">
        ...
        {/* One direction only, since 2026-08-14: hovering a stage here scrolls the
            ...
            StageLinkOverlay. */}
        <label
          class="pipeline-inspect-opt pipeline-inspect-autoscroll"
          title="Scroll the pipeline editor to the stage you hover here"
        >
          <input
            type="checkbox"
            checked={autoscroll}
            onChange={(e) => {
              stagesAutoscroll.value = e.currentTarget.checked;
            }}
          />
        ...
        <label
          class="pipeline-inspect-opt pipeline-inspect-autoscroll"
          title="Show each stage's query with variables substituted (as sent to the Data Storage API)"
        >
          <input
            type="checkbox"
            checked={showDef}
            onChange={(e) => {
              stagesShowDef.value = e.currentTarget.checked;
            }}
          />
      ...
      <div class="pipeline-inspect-scroll">
        ...
        <section class={sectionCls(-1) + ' pipeline-inspect-source'} data-idx="-1">
          <button
            ...
            onClick={() => {
              stagesSourceOpen.value = !sourceOpen;
            }}
            title={
              sourceOpen
                ? 'Hide the sample records'
                : 'Show what the collection looks like before the pipeline runs'
            }
          >
            <span class="pipeline-inspect-source-chev" aria-hidden="true">
              {sourceOpen ? '▾' : '▸'}
            </span>
            ...
            <span
              class={
                'pipeline-inspect-count' + (inputInfo?.count === 0 ? ' pipeline-inspect-zero' : '')
              }
            >
              {typeof inputInfo?.count === 'number'
                ? `${inputInfo.count.toLocaleString()} docs`
                : '…'}
            </span>
            {typeof inputInfo?.ms === 'number' && (
              <span class={timeCls(inputInfo.ms)}>{inputInfo.ms}ms</span>
            )}
          </button>
          {sourceOpen && (
            <div class="pipeline-inspect-body">
              <div class="pipeline-inspect-output">
                <StageOutput info={previews.input} />
              </div>
            </div>
          )}
        </section>
        ...
        {list.map((entry, entryIndex) => {
          ...
          if (entry.disabled) {
            return (
              <section
                class="pipeline-inspect-section pipeline-inspect-disabled"
                key={entryIndex}
                data-entry={entryIndex}
                onMouseEnter={(e) => {
                  hoveredStage.value = { entryIndex, el: e.currentTarget };
                }}
                onMouseLeave={() => {
          ...
          return (
            ...
            >
              <StageHeader
                toggle={
                  <StageToggle entryIndex={entryIndex} disabled={false} onToggle={onToggleStage} />
                }
                num={`${myIdx + 1}`}
                label={stageKey}
                prevCount={prevCount}
                count={counts[myIdx]?.count}
                ms={counts[myIdx]?.ms}
              />
              {showDef && (
              ...
              <div
                class={
                  'pipeline-inspect-body' + (isEmptyStage ? ' pipeline-inspect-body-empty' : '')
                }
              >
                <div class="pipeline-inspect-output">
