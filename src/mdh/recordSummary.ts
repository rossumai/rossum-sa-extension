...
function indexedTopLevelPaths(indexes: any[]): Set<string> {
...
function classifyTier(key: string, value: unknown, indexedPaths: Set<string>): number {
  if (key === '_id') return 5;
...
function oidString(value: any): string | null {
...
function formatIdFallback(value: any): string {
...
function narrowFirstEntry(key: string, value: unknown, charBudget: number): string {
