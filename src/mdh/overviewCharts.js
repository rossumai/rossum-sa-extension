...
function toHex(c) { const s = c.toString(16); return s.length < 2 ? `0${s}` : s; }

// ratio (0..∞) → [r,g,b], clamped at 1.0 (100%+ index overhead = the top stop).
function overheadRgb(ratio) {
...
export function overheadColor(ratio) {
...
export function scaleArea(bytes, mode) {
...
export function treemapItems(rows, topN = TOP_N) {
  ...
  if (tail.length) {
    const storageSize = tail.reduce((s, r) => s + r.storageSize, 0);
    const totalIndexSize = tail.reduce((s, r) => s + (r.totalIndexSize || 0), 0);
    items.push({
...
export function buildTreemap(rows, { width, height, topN = TOP_N, mode = 'linear' } = {}) {
...
function niceLinearTicks(max, count = 4) {
  if (!(max > 0)) return [0];
  const raw = max / count;
...
function axisScale(values, mode, pxLo, pxHi) {
  ...
  if (mode === 'log') {
    ...
    if (hi === lo) hi = lo + 1;
    const pos = (v) => pxLo + ((Math.log10(v) - lo) / (hi - lo)) * (pxHi - pxLo);
    const ticks = [];
    for (let k = lo; k <= hi; k++) ticks.push({ v: 10 ** k, p: pos(10 ** k) });
    return { pos, ticks };
  }
  const tf = mode === 'sqrt' ? Math.sqrt : (x) => x;
  const domMax = tf(max) || 1;
  const pos = (v) => pxLo + (tf(v) / domMax) * (pxHi - pxLo);
...
export function buildScatter(rows, { width, height, mode = 'log' } = {}) {
