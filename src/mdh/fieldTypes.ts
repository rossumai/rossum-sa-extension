...
export function foldBsonType(t?: string | null): string {
  if (t === 'string') return 'string';
  if (t === 'bool') return 'boolean';
  if (t === 'null') return 'null';
  if (NUMBER_BSON.has(t as string)) return 'number';
  return 'other';
}
...
export async function resolveFieldTypes(collectionName: string, fields: string[]) {
  if (!collectionName || !fields || fields.length === 0) return {};
  const cached = { ...(cache.get(collectionName, FIELD_TYPES_KEY) || {}) };
  let missing = fields.filter((f: string) => !(f in cached));
