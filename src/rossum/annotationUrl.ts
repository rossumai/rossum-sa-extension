...
// with a slash.
function pathnameOf(urlOrPath: unknown): string {
...
export function annotationIdFromPath(urlOrPath: unknown): string | null {
  const match = PATH_RE.exec(pathnameOf(urlOrPath));
  return match ? match[1] : null;
}

export function annotationIdFromInput(raw: unknown): string | null {
