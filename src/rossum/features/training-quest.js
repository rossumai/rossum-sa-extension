...
function watchProgress(deps) {
  ...
  chrome.storage.onChanged?.addListener((changes, area) => {
    ...
    onExternalProgress?.(record ? migrate(TRACK, record) : null);
  });
}
...
async function start(deps) {
  ...
  let lastApiAt = 0;
  let onFocus = null; // this init() call's own focus handler, so stop() removes exactly it
  ...
  const markStepAndTrack = async (missionId, stepId, state) => {
