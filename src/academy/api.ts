...
/** The request shape the content-script fetcher also takes — see the note above. */
export type AcademyRequest = {
  method?: string;
  body?: unknown;
  /** `bearer` only for Data Storage; everything under /api/v1/ uses `token`. */
  auth?: 'token' | 'bearer';
};

// `T` is what the caller expects back. The wire is untyped JSON, so it defaults to `any`
// rather than forcing every JS call site to narrow.
export function fetchAcademyApi<T = any>(
  path: string,
  { method = 'GET', body, auth = 'token' }: AcademyRequest = {},
): Promise<T> {
