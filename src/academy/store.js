...
export const mintNote = signal(null);
...
export async function startTrack(now = Date.now) {
  try {
    ...
    error.value = `Couldn't start the training track: ${e?.message || e}`;
  }
}
...
export async function attestStep(missionId, stepId, now = Date.now) {
  ...
  try {
    ...
    error.value = `Couldn't save that step: ${e?.message || e}`;
  }
}
...
export async function restartTrack() {
  try {
    ...
    error.value = `Couldn't restart the training track: ${e?.message || e}`;
  }
}
