...
const fmt = (n: unknown) => (typeof n === 'number' ? (Math.round(n * 100) / 100).toString() : String(n ?? '?'));
...
export function fieldThresholds(schema: any, queue: any): { bySchemaId: Record<string, number>; defaultThreshold: number | null } {
...
export function computeVerdict(
  { annotation, blocker, content, queue, schema, thr }: Record<string, any>,
) {
  const a = annotation || {};
  const reasons: any[] = [];
...
export function buildEvidence(input: any) {
  ...
  for (const [schemaId, group] of byField) {
    ...
    const configs = group.some((p: any) => p.primary === 'data_matching') ? matchConfigsForField(schemaId, hooks) : [];
...
export function arrivalLabel(attachmentStatus?: string | null): string {
  ...
  return Object.prototype.hasOwnProperty.call(ARRIVAL_LABEL, key as string) ? ARRIVAL_LABEL[key as keyof typeof ARRIVAL_LABEL] : String(attachmentStatus);
}

export function intakeEvidence(
  { annotation, document: doc, parentDocument, relations = [], email }: Record<string, any>,
): any[] {
  const items: any[] = [];
  const a = annotation || {};
  if (doc) {
    const key = doc.attachment_status ?? null;
    const how = Object.prototype.hasOwnProperty.call(ARRIVAL, key) ? ARRIVAL[key as keyof typeof ARRIVAL] : `arrived (attachment_status: "${doc.attachment_status}")`;
...
export function workflowEvidence(
  { workflowRuns = [], workflowSteps = [], enrichment = {} }:
    { workflowRuns?: any[]; workflowSteps?: any[]; enrichment?: Record<string, any> },
): any[] {
  const items: any[] = [];
