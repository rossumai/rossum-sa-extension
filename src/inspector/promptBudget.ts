...
export function budgetedJoin(
  ...
  for (const m of middle) {
    if (used + m.length + sep.length > max - NOTE_RESERVE) {
      omitted++;
      continue;
    }
  ...
  if (omitted)
    kept.push(
      `(… ${omitted} more candidate item(s) omitted to stay within the length limit — fetch them with your tools if needed.)`,
    );
