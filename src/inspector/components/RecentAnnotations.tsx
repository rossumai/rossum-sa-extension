  ...
  if (!list || list.length === 0) {
    return (
      <div class="inspector-recents-empty">
        Nothing viewed yet. Annotations you open in the Rossum UI will appear here {'—'} or paste an id above.
      </div>
    );
  }
  const now = Date.now();
      ...
      <table class="inspector-rectable">
        <thead>
          <tr>
            ...
            <th>Status</th>
            <th>When</th>
            <th class="inspector-rectable-idh">Id <button type="button" class="inspector-recents-clear" onClick={clearRecents}>Clear all</button></th>
          </tr>
        </thead>
        <tbody>
          {list.map((r) => (
            <tr class="inspector-recent" title={`Inspect #${r.id}`} onClick={() => onSelect(String(r.id))}>
              <td class="inspector-recent-name">{r.fileName || `#${r.id}`}</td>
              <td>{r.queue ? <span class="inspector-chip inspector-recent-queue">{r.queue}</span> : null}</td>
              <td>{r.status ? <span class={`inspector-pill inspector-recent-status inspector-pill-${r.status}`}>{String(r.status).replace(/_/g, ' ')}</span> : null}</td>
