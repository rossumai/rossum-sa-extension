...
export default function ExportPanel() {
  ...
  let culprit; let culpritBadge = null;
  if (failing) {
    culprit = <Fragment>failed in extension <b>{failing.hookName}</b></Fragment>;
  } else if (candidates.length === 1) {
    culprit = <Fragment>likely extension <b>{candidates[0].hookName}</b> (the only export extension on this queue)</Fragment>;
  } else if (candidates.length > 1) {
    culprit = <Fragment>one of {candidates.length} export extensions: {candidates.map((c) => c.hookName).join(', ')} — the failing one is not in the logs</Fragment>;
  } else {
