...
export function viewedRows(stored, origin, max = MAX_RECENTS) {
...
export function enrichRows(rows, payload) {
  const anns = new Map((payload?.results || []).map((a) => [String(a.id), a]));
  const docs = new Map((payload?.documents || []).map((d) => [d.url, d]));
  const queues = new Map((payload?.queues || []).map((q) => [q.url, q]));
  return (rows || []).map((r) => {
...
export async function enrichRecents(api) {
