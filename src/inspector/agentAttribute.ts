...
export function buildAttributionPrompt(
  { kind, annotation = {}, target = {}, candidates = [] }:
  { kind: string; annotation?: any; target?: any; candidates?: any[] },
) {
  const head = [
...
export async function runAttribution(
  { agentApi, kind, context, onPhase = () => {}, signal }:
  { agentApi: any; kind: string; context: any; onPhase?: PhaseCb; signal?: AbortSignal },
) {
  onPhase('thinking');
  ...
  await agentApi.streamMessage(chatId, buildAttributionPrompt({ kind, ...context }), {
    ...
    onEvent: (ev: unknown) => { foldEvents(acc, [ev]); if (acc.status && acc.status !== lastStatus) { lastStatus = acc.status; onPhase(acc.status); } },
  });
...
export function parseFieldBatch(text: unknown) {
  ...
  try {
    ...
    return { fields: arr.filter((f: any) => f && f.schema_id).map((f: any) => ({ schema_id: String(f.schema_id), ...normalizeVerdictObject(f) })) };
  } catch { return { fields: [] }; }
}

export async function runFieldBatchAttribution(
  { agentApi, items, context, onPhase = () => {}, signal }:
  { agentApi: any; items: { key: string; schemaId: string; value?: unknown }[]; context: any; onPhase?: PhaseCb; signal?: AbortSignal },
) {
  onPhase('thinking');
  const chatId = await agentApi.createChat();
  await agentApi.streamMessage(chatId, '/persona cautious', { onEvent: () => {}, signal });
