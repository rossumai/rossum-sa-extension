...
export function buildAttributionPrompt({
  ...
  if (kind === 'label') {
    head.push(
      `Question: which extension applied the label "${target.name}" (id ${target.id}) to this annotation, and why?`,
    );
  } else if (kind === 'message') {
    head.push(`Question: which extension produced this ${target.level || ''} message, and why?`);
    head.push(
      `Message text: ${JSON.stringify(target.content || '')}${target.schemaId ? ` (on field ${target.schemaId})` : ''}.`,
    );
  } else if (kind === 'blocker') {
    head.push(
      `Question: explain this automation blocker — type "${target.type}"${target.schemaId ? ` on field ${target.schemaId}` : ''}. What does it mean, and what most likely caused it? If a specific extension is responsible, name it; otherwise use kind "unknown".`,
    );
  } else if (kind === 'export') {
    head.push(
      `Question: which export extension failed for this annotation, and why?${target.error ? ` Recorded error: ${JSON.stringify(target.error)}.` : ''} Explain the failure in plain language.`,
    );
  } else {
    head.push(
      `Question: which extension rejected this annotation${target.rejectedAt ? ` (rejected at ${target.rejectedAt})` : ''}, and why?`,
    );
...
export function parseAttribution(text: unknown) {
  ...
  try {
    ...
    const c = o.culprit;
    const culprit =
      c && c.kind && c.kind !== 'unknown' && c.kind !== 'manual'
        ? { kind: String(c.kind), id: c.id == null ? null : c.id, name: String(c.name || '') }
        : c && c.kind === 'manual'
          ? { kind: 'manual', id: null, name: c.name ? String(c.name) : 'manual' }
          : null;
...
// Named once: a bare `() => {}` default is otherwise inferred as taking no arguments, and every
// caller passes a phase.
...
export function buildFieldBatchPrompt(
  fields: { schemaId: string; value?: unknown }[] = [],
  { annotation = {}, candidates = [] }: { annotation?: any; candidates?: any[] } = {},
) {
  const head = [
    'You are investigating a single Rossum annotation in a READ-ONLY forensic tool. Never modify anything — only read and reason.',
    TOOL_INSTRUCTION,
  ...
  const middle = candidates.map(candidateLine);
  const tail = [
    'Respond with ONLY this JSON object and nothing else: {"fields":[{"schema_id":"<id>","culprit":{"kind":"hook|webhook|rule|connector|manual|unknown","id":<number|null>,"name":"<name>"},"confidence":"high|medium|low","explanation":"<one short sentence>"}]}',
  ];
  return budgetedJoin(head, middle, tail);
}

function normalizeVerdictObject(o: any) {
  const c = o && o.culprit;
  const culprit =
    c && c.kind && c.kind !== 'unknown'
      ? { kind: String(c.kind), id: c.id == null ? null : c.id, name: String(c.name || '') }
      : null;
  ...
  return {
    culprit,
    confidence,
    explanation: typeof (o && o.explanation) === 'string' ? o.explanation : '',
  };
}

export function parseFieldBatch(text: unknown) {
  ...
  try {
    ...
    return {
      fields: arr
        .filter((f: any) => f && f.schema_id)
        .map((f: any) => ({ schema_id: String(f.schema_id), ...normalizeVerdictObject(f) })),
    };
  } catch {
    return { fields: [] };
  }
...
export async function runFieldBatchAttribution({
  agentApi,
  items,
  context,
  onPhase = () => {},
  ...
  agentApi: any;
  items: { key: string; schemaId: string; value?: unknown }[];
  context: any;
  onPhase?: PhaseCb;
  signal?: AbortSignal;
}) {
