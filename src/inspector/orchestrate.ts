...
export async function orchestrateAttributions(
  { store: s = store, api, agentApi, signal }:
  ...
  const setDone = (key: string, verdict: any, reliability: any) => { if (!aborted()) s.setAttribution(key, { status: 'done', verdict, reliability, source: 'programmatic' }); };
