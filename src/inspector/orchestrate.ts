...
import {
  ...
  gatherBlockerContext,
  gatherExportContext,
  gatherFieldsContext,
  gatherRejectContext,
  gatherLabelContext,
} from './attributionContext.js';

export const messageKey = (i: number) => `message:${i}`;
export const blockerKey = (i: number) => `blocker:${i}`;
export const fieldKey = (schemaId: string) => `field:${schemaId}`;
export const labelKey = (id: unknown) => `label:${id}`;
const STD_BLOCKERS = new Set(['low_score', 'automation_disabled', 'error_message']);

function walkDatapoints(nodes: any[] | null | undefined, out: any[]): any[] {
...
// One thing on the annotation that needs a cause: `key` is the stable citation id, `payload`
// carries whatever that kind's attribution needs.
type Finding = { key: string; kind: string; payload: any };

export function computeFindings(s: any): Finding[] {
    ...
    if (!m.culprit)
      findings.push({
        ...
        payload: {
          ...
          level: m.level,
          content: m.content,
          schemaId: m.datapointId ? null : null,
  ...
  (d.blocker?.content || []).forEach((raw: any, i: number) => {
    const b = explainBlocker(raw, { queue: d.resolved?.queue });
    if (!STD_BLOCKERS.has(b.type) && !b.culprit)
      findings.push({
        key: blockerKey(i),
        kind: 'blocker',
        payload: { index: i, type: b.type, schemaId: b.schemaId },
      });
  });
  ...
  if (failed) {
    ...
    if (!failing && candidates.length > 1)
      findings.push({ key: 'export', kind: 'export', payload: { error: null } });
  }
  ...
  for (const dp of walkDatapoints(d.content?.content || [], [])) {
    ...
    const ambiguous =
      p.primary === 'rules' ||
      p.primary === 'connector' ||
      (p.primary === 'data_matching' && matchConfigsForField(p.schemaId, hooks).length === 0);
    if (ambiguous)
      findings.push({
        key: fieldKey(p.schemaId),
        kind: 'field',
        payload: { schemaId: p.schemaId, value: p.value, primary: p.primary },
      });
  }
  ...
  const rej = classifyRejection({
    annotation: a,
    workflowActivities: s.enrichment.value.workflow || [],
    notes: s.enrichment.value.notes || [],
    usersById: d.resolved?.usersById || {},
  });
  if (rej.type === 'hook')
    findings.push({ key: 'reject', kind: 'reject', payload: { reason: rej.reason?.text || null } });
  if (d.resolved?.labelsById !== undefined) {
    const { applied } = labelAttribution({
      annotation: a,
    ...
    for (const l of applied)
      if (!l.rule)
        findings.push({ key: labelKey(l.id), kind: 'label', payload: { id: l.id, name: l.name } });
  }
  return findings;
}
...
export async function orchestrateAttributions({
  store: s = store,
  api,
  agentApi,
  signal,
}: { store?: any; api?: any; agentApi?: any; signal?: AbortSignal } = {}) {
  const d = s.data.value;
  if (!d) return;
  ...
  const setDone = (key: string, verdict: any, reliability: any) => {
    if (!aborted())
      s.setAttribution(key, { status: 'done', verdict, reliability, source: 'programmatic' });
  };
  const ai: { key: string; run: (onPhase: (phase: string) => void) => Promise<any> }[] = [];
  const fieldItems: { key: string; schemaId: string; value: unknown }[] = [];
  ...
  for (const f of findings) {
    ...
    if (f.kind === 'message') {
      ...
      if (c) {
        setDone(f.key, { culprit: c.culprit, confidence: null, explanation: '' }, c.reliability);
        continue;
      }
      ai.push({
        key: f.key,
        run: (onPhase: (phase: string) => void) =>
          gatherMessageContext({ api, store: s, message: f.payload }).then((context) =>
            runAttribution({ agentApi, kind: 'message', context, onPhase, signal }),
          ),
      });
      ...
      ai.push({
        key: f.key,
        run: (onPhase: (phase: string) => void) =>
          gatherBlockerContext({ api, store: s, blocker: f.payload }).then((context) =>
            runAttribution({ agentApi, kind: 'blocker', context, onPhase, signal }),
          ),
      });
      ...
      ai.push({
        key: f.key,
        run: (onPhase: (phase: string) => void) =>
          gatherExportContext({ api, store: s, error: f.payload.error }).then((context) =>
            runAttribution({ agentApi, kind: 'export', context, onPhase, signal }),
          ),
      });
      ...
      ai.push({
        key: f.key,
        run: (onPhase: (phase: string) => void) =>
          gatherRejectContext({ api, store: s, reason: f.payload.reason }).then((context) =>
            runAttribution({ agentApi, kind: 'reject', context, onPhase, signal }),
          ),
      });
      ...
      ai.push({
        ...
        run: (onPhase: (phase: string) => void) =>
          gatherLabelContext({
            api,
            store: s,
            labelId: f.payload.id,
            labelName: f.payload.name,
          }).then((context) =>
            runAttribution({ agentApi, kind: 'label', context, onPhase, signal }),
          ),
      });
    }
  }
  ...
  for (const item of ai) {
    ...
    const onPhase = (phase: string) => {
      if (aborted()) return;
      const cur = s.attributions.value[item.key];
      if (cur && cur.status === 'loading' && cur.phase !== phase)
        s.setAttribution(item.key, { status: 'loading', phase, source: 'ai' });
    };
    pending.push(
      item
        .run(onPhase)
        .then(({ verdict }) => {
          if (!aborted()) s.setAttribution(item.key, { status: 'done', verdict, source: 'ai' });
        })
        .catch((e) => {
          if (!aborted() && e?.name !== 'AbortError')
            s.setAttribution(item.key, {
              status: 'error',
  ...
  if (fieldItems.length) {
    for (const it of fieldItems)
      s.setAttribution(it.key, { status: 'loading', phase: 'thinking', source: 'ai' });
    const onPhase = (phase: string) => {
      if (aborted()) return;
      for (const it of fieldItems) {
        const cur = s.attributions.value[it.key];
        if (cur && cur.status === 'loading' && cur.phase !== phase)
          s.setAttribution(it.key, { status: 'loading', phase, source: 'ai' });
      }
    };
    pending.push(
      gatherFieldsContext({ api, store: s })
        .then((context) =>
          runFieldBatchAttribution({ agentApi, items: fieldItems, context, onPhase, signal }),
        )
        .then(({ verdicts }) => {
          if (aborted()) return;
          ...
          for (const it of fieldItems) {
            ...
            s.setAttribution(it.key, {
              status: 'done',
              verdict: v
                ? { culprit: v.culprit, confidence: v.confidence, explanation: v.explanation }
                : { culprit: null, confidence: 'low', explanation: '' },
              source: 'ai',
            });
          }
        })
