...
let attrController: AbortController | null = null;
...
export async function loadAnnotation(id: any) {
  const myId = ++loadId;
  if (attrController) attrController.abort();
...
async function fetchPreviewBatch(pageResources: any, myId: any) {
...
export async function askFabry(question: any) {
  const syn = store.synthesis.value;
  const q = String(question || '').trim();
  if (!q || !syn || syn.status !== 'done' || !syn.chatId) return;
  if ((syn.followups || []).some((f: any) => f.status === 'streaming')) return; // one at a time
