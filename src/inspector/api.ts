...
function toUrl(p: string): string { return /^https?:\/\//.test(p) ? p : `${baseDomain}${p}`; }
function apiError(message: string, status: number): ApiError { const e = new Error(message) as ApiError; e.status = status; return e; }
...
export async function get(pathOrUrl: string, { signal: externalSignal }: RequestOpts = {}): Promise<any> {
  ...
  try {
    ...
    clearTimeout(timer);
    if ((err as Error).name === 'AbortError') { if (ext?.aborted) throw err; throw apiError('Request timed out after 30s', 0); }
    throw err;
  }
  clearTimeout(timer);
...
export async function safeListAll(pathOrUrl: string, opts?: RequestOpts): Promise<any[]> {
  ...
  catch (err) { if ((err as ApiError).status === 403 || (err as ApiError).status === 404) return []; throw err; }
}

export function whoami({ signal }: RequestOpts = {}): Promise<any> { return get('/api/v1/auth/user/', { signal }); }

// ---- forensics: core (GET) ----
export const getAnnotation = (id: ResourceRef, o?: RequestOpts): Promise<any> => get(`/api/v1/annotations/${id}`, o);
export const getAutomationBlocker = (url: string, o?: RequestOpts): Promise<any> => get(url, o); // url from annotation.automation_blocker
export const getContent = (id: ResourceRef, o?: RequestOpts): Promise<any> => get(`/api/v1/annotations/${id}/content`, o);
...
export const listNotes = (annId: ResourceRef, o?: RequestOpts): Promise<any[]> => safeListAll(`/api/v1/notes?${buildQuery({ annotation: annId, page_size: 100 })}`, o);
export const listWorkflowActivities = (annId: ResourceRef, o?: RequestOpts): Promise<any[]> => safeListAll(`/api/v1/workflow_activities?${buildQuery({ annotation: annId, page_size: 100 })}`, o);
export const listAuditLogs = (annId: ResourceRef, o?: RequestOpts): Promise<any[]> => safeListAll(`/api/v1/audit_logs?${buildQuery({ object_type: 'annotation', object_id: annId, page_size: 100 })}`, o);
// Hook logs live at /hooks/logs (NOT /hook_logs, which 404s). Only annotation/hook/queue filter.
export const listHookLogs = (annId: ResourceRef, o?: RequestOpts): Promise<any[]> => safeListAll(`/api/v1/hooks/logs?${buildQuery({ annotation: annId, page_size: 100 })}`, o);
export const listRuleExecutionLogs = (annId: ResourceRef, o?: RequestOpts): Promise<any[]> => safeListAll(`/api/v1/rules_execution_logs?${buildQuery({ annotation_id: annId, page_size: 100 })}`, o);
export const listWorkflowRuns = (annId: ResourceRef, o?: RequestOpts): Promise<any[]> => safeListAll(`/api/v1/workflow_runs?${buildQuery({ annotation: annId, page_size: 100 })}`, o);
export const listWorkflowSteps = (workflowId: ResourceRef, o?: RequestOpts): Promise<any[]> => safeListAll(`/api/v1/workflow_steps?${buildQuery({ workflow: workflowId, page_size: 100 })}`, o);
...
export const listAnnotationsByIds = (ids: ResourceRef[] | null | undefined, o?: RequestOpts): Promise<any> => get(`/api/v1/annotations?${buildQuery({ id: (ids || []).join(','), sideload: 'documents,queues', page_size: Math.max(1, (ids || []).length) })}`, o);

// Page resources list — one call for all pages (verified live 2026-07-04).
export const listPages = (annId: ResourceRef, o?: RequestOpts): Promise<any[]> => safeListAll(`/api/v1/pages?${buildQuery({ annotation: annId, page_size: 100 })}`, o);
...
export async function getBlob(pathOrUrl: string, { signal: externalSignal }: RequestOpts = {}): Promise<Blob> {
  const { signal, timer, externalSignal: ext } = combinedSignal(externalSignal);
  let res: Response;
  try {
...
export const listHooks = (queueId: ResourceRef, o?: RequestOpts): Promise<any[]> => safeListAll(`/api/v1/hooks?${buildQuery({ queue: queueId, page_size: 100 })}`, o);
export const listLabels = (o?: RequestOpts): Promise<any[]> => safeListAll(`/api/v1/labels?${buildQuery({ page_size: 100 })}`, o);
export const listRules = (queueId: ResourceRef, o?: RequestOpts): Promise<any[]> => safeListAll(`/api/v1/rules?${buildQuery({ queue: queueId, page_size: 100 })}`, o);
