...
export function buildPrintRequest({ html, title, uuid, now }) {
  return {
...
const realDeps = {
  ...
  sessionSet: (obj) => chrome.storage.session.set(obj),
...
// session in any case.
export async function openPrintTab({ html, title }, deps = realDeps) {
  if (!html) return null;
