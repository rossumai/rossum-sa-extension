...
export function planCollection({ hasNew, hasOld } = {}) {
...
export function isRaceLostError(err) {
...
export function mergeDeliverables(primary, legacy) {
  const stamp = (d) => (typeof d?.editedAt === 'number' ? d.editedAt : (typeof d?.createdAt === 'number' ? d.createdAt : -1));
  const byId = new Map();
...
export function legacyOnlyIds(primary, legacy) {
