...
export async function runImplement(deliverables, {
  ...
  for (const d of deliverables) {
    ...
    try {
      ...
      // implementLoop.js:28.
      tasks = planned.slice(0, maxPlanTasks).map((t) => mkTask(t, 'plan'));
      ...
      for (;;) {
        ...
        while ((task = tasks.find((t) => t.status === 'pending'))) {
          ...
          for (let attempt = 1; attempt <= maxAttemptsPerTask; attempt++) {
            ...
            catch (err) { impl = { writes: (err && err.writes) || [], summary: `Task attempt failed: ${err?.message || err}`, discovered: [] }; }
            ...
            const entry = { attempt, summary: impl.summary || '', verdict: v.verdict, learnings: v.evidence || '' };
