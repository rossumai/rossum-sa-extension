...
const READ_RE = /^(get|list|search|find|aggregate|read|fetch|whoami|healthz|render|extract|validate|generate|describe|count)/i;
const AGENT_INTERNAL = new Set([
  'load_skill', 'load_tool', 'ask_user_question', 'write_file',
  'execute_python', 'run_grep', 'run_jq', 'create_task', 'update_task', 'list_tasks',
]);

export function baseToolName(name) {
...
export function makeAuditFolder({ now = () => Date.now() } = {}) {
  ...
  return {
    ...
    feed(ev) {
      ...
      if (type === 'tool-output-available') { const e = byId.get(toolCallId); if (e && e.entry) e.entry.ok = true; return undefined; }
      return undefined;
    },
  };
}
