...
export function openPdfFlow(current, { onNote = () => {}, onWarnings = () => {} } = {}) {
  const all = store.deliverables.value;
  openPdfDialog(
    { deliverableTitle: current ? displayTitle(current) : 'this deliverable', count: all.length },
    ({ scope, options }) => runPdf({ current, scope, options, onNote, onWarnings }),
  );
}

export async function runPdf({ current, scope, options, onNote = () => {}, onWarnings = () => {} }) {
  onNote('busy');
  try {
    ...
    onNote(`could not open the print view: ${err && err.message ? err.message : err}`);
  }
}
