...
export async function runImplement(
  ...
  const mkTask = (t: any, origin: ImplementTask['origin']): ImplementTask => ({
    id: 'k' + ++seq,
    text: t.text,
  ...
  const dup = (tasks: ImplementTask[], text: unknown) =>
    tasks.some((t) => t.text.trim().toLowerCase() === String(text).trim().toLowerCase());
  ...
  for (const d of deliverables) {
    ...
    if (totalWrites >= maxTotalWrites) {
      onEvent(d.id, { status: 'blocked', error: 'Write budget reached.', done: true });
      continue;
    }
    ...
    try {
      ...
      let planError = null;
      try {
        planned = await planOne(d);
      } catch (err) {
        planError = (err as Error)?.message || String(err);
        planned = null;
      }
      ck();
      if (planError != null) {
        onEvent(d.id, { status: 'failed', error: `Planning failed: ${planError}`, done: true });
        continue;
      }
      ...
      for (;;) {
        ...
        while ((task = tasks.find((t) => t.status === 'pending'))) {
          ...
          for (let attempt = 1; attempt <= maxAttemptsPerTask; attempt++) {
            ...
            let impl;
            try {
              impl = await implementTaskOne(d, task, { attempt, journal: taskJournal, doneTasks });
            } catch (err) {
              impl = {
                writes: (err && (err as any).writes) || [],
                summary: `Task attempt failed: ${(err as Error)?.message || err}`,
                discovered: [],
              };
            }
            ...
            for (const disc of impl.discovered || []) {
              if (tasks.length >= maxTotalTasks) {
                onEvent(d.id, {
                  note: `Task cap (${maxTotalTasks}) reached — dropped discovered task: ${disc.text}`,
                });
                break;
              }
            ...
            if (v.verdict === 'pass') {
              task.status = 'done';
              onEvent(d.id, { summary: impl.summary });
              break;
            }
            if (checkErrored) {
              task.status = 'failed';
            ...
            const entry: Record<string, any> = {
              ...
              verdict: v.verdict,
              learnings: v.evidence || '',
            };
            taskJournal.push(entry);
            journal.push(entry);
        ...
        // ---- roll-up ----
        ck();
        if (status === 'blocked' || totalWrites >= maxTotalWrites) {
          status = 'blocked';
          break;
        }
        let rollup;
        let rollupErrored = false;
        try {
          rollup = await checkDeliverable(d);
        } catch (err) {
          rollup = {
            verdict: 'uncertain',
            evidence: `Roll-up check could not complete: ${(err as Error)?.message || err}`,
            chatId: null,
          };
          rollupErrored = true;
        }
        ...
        if (rollupErrored) {
          status = 'uncertain';
          break;
        } // transient gate error → do NOT spend a remediation round / more writes
        rounds += 1;
        if (rounds >= maxRollupRounds) {
          status = 'failed';
          break;
        }
        let remedy;
        try {
          remedy = await planOne(d);
        } catch {
          remedy = [];
        }
        ...
        for (const t of fresh) {
          if (tasks.length >= maxTotalTasks) {
            onEvent(d.id, {
              note: `Task cap (${maxTotalTasks}) reached — dropped remediation task: ${t.text}`,
            });
            break;
          }
