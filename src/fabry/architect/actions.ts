...
export async function loadArchitect() {
  ...
  try {
    ...
    store.loadError.value = (err as any)?.message || 'Could not load deliverables.';
  } finally {
    loading = false;
  }
...
export async function addDeliverable(text = '') {
  ...
  const d = { id: newId(), text: String(text || ''), order } as Deliverable;
  ...
  try {
    ...
    store.loadError.value = (err as any)?.message || 'Could not create deliverable.';
  }
}
...
export async function moveDeliverable(id: string, toIndex: number) {
...
export async function updateDeliverable(
  ...
  try {
    ...
    store.loadError.value = (err as any)?.message || 'Could not save edit.';
  }
}
...
export async function ensureRevisionText(deliverableId: string, revisionId: string) {
...
export async function openRevision(deliverableId: string, revisionId: string) {
...
function refineResult(id: string, acc: any) {
...
export async function refineTurn({
  ...
  try {
    ...
    if (signal?.aborted || (err as any)?.name === 'AbortError') return null;
    throw err;
  }
}
...
async function runOne(d: Deliverable, signal?: AbortSignal) {
  const chatId = await agentApi.createChat();
  if (signal?.aborted) return null;
...
function persist(id: string, r: any) {
  if (r.error) {
...
let implController: AbortController | null = null;
let implRunId = 0;

// READ-ONLY planning turn (DEFAULT persona — the model plans best autonomously; no writes).
async function planOne(d: Deliverable, signal?: AbortSignal) {
  const chatId = await agentApi.createChat();
  if (signal?.aborted) return null;
  const acc = newAcc();
...
async function implementTaskOne(
  d: Deliverable,
  task: any,
  { attempt, journal, doneTasks }: { attempt?: number; journal?: any[]; doneTasks?: any[] },
  signal?: AbortSignal,
) {
  const chatId = await agentApi.createChat();
  if (signal?.aborted) return null;
  ...
  try {
    ...
    if (err && typeof err === 'object') (err as any).writes = folder.writes;
    throw err;
  }
...
// persist on `done`.
function applyImplementPatch(id: string, patch: any) {
  const cur: any = store.implement.value[id] || {};
...
async function runImplementList(ds: Deliverable[]) {
