...
export const VERDICT = { pass: ['pass', '✓ Met'], fail: ['fail', '✗ Not met'], uncertain: ['uncertain', '? Uncertain'] };
export function CheckBadge({ result }) {
  if (result && result.running) return <span class="fabry-spec-pill run">{'Checking…'}</span>;
  const known = result && VERDICT[result.verdict];
...
function SectionHeader({ deliverable }) {
...
function ReviewHost({ deliverable, kind }) {
  return (
...
export default function SpecView() {
  ...
  useEffect(() => {
    store.setOutlineNavigator((slug, docId) => {
  ...
  useEffect(() => {
    const onKey = (e) => { if (e.key === 'Escape' && store.reviewTarget.value) store.setReviewTarget(null); };
...
export function SourceColumn({ sections, headerFor, docRef, onScroll }) {
  ...
  useEffect(() => {
    ...
    let seq = 0;
    const topOf = (el) => root.scrollTop + (el.getBoundingClientRect().top - root.getBoundingClientRect().top);
    const settle = (el, mine, left) => {
    ...
    const jumpToEl = (el, { instant = false } = {}) => {
      if (!el) return false;
      const mine = ++seq;
    ...
    const api = {
      ...
      scrollToDeliverable: (id, opts) => jumpToEl(
        [...root.querySelectorAll('[data-deliverable]')].find((el) => el.dataset.deliverable === id),
        opts,
      ),
      scrollToSlug: (slug, _prefix, opts = {}) => {
