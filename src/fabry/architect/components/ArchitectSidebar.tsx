  ...
  return (
    <div class="fabry-arch-summary">
      ...
      {met > 0 && (
        <>
          {' · '}
          <span class="ok">{met} met</span>
        </>
      )}
      {notmet > 0 && (
        <>
          {' · '}
          <span class="bad">{notmet} not met</span>
        </>
      )}
      {unc > 0 && (
        <>
          {' · '}
          <span class="warn">{unc} uncertain</span>
        </>
      )}
    </div>
  );
...
export default function ArchitectSidebar() {
  ...
  function openMenu(id: any, e: any) {
    ...
    try {
      ...
      if (lr) {
        const below = lr.bottom - kb.bottom;
        up = below < 96 && kb.top - lr.top > below;
      }
    } catch {
      /* no layout (jsdom) → open downward */
    }
    setMenuUp(up);
    setMenuId(id);
  }
  ...
  useEffect(() => {
    ...
    const onDown = (e: any) => {
      if (!e.target.closest || !e.target.closest('.fabry-arch-menu, .fabry-arch-kebab'))
        closeMenu();
    };
  ...
  function confirmDelete(id: any) {
    closeMenu();
    confirmModal('Delete deliverable', "Delete this deliverable? This can't be undone.", () =>
  ...
  return (
    <div class="fabry-arch-side">
          ...
          <Fragment key={d.id}>
            <div
              ...
              class={
                ...
                (store.spyTarget.value === d.id ||
                (!store.spyTarget.value && store.activeId.value === d.id)
                  ? ' active'
                  : '') +
                (dragId === d.id ? ' dragging' : '') +
                (menuId === d.id ? ' menuopen' : '') +
                (overId === d.id && dragId && dragId !== d.id
                  ? dragIndex >= 0 && dragIndex < i
                    ? ' dragover-after'
                    : ' dragover-before'
                  : '')
              }
              onClick={() => {
                ...
                store.navigateOutline(null, d.id);
              }}
              onKeyDown={(e) => {
            ...
            >
              ...
              {menuId === d.id && (
                <div
                  class={'fabry-arch-menu' + (menuUp ? ' up' : '')}
                  onClick={(e) => e.stopPropagation()}
                >
                  <button
                    type="button"
                    class="fabry-arch-menu-item"
                    disabled={running || implementRunning || results[d.id]?.running}
                    onClick={() => {
                      reRun(d.id);
                      closeMenu();
                    }}
                  ...
                  {implementAllowed && (
                    <button
                      type="button"
                      class="fabry-arch-menu-item"
                      disabled={implementRunning || running || results[d.id]?.running}
                      onClick={() => {
                        closeMenu();
                        openArmDialog(1, () => reImplement(d.id));
                      }}
                    >
                      {'Implement ▷'}
                    </button>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      promptModal(
                        'Rename deliverable',
                        {
                          initialValue: displayTitle(d),
                          placeholder: 'Deliverable title',
                          submitLabel: 'Rename',
                        },
                        (v) => renameDeliverable(d.id, v),
                      );
                    }}
                  >
                    {'Rename…'}
                  ...
                  <button
                    type="button"
                    class="fabry-arch-menu-item danger"
      ...
      <div class="fabry-arch-foot">
        ...
        <button type="button" class="fabry-arch-new" onClick={() => addDeliverable()}>
          {'＋ New deliverable'}
        </button>
      </div>
    </div>
