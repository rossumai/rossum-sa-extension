...
export async function openChat(chatId: string, { restore = false }: { restore?: boolean } = {}) {
...
async function streamTurn(
  chatId: string,
  content: string,
  { images, signal }: { images?: any[]; signal?: AbortSignal } = {},
) {
  const acc = newAcc();
...
export async function sendMessage(text: string, images: any[] = []) {
...
export function formatAnswers(answers: { question: string; answer: string }[]) {
...
export function answerQuestions(answers: { question: string; answer: string }[]) {
