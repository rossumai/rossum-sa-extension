...
export function isRossumTab(tab?: { url?: string } | null): boolean {
  return !!tab && detectSite(tab.url || '') === 'rossum';
}

export function viewState(tab?: { url?: string } | null): string {
  if (!tab) return 'no-tab';
  return isRossumTab(tab) ? 'ready' : 'unsupported';
}

export function sameTarget(a: any, b: any): boolean {
