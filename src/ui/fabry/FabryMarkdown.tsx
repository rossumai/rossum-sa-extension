...
function BlockView({ b, streaming }: { b: Block; streaming?: boolean }) {
  ...
  if (b.type === 'ul') return <ul>{b.items.map((it: Span[], i: number) => <li key={i}><Spans spans={it} /></li>)}</ul>;
  if (b.type === 'ol') return <ol>{b.items.map((it: Span[], i: number) => <li key={i}><Spans spans={it} /></li>)}</ol>;
    ...
    return (
      <div class={styles.tablewrap}>
        <table class={styles.table}>
          <thead><tr>{b.header.map((c: Span[], i: number) => <th key={i}><Spans spans={c} /></th>)}</tr></thead>
          <tbody>{b.rows.map((r: Span[][], i: number) => <tr key={i}>{r.map((c: Span[], j: number) => <td key={j}><Spans spans={c} /></td>)}</tr>)}</tbody>
        </table>
      </div>
    );
  }
...
export default function FabryMarkdown(
  { text, streaming }: { text?: string | null; streaming?: boolean },
) {
  return (
    <div class={styles.md}>
