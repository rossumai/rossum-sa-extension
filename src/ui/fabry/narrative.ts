...
/** An inline run of a narrative: plain text, or a `[e:<id>]` citation chip. */
export type CiteSegment = { type: 'text'; text: string } | { type: 'cite'; id: string };

/** A narrative line: a paragraph, or a bullet. */
export type NarrativeBlock = { type: 'p' | 'li'; segments: CiteSegment[] };

const CITE_RE = /\[e:([A-Za-z0-9_.:-]+)\]/g;

export function parseCitations(text: unknown, streaming = false): CiteSegment[] {
...
export function parseNarrative(text: unknown, streaming = false): NarrativeBlock[] {
  const s = typeof text === 'string' ? text : '';
  if (!s) return [];
