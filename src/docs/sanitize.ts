...
function scrubAttrs(el: Element, inSvg: boolean): void {
...
function clean(parent: Element, inSvg: boolean): void {
...
function parse(html: unknown, DP?: typeof DOMParser | null): Document {
...
export type SanitizeOptions = { DOMParserImpl?: typeof DOMParser | null };

export function sanitizeBody(html: unknown, { DOMParserImpl }: SanitizeOptions = {}): HTMLElement {
  const doc = parse(html, DOMParserImpl);
  clean(doc.body, false);
  return doc.body;
}

export function sanitizeHtml(html: unknown, opts?: SanitizeOptions): string {
