...
const WHERE =
  'this element is not supported here — a deliverable\u2019s status comes from its check verdict';
...
function notice(tag: string): string {
  return (
    `<span class="state-label state-error" data-state="error">` +
...
export function reportDocWarnings(
  env: any,
  file: unknown,
