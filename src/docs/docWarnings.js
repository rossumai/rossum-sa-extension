...
const normalize = (tag) => String(tag || '').toLowerCase().replace(/[^a-z]/g, '');

// The tag name at the start of an HTML fragment, or ''.
export function leadingTagName(html) {
  const m = String(html || '').trim().match(/^<\/?\s*([a-zA-Z][\w:-]*)/);
  return m ? m[1] : '';
}

export function isStateLabelish(tag) {
...
const lineOf = (token) => (token && token.map ? token.map[0] + 1 : 0);
...
export function reportDocWarnings(env, file, emit = console.warn) {
...
export default function docWarnings(md) {
  md.core.ruler.push('doc_warnings', (state) => {
    const env = state.env || {};
