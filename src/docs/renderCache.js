...
export function cacheKey({ id, text, dark, syncLines, withMermaid }) {
...
export function renderDocument({ id, text, mermaid = null, dark = false, syncLines = false }) {
  ...
  const env = { syncLines, mermaidErrors: [] };
  let entry;
  try {
    ...
    return { body: null, warnings: [`render failed: ${err && err.message ? err.message : err}`] };
  }
