...
function siblingBody(slug, text) {
  const key = slug + '\u0000' + text;
...
export function resolveSiblingHref(href, resolveDoc) {
...
function jumpTo(scroller, el, inset = NAV_INSET, { instant = false } = {}) {
...
export default function DocView({ sections = null, headerFor = null, docId = '', text = '', domain = '', token = '', onWarnings, onNavigate, resolveDoc, onScroll: onSpy, docRef }) {
    ...
    // measured: the scroll spy died permanently after one mode switch.
    {
      ...
      const readGeo = () => {
        ...
        geo = {
          ...
          headings: secs.flatMap((el) => [...el.querySelectorAll('h1[id], h2[id], h3[id]')].map((hEl) => ({
      ...
      const api = {
        ...
        scrollToSlug: (slug, prefix = '', opts) => jumpTo(root, resolveInPage(root, slug, prefix), NAV_INSET, opts),
        ...
        scrollToDeliverable: (id, opts) => jumpTo(root, [...root.querySelectorAll('[data-deliverable]')]
  ...
  function onClick(e) {
    ...
    if (href.startsWith('#')) {
      ...
      const target = resolveInPage(rootRef.current, href.slice(1), inSec ? prefixFor(inSec.dataset.slug) : '');
      e.preventDefault();
