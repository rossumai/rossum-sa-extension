...
import { buildSpecSections, type SpecDoc, type SpecResults } from './specDocument.js';
...
function verdictChip(result: { verdict?: string | null } | undefined) {
  const v = (result && result.verdict) as string;
  if (!VERDICT[v]) return '';
...
export function buildPrintDocument({
  deliverables = [],
  displayTitle = (d: SpecDoc) => d.title || 'Untitled',
  results = {},
  md,
  options = {},
