...
function labelSprite(text: string, color: string, bg: string): any {
  const padX = 11,
    padY = 6,
    font = 28,
    radius = 8;
...
function rgbaColor(s: string): any {
  const m = /rgba?\(([^)]+)\)/.exec(s || '');
...
const EDGE_HI_DARK = new THREE.Color(0.86, 0.9, 1.0); // ...and bright on a dark backdrop
...
export function fogRange(dist: number, r: number): { near: number; far: number } {
...
export function createScene(container: HTMLElement) {
  ...
  scene.fog = new THREE.Fog(
    new THREE.Color(cssVar('--bg-base', prefersDark() ? '#12121e' : '#f1f1f5')),
    1,
    4000,
  );
  ...
  let linkGeom: any = null,
    linkLines: any = null,
    baseLinkColors: any = null;
  let sim: any = null;
  type NodeCb = (id: string | null) => void;
  let hoverCb: NodeCb = () => {},
    clickCb: NodeCb = () => {};
  let raf: any = null,
    idleTimer: any = null;
  let focusTween: any = null;
  let fogRadius = 1; // current scene radius; set by fitToView, read by applyFog each frame
  let pinnedId: string | null = null; // set by a click; freezes hover so the dim stays while rotating
  let downX = 0,
    downY = 0; // pointerdown position, to tell a real click from a rotate/pan drag
  const raycaster = new THREE.Raycaster();
  const ndc = new THREE.Vector2();

  const endId = (e: any) => (typeof e === 'object' && e ? e.id : e);
  const isVisible = (type: string) => (visibleTypes as any)[type] !== false;
  ...
  function edgeColor(kind: string) {
    const s = (LINK_STYLE as any)[kind];
  ...
  function edgeVertexColor(kind: string, v: number) {
    ...
    return {
      r: c.r * (1 - amt) + dimC.r * amt,
      g: c.g * (1 - amt) + dimC.g * amt,
      b: c.b * (1 - amt) + dimC.b * amt,
    };
  }
  function labelColorFor() {
    return dark ? '#e6e6f2' : '#243044';
  }
  function labelBgFor() {
    return dark ? 'rgba(18,18,30,0.72)' : 'rgba(241,241,245,0.78)';
  }
  ...
  function applyTheme() {
    dark = prefersDark();
    if (scene.fog)
      scene.fog.color = new THREE.Color(cssVar('--bg-base', dark ? '#12121e' : '#f1f1f5'));
    if (linkGeom && baseLinkColors) {
      links.forEach((l, i) => {
        for (const v of [0, 1]) {
          const c = edgeVertexColor(l.kind, v);
          const rgb = i * 6 + v * 3;
          baseLinkColors[rgb] = c.r;
          baseLinkColors[rgb + 1] = c.g;
          baseLinkColors[rgb + 2] = c.b;
        }
      });
    }
    for (const m of meshes.values()) {
    ...
    if (linkGeom) {
      ...
      links.forEach((l, i) => {
        ...
        for (const v of [0, 1]) {
          ...
          col[rgba + 1] = tint
            ? baseLinkColors[rgb + 1] * (1 - amt) + tint.g * amt
            : baseLinkColors[rgb + 1];
          col[rgba + 2] = tint
            ? baseLinkColors[rgb + 2] * (1 - amt) + tint.b * amt
            : baseLinkColors[rgb + 2];
          col[rgba + 3] = alpha;
        }
      });
  ...
  function bfsInto(seeds: any[], maps: any[]) {
  ...
  function setHighlight(id: string | null, transitive?: boolean) {
    highlight.clear();
    if (id) {
      highlight.add(id);
  ...
  return {
    ...
    onClick(cb: (id: string | null) => void) {
      clickCb = cb || (() => {});
    },
    ...
    setVisibleTypes(vis: Record<string, boolean>) {
      visibleTypes = vis || {};
      applyVisibility();
      ...
      const visLinks = links.filter(
        (l) => isVisible(typeById.get(endId(l.source))) && isVisible(typeById.get(endId(l.target))),
      );
    ...
    destroy() {
      ...
      if (themeMq && themeMq.removeEventListener)
