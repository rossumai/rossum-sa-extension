...
/** The five node kinds the scene draws. `connector` was intentionally dropped. */
export type NodeType = 'organization' | 'workspace' | 'queue' | 'hook' | 'engine';

/** How a node got into the graph: containment, a reference, or a run_after chain. */
export type LinkKind = 'containment' | 'reference' | 'runAfter';

/** A curated `[label, value]` row shown in the detail card. */
...
// `rawId` may be null: the resulting id then matches nothing in `present`, so addLink
// no-ops — which is how a missing ref degrades to a missing edge.
function nodeId(type: string, rawId: string | null): string {
  return `${type}:${rawId}`;
}

function pairs(...rows: [string, unknown][]): DetailRow[] {
  return rows
...
export function buildGraph(raw: any): { nodes: GraphNode[]; links: GraphLink[] } {
  ...
  const hookById = new Map<string, any>();
  for (const hk of raw?.hooks || []) {
    const k = hk.id ?? idFromUrl(hk.url);
    if (k != null) hookById.set(String(k), hk);
  }
  const isDisabledHook = (id: unknown) => hookById.get(String(id))?.active === false;
  ...
  function effectivePredIds(predIds: unknown[], visited: Set<string>): string[] {
    const out: string[] = [];
