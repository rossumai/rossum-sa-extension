// src/audit/components/FabryPanel.tsx
...
import type { FabryTurn } from '../store.js';
...
function Turn({ turn }: { turn: FabryTurn }) {
  return (
    <div class="audit-fabry-turn">
      <div class="inspector-followup-q">
        <span class="inspector-followup-role">
          {turn.question == null ? 'Latest activity' : 'You'}
        </span>
...
// text at all.
export function previewText(f: any) {
  const summaries = (f.turns || []).filter((t: any) => t.question == null);
  if (!summaries.length) return null;
  const last = summaries[summaries.length - 1];
  if (last.state === 'streaming') {
    const line = String(last.text || '')
      .split('\n')[0]
      .trim();
  ...
  const lastGood = [...summaries].reverse().find(
    (t) =>
      t.state === 'done' &&
      String(t.text || '')
        .split('\n')[0]
...
export default function FabryPanel() {
  ...
  const toggle = () => {
    ...
    setOpen(next);
    if (next) {
      if (store.fabry.value.status === 'idle')
        runDefaultSummary(); // fallback (normally auto-run)
  ...
  useEffect(() => {
    if (open && stale && avail === 'available' && !busy && f.refreshFailedFor !== sig)
      refreshSummary();
  ...
  return (
    <div class="audit-fabry">
      <div class="audit-fabry-bar">
        <button
          type="button"
          class="audit-fabry-toggle"
          aria-expanded={open ? 'true' : 'false'}
          onClick={toggle}
        >
          <span class="audit-fabry-title">
            <span class="audit-fabry-mark">
              <FabryMark />
            </span>{' '}
            Audit insights
          </span>
          ...
          {!open && preview && stale ? (
            <span class="audit-fabry-stale">{'· view changed'}</span>
          ) : null}
          {!open && !preview ? (
            <span class="audit-fabry-hint">summarize &amp; ask about the loaded page</span>
          ) : null}
        </button>
        {open && lastDone ? (
          <button
            type="button"
            class="inspector-fold-btn audit-fabry-tx"
      ...
      {showTx && lastDone ? (
        <FabryTranscript
          reasoning={lastDone.reasoning}
          tools={lastDone.tools || []}
