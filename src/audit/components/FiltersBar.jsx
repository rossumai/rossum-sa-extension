...
function DebouncedInput({ value, type, placeholder, onCommit }) {
  const [v, setV] = useState(value);
  useEffect(() => { setV(value); }, [value]);          // re-sync on external change (Clear / source switch)
  useEffect(() => {
    const id = setTimeout(() => { if (v !== value) onCommit(v); }, 300);
    return () => clearTimeout(id);
  }, [v]);
  return <input class="input" type={type} placeholder={placeholder || ''} value={v} onInput={(e) => setV(e.target.value)} />;
}
...
export default function FiltersBar() {
  ...
  const clearFilters = () => {
    const cleared = { page: 1, cursor: null, search: '' };
  ...
  const hasActiveFilters = desc.filters.some((f) => !f.required && st[f.name]) || !!st.search;
          ...
          <label class="filter">
            ...
            {f.kind === 'select' ? (
              ...
              <DebouncedInput
                ...
                value={st[f.name]}
                onCommit={(val) => setFilter(f.name, val)}
              />
            )}
          </label>
        ))}
