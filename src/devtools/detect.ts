...
const ROUTES = [
  ...
  {
    type: 'rule',
    re: /\/queues\/\d+\/settings\/rules\/(\d+)\/detail(?:[/?#]|$)/,
    api: (id: string) => `/api/v1/rules/${id}`,
    label: 'Rule',
  },
  {
    type: 'queue',
    re: /\/queues\/(\d+)(?:[/?#]|$)/,
    api: (id: string) => `/api/v1/queues/${id}`,
    label: 'Queue',
  },
  {
    type: 'hook',
    re: /\/extensions\/my-extensions\/(\d+)(?:[/?#]|$)/,
    api: (id: string) => `/api/v1/hooks/${id}`,
    label: 'Hook',
  },
  {
    type: 'user',
    re: /\/settings\/users\/(\d+)(?:[/?#]|$)/,
    api: (id: string) => `/api/v1/users/${id}`,
    label: 'User',
  },
  ...
  {
    type: 'schema',
    re: /\/settings\/field-manager\/detail\/(\d+)(?:[/?#]|$)/,
    api: (id: string) => `/api/v1/schemas/${id}`,
    label: 'Schema',
  },
  {
    type: 'engine',
    re: /\/automation\/engines\/(\d+)(?:[/?#]|$)/,
    api: (id: string) => `/api/v1/engines/${id}`,
    label: 'Engine',
  },
  ...
  {
    type: 'annotation',
    re: ANNOTATION_PATH_RE,
    api: (id: string) => `/api/v1/annotations/${id}`,
    label: 'Annotation',
  },
];
...
function queueFromDocumentsSearch(search: string) {
...
export function detectResource(
  location: { pathname?: string; search?: string } | null /*, document */,
) {
  ...
  if (/^\/extensions\/my-extensions\/?$/.test(path))
    return { type: 'hook', apiPath: '/api/v1/hooks', label: 'Hooks', readOnly: true };
  if (/^\/settings\/users\/?$/.test(path))
    return { type: 'user', apiPath: '/api/v1/users', label: 'Users', readOnly: true };
  if (/^\/settings\/labels\/?$/.test(path))
    return { type: 'label', apiPath: '/api/v1/labels', label: 'Labels', readOnly: true };
  ...
  if (/^\/documents\/?$/.test(path)) {
    ...
    if (params.get('level') === 'all')
      return { type: 'organization', via: 'org', label: 'Organization' };
