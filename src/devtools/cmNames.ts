...
export function rossumNames(
  ...
  const plugin = ViewPlugin.fromClass(
    class {
      ...
      update(u: any) {
        const refreshed = u.transactions.some((t: any) =>
          t.effects.some((e: any) => e.is(refreshNames)),
        );
      ...
      build(view: any) {
        ...
        for (const pos of [...byLine.keys()].sort((a, b) => a - b)) {
          builder.add(
            pos,
            pos,
            Decoration.widget({ widget: new NameWidget(byLine.get(pos)!.join(' · ')), side: 1 }),
          );
        }
