...
const LABELS = {
  ...
  organization_groups: 'Organization group',
  inboxes: 'Inbox',
};
const TYPE = {
  queues: 'queue',
...
function titleSingular(collection: string): string {
  let s = collection;
  if (/(?:s|x|z|ch|sh)es$/.test(s))
    s = s.slice(0, -2); // boxes→box, inboxes→inbox, batches→batch
  else if (/ies$/.test(s))
    s = s.slice(0, -3) + 'y'; // policies→policy
